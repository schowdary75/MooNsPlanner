...
function runMigrations(db: Database.Database): void {
  ...
  const migrations: Migration[] = [
    ...
    // Central moon_photos registry; trip_photos + journey_photos reference via photo_id
    () => {
      // 1. Create the central photo registry
      db.exec(`
        CREATE TABLE IF NOT EXISTS moon_photos (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          provider TEXT NOT NULL,
          asset_id TEXT,
      ...
      `);
      db.exec(
        'CREATE UNIQUE INDEX IF NOT EXISTS idx_moon_photos_provider_asset ON moon_photos(provider, asset_id, owner_id) WHERE asset_id IS NOT NULL',
      );
      db.exec('CREATE INDEX IF NOT EXISTS idx_moon_photos_owner ON moon_photos(owner_id)');

      // 2. Migrate trip_photos → moon_photos + photo_id FK
      const tripPhotosExists = db
        .prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'trip_photos'")
        .get();
      if (tripPhotosExists) {
        ...
        if (assetCol) {
          ...
          // Qualified alias needed in JOIN context where both trip_photos and moon_photos have provider
          ...
          // Insert existing trip photo references into moon_photos
          db.exec(`
            INSERT OR IGNORE INTO moon_photos (provider, asset_id, owner_id, created_at)
          ...
          db.exec(`
            CREATE TABLE trip_photos_new (
              ...
              trip_id INTEGER NOT NULL REFERENCES trips(id) ON DELETE CASCADE,
              user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
              photo_id INTEGER NOT NULL REFERENCES moon_photos(id) ON DELETE CASCADE,
              shared INTEGER NOT NULL DEFAULT 1,
          ...
          db.exec(`
            ...
            FROM trip_photos tp
            JOIN moon_photos tkp ON tkp.provider = ${providerJoinExpr} AND tkp.asset_id = tp.${assetCol} AND tkp.owner_id = tp.user_id
          `);
        } else {
      ...
      // 3. Migrate journey_photos → moon_photos + photo_id FK
      ...
      if (journeyPhotosExists) {
        // Insert provider-based journey photos into moon_photos
        db.exec(`
          INSERT OR IGNORE INTO moon_photos (provider, asset_id, owner_id, width, height, created_at)
        ...
        `);
        // Insert local journey photos into moon_photos (each is unique)
        db.exec(`
          INSERT INTO moon_photos (provider, file_path, thumbnail_path, width, height, created_at)
        ...
        db.exec(`
          ...
          JOIN moon_photos tkp ON tkp.provider = jp.provider AND tkp.asset_id = jp.asset_id AND tkp.owner_id = jp.owner_id
        ...
        db.exec(`
          ...
          SELECT jp.entry_id, tkp.id, jp.caption, jp.sort_order, jp.shared, jp.created_at
          FROM journey_photos jp
          JOIN moon_photos tkp ON tkp.provider = 'local' AND tkp.file_path = jp.file_path
    ...
    () => {
      ...
      try {
        db.exec('ALTER TABLE moon_photos ADD COLUMN passphrase TEXT DEFAULT NULL');
      } catch (err: any) {
        if (!err.message?.includes('duplicate column name')) throw err;
      }
    },
    ...
    () =>
      db.exec(`
      CREATE TABLE IF NOT EXISTS moon_photo_cache_meta (
        cache_key  TEXT    PRIMARY KEY,
        content_type TEXT  NOT NULL DEFAULT 'image/jpeg',
        fetched_at INTEGER NOT NULL
      );
      CREATE INDEX IF NOT EXISTS idx_moon_photo_cache_meta_fetched_at ON moon_photo_cache_meta (fetched_at);
    `),
    ...
    () => {
      // AirTrail flight linkage on reservations (#214) — lets a moon transport
    ...
    () => {
      // Per-user opt-in for writing moon edits back to AirTrail (#1240). Default
      // off: AirTrail is the source of truth and moon never writes unless asked.
      try {
    ...
    // Video support (#823): the moon_photos registry held only images. media_type
    ...
    () => {
      for (const stmt of [
        "ALTER TABLE moon_photos ADD COLUMN media_type TEXT NOT NULL DEFAULT 'image'",
        'ALTER TABLE moon_photos ADD COLUMN duration_ms INTEGER',
      ]) {
        try {
          db.exec(stmt);
        } catch (err: any) {
    ...
    // Migration 157: plugin capabilities (from moon-plugin.json) — the client
    ...
    // Plugin dependencies (#plugins): a plugin's moon-plugin.json can now declare
    ...
    // Durable GDPR erasure queue (#plugins). When a moon account is deleted, every
    ...
    // The semver RANGE of moon versions a plugin declares it supports (its manifest's
    // `moon`, e.g. ">=3.2.0 <4.0.0"). The existing `min_moon_version` only carries the
    // lower bound, so it cannot express "stops working at 4.0" — which is precisely the
    // case the activation gate has to catch after a moon upgrade. Kept nullable: a plugin
    // installed before this column existed has no range recorded, and the gate refuses to
    // activate it rather than guessing (see moon_VERSION_UNKNOWN).
    () => {
      try {
        db.exec('ALTER TABLE plugins ADD COLUMN moon_range TEXT;');
      } catch (err) {
        console.warn('[migrations] Non-fatal migration step failed:', err);
      }
    },
