...
export function exportICS(tripId: string | number): { ics: string; filename: string } {
  ...
  const uid = (id: number, type: string) => `moon-${type}-${id}@moon`;
  ...
  // subscribers see the time in moon's zone. Falls back to a floating local time
  ...
  let ics = 'BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//moon//Travel Planner//EN\r\nCALSCALE:GREGORIAN\r\nMETHOD:PUBLISH\r\n';
  ics += `X-WR-CALNAME:${esc(trip.title || 'moon Trip')}\r\n`;
  ...
  const safeFilename = (trip.title || 'moon-trip').replace(/["\r\n]/g, '').replace(/[^\w\s.-]/g, '_');
