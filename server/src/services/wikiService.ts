...
const REPO = 'schowdary75/MooNs';
const RAW_BASE = `https://raw.githubusercontent.com/${REPO}/main/wiki`;
const TTL_MS = 60 * 60 * 1000;
const SLUG_RE = /^[A-Za-z0-9][A-Za-z0-9._-]*$/;

const WIKI_DIR = process.env.MOONS_WIKI_DIR ?? process.env.moon_WIKI_DIR ?? path.join(__dirname, '..', '..', '..', 'wiki');
...
async function fetchText(file: string): Promise<string> {
  ...
  try {
    const res = await fetch(`${RAW_BASE}/${encodeURIComponent(file)}`, {
      headers: { 'User-Agent': 'moon-help', Accept: 'text/plain' },
    });
    if (res.ok) {
