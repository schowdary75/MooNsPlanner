...
function generateAccessToken(): string {
  return 'moonoa_' + randomBytes(32).toString('hex');
}

function generateRefreshToken(): string {
  return 'moonrf_' + randomBytes(32).toString('hex');
}
...
export function createOAuthClient(
  ...
  const rawSecret   = isPublic ? null : 'mooncs_' + randomBytes(24).toString('hex');
...
export function rotateOAuthClientSecret(
  ...
  const rawSecret  = 'mooncs_' + randomBytes(24).toString('hex');
...
export function validateAuthorizeRequest(
  ...
  // RFC 8707 resource indicator: if provided, must identify the moon
  ...
  if (resource !== mcpResource) {
    return { valid: false, error: 'invalid_target', error_description: 'Requested resource must be the moon MCP endpoint' };
  }
