...
import type { moonPhoto } from '../../types';
...
import * as photoCache from './moonsPhotoCache';
import { ensureLocalThumbnail } from './thumbnailService';

// ── Lookup / Register ────────────────────────────────────────────────────

export function getOrCreatemoonPhoto(
  provider: string,
  assetId: string,
  ownerId: number,
  passphrase?: string,
  ...
  const existing = db.prepare(
    'SELECT id FROM moon_photos WHERE provider = ? AND asset_id = ? AND owner_id = ?'
  ).get(provider, assetId, ownerId) as { id: number } | undefined;
  if (existing) {
    if (passphrase) {
      db.prepare('UPDATE moon_photos SET passphrase = ? WHERE id = ?')
  ...
  const res = db.prepare(
    'INSERT INTO moon_photos (provider, asset_id, owner_id, passphrase, media_type) VALUES (?, ?, ?, ?, ?)'
  ).run(provider, assetId, ownerId, passphrase ? encrypt_api_key(passphrase) : null, mediaType);
  return Number(res.lastInsertRowid);
}

export function getOrCreateLocalmoonPhoto(
  filePath: string,
  ...
  const existing = db.prepare(
    "SELECT id FROM moon_photos WHERE provider = 'local' AND file_path = ?"
  ).get(filePath) as { id: number } | undefined;
  if (existing) return existing.id;

  const res = db.prepare(
    'INSERT INTO moon_photos (provider, file_path, thumbnail_path, width, height, media_type, duration_ms) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ).run('local', filePath, thumbnailPath || null, width || null, height || null, mediaType, durationMs ?? null);
  return Number(res.lastInsertRowid);
}

export function resolvemoonPhoto(photoId: number): moonPhoto | null {
  return db.prepare('SELECT * FROM moon_photos WHERE id = ?').get(photoId) as moonPhoto | undefined || null;
}
...
export async function streamPhoto(
  ...
  const photo = resolvemoonPhoto(photoId);
  if (!photo) {
    ...
    if (kind === 'thumbnail') {
      ...
      if (!thumbRel && !isVideo) {
        ...
        if (result) {
          ...
          db.prepare(
            'UPDATE moon_photos SET thumbnail_path = ?, width = COALESCE(width, ?), height = COALESCE(height, ?) WHERE id = ?'
...
// ── Update provider on existing moon_photo (for Immich upload sync) ─────

export function setmoonPhotoProvider(
  moonPhotoId: number,
  provider: string,
  assetId: string,
  ownerId: number,
  ...
  db.prepare(
    'UPDATE moon_photos SET provider = ?, asset_id = ?, owner_id = ? WHERE id = ?'
  ).run(provider, assetId, ownerId, moonPhotoId);
}

// ── Orphan cleanup ───────────────────────────────────────────────────────

export function deletemoonPhotoIfOrphan(photoId: number): void {
  ...
  `).get(photoId, photoId);
  if (stillUsed) return;
  db.prepare("DELETE FROM moon_photos WHERE id = ? AND provider != 'local'").run(photoId);
}
