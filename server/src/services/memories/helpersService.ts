...
export function canAccessUserPhoto(requestingUserId: number, ownerUserId: number, tripId: string, assetId: string, provider: string): boolean {
    ...
    if (tripId === '0') {
        const journeyPhoto = db.prepare(`
            ...
            JOIN moon_photos tkp ON tkp.id = gp.photo_id
            WHERE tkp.asset_id = ?
              AND tkp.provider = ?
              AND tkp.owner_id = ?
            LIMIT 1
    ...
    // Regular trip photos — join through moon_photos
    const sharedAsset = db.prepare(`
    SELECT 1
    FROM trip_photos tp
    JOIN moon_photos tkp ON tkp.id = tp.photo_id
    WHERE tp.user_id = ?
      AND tkp.asset_id = ?
      AND tkp.provider = ?
      AND tp.trip_id = ?
...
// ── Unified photo access check (moon_photos based) ──────────────────────

export function canAccessmoonPhoto(requestingUserId: number, moonPhotoId: number): boolean {
    const photo = db.prepare('SELECT * FROM moon_photos WHERE id = ?').get(moonPhotoId) as { id: number; provider: string; owner_id: number | null } | undefined;
    ...
    const tripAccess = db.prepare(`
        ...
        LIMIT 1
    `).get(moonPhotoId, requestingUserId, requestingUserId);
