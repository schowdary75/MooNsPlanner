...
import {
  getOrCreatemoonPhoto,
  getOrCreateLocalmoonPhoto,
  setmoonPhotoProvider,
  deletemoonPhotoIfOrphan,
...
// Per-entry photo view: join journey_entry_photos → journey_photos (gallery) → moon_photos.
...
const JP_JOIN = `journey_entry_photos jep
  ...
  JOIN moon_photos    tp ON tp.id  = gp.photo_id`;

// Per-journey gallery view: journey_photos → moon_photos (no entry context).
const GALLERY_SELECT = `
  gp.id, gp.journey_id, gp.photo_id, gp.caption, gp.shared, gp.sort_order, gp.created_at,
  tp.provider, tp.asset_id, tp.owner_id, tp.file_path, tp.thumbnail_path, tp.width, tp.height,
  tp.media_type, tp.duration_ms
`;
const GALLERY_JOIN = 'journey_photos gp JOIN moon_photos tp ON tp.id = gp.photo_id';
...
// Ensure a moon_photo_id is in the journey gallery; return its gallery row id.
function ensureInGallery(journeyId: number, moonPhotoId: number, caption?: string, shared?: number): number {
  const now = ts();
  const maxOrderRow = db
    .prepare('SELECT MAX(sort_order) as m FROM journey_photos WHERE journey_id = ?')
    .get(journeyId) as { m: number | null };
  db.prepare(
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `,
  ).run(journeyId, moonPhotoId, caption || null, shared ?? 0, (maxOrderRow?.m ?? -1) + 1, now);
  const row = db
    .prepare('SELECT id FROM journey_photos WHERE journey_id = ? AND photo_id = ?')
    .get(journeyId, moonPhotoId) as { id: number };
  return row.id;
}
...
export function addPhoto(
  ...
  const moonPhotoId = getOrCreateLocalmoonPhoto(filePath, thumbnailPath);
  const galleryId = db.transaction(() => ensureInGallery(entry.journey_id, moonPhotoId, caption))();
...
export function addProviderPhoto(
  ...
  const moonPhotoId = getOrCreatemoonPhoto(provider, assetId, userId, passphrase, mediaType);
...
export function uploadGalleryPhotos(
  ...
  for (const f of filePaths) {
    const moonPhotoId = getOrCreateLocalmoonPhoto(f.path, f.thumbnail, null, null, f.mediaType || 'image', f.durationMs ?? null);
    db.prepare(
      `
      INSERT OR IGNORE INTO journey_photos (journey_id, photo_id, shared, sort_order, created_at)
      VALUES (?, ?, 0, ?, ?)
    `,
    ).run(journeyId, moonPhotoId, nextOrder++, now);
    const row = db
      .prepare(`SELECT ${GALLERY_SELECT} FROM ${GALLERY_JOIN} WHERE gp.journey_id = ? AND gp.photo_id = ?`)
      .get(journeyId, moonPhotoId);
...
export function addProviderPhotoToGallery(
  ...
  if (!canEdit(journeyId, userId)) return null;
  const moonPhotoId = getOrCreatemoonPhoto(provider, assetId, userId, passphrase, mediaType);
  const galleryId = db.transaction(() => ensureInGallery(journeyId, moonPhotoId, caption))();
...
export function deleteGalleryPhoto(
  ...
  const moonRow = db.prepare('SELECT file_path, provider FROM moon_photos WHERE id = ?').get(row.photo_id) as
  ...
  deletemoonPhotoIfOrphan(row.photo_id);

  return { photo_id: row.photo_id, file_path: moonRow?.file_path ?? null };
}

export function setPhotoProvider(photoId: number, provider: string, assetId: string, ownerId: number) {
  // photoId = journey_photos.id (gallery row); look up the moon_photo_id
  const jp = db.prepare('SELECT photo_id FROM journey_photos WHERE id = ?').get(photoId) as
    | { photo_id: number }
    | undefined;
  if (!jp) return;
  setmoonPhotoProvider(jp.photo_id, provider, assetId, ownerId);
...
export function deletePhoto(
  ...
  return { id: row.id, photo_id: row.photo_id, file_path: moonRow?.file_path ?? null, journey_id: row.journey_id };
}
