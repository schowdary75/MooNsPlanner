...
export function buildUserAgent(instanceUrl: string | undefined): string {
  const base = 'MooNs Travel Planner (https://github.com/schowdary75/MooNsPlanner)';
...
// moon's internal language codes mostly coincide with valid BCP-47 codes, but a
...
// Operators behind locked-down egress — or running their own Overpass — can point moon
...
export async function resolveGoogleMapsUrl(
  ...
  if (!coords) {
    try {
      const pageRes = await followRedirects(resolvedUrl, {
        headers: { 'User-Agent': 'moon-Travel-Planner/1.0' },
      });
  ...
  const nominatimRes = await fetch(
    ...
    { headers: { 'User-Agent': 'moon-Travel-Planner/1.0' }, signal: AbortSignal.timeout(8000) },
  );
