 ...
 * code when AirTrail doesn't provide a name. Used for what moon displays/stores; the
...
export function mapFlightToReservation(raw: AirtrailFlightRaw): MappedReservation {
  // Prefer the scheduled (booked) time moon plans against, but fall back to the
 ...
 * remote changes (AirTrail exposes no updated_at/etag) and to suppress moon's own
...
export function canonicalHash(raw: AirtrailFlightRaw): string {
  const snapshot = {
    ...
    // whichever time moon actually shows triggers a re-sync — and existing flights
