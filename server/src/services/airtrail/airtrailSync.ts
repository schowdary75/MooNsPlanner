 ...
 * updated_at) and, when a flight is gone from AirTrail, keep the moon row but
 ...
 * GET object) and overwrite ONLY the fields moon manages. Everything else —
...
export function buildSavePayload(reservation: any, existing: AirtrailFlightRaw): AirtrailSavePayload | null {
  ...
  // Push the seat the user set in moon onto their own AirTrail seat (the one with
  ...
  // overwrite only what moon manages. `from`/`to`/`airline`/`aircraft` come back
  ...
  return {
    ...
    // Import reads the SCHEDULED time, so a moon edit must write back there too —
    ...
    // These are AirTrail-owned details moon doesn't surface in its edit UI — a moon
    // edit can leave them out of `metadata`. Preserve AirTrail's current value when
    // moon has none rather than nulling it out (#1240). Use airline_code (not the
