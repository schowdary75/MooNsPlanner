  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      if (fs.existsSync(uploadsDir)) {
        // Exclude the place-photo and moon-memory caches: both are re-derivable
        ...
        archive.glob(
          ...
          {
            cwd: uploadsDir,
            ignore: ['photos/google/**', 'photos/moon/**', 'backups/**', 'restore-*/**'],
            nodir: true,
            dot: true,
          },
          { prefix: 'uploads' },
    ...
    try {
      ...
      for (const table of requiredTables) {
        if (!tableNames.has(table)) {
          fs.rmSync(extractDir, { recursive: true, force: true });
          return { success: false, error: `Uploaded database is missing required table: ${table}. This does not appear to be a moon backup.`, status: 400 };
        }
      }
    } catch (err) {
      fs.rmSync(extractDir, { recursive: true, force: true });
