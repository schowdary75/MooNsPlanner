 ...
 * moon_API_DOCS_ENABLED. Must run BEFORE app.init() — SwaggerModule.setup
 ...
 *
 * The bearer button works with a plain moon session JWT: extractToken
...
export function setupApiDocs(app: INestApplication): void {
  ...
  const config = new DocumentBuilder()
    .setTitle('moon API')
    .setDescription(
      'The REST API the moon web app itself runs on. Authenticate with a session JWT — '
      + 'either the `moon_session` cookie (same browser) or an `Authorization: Bearer <jwt>` header.',
    )
    .setVersion(version)
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'session')
    .addCookieAuth('moon_session')
