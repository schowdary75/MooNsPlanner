 ...
 * client) keeps the standard moon `{ error }` 404 envelope. The `@Catch(NotFoundException)`
 * is more specific than the global moonExceptionFilter, so Nest routes 404s here
 * while every other error still flows through moonExceptionFilter.
 */
...
export class SpaFallbackFilter implements ExceptionFilter {
    ...
    // Non-production, or a non-GET miss: keep the standard moon 404 envelope
    // (identical to what moonExceptionFilter produces for a NotFoundException).
