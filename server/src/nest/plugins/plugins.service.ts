 ...
 * enabled (moon_PLUGINS_ENABLED). No execution here — the isolated runtime,
...
export interface PluginListItem {
  ...
  /** The moon range the plugin declares it supports; null if it declared none. */
  moonRange: string | null;
  /** The running moon, so the UI can say "needs X, you have Y" without doing semver. */
  hostVersion: string;
...
export class PluginsService {
  list(): { enabled: boolean; devLink: boolean; plugins: PluginListItem[] } {
    const rows = db
      .prepare(
        `SELECT id, name, description, type, icon, version, status, enabled, last_error, reviewed_at, source_repo,
                permissions, capabilities, dependencies, operator_egress, moon_range,
    ...
    const plugins: PluginListItem[] = rows.map((r) => {
      ...
      const dependencyStatus: PluginDependencyStatus = !hostSatisfies(r.moon_range)
        ? 'hostIncompatible'
