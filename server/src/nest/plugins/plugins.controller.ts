 ...
 * moonExceptionFilter passes a `{ error, code }` body through verbatim.
 *
...
export class PluginsController {
  ...
  async upload(@UploadedFile() file?: Express.Multer.File) {
    ...
    try {
      ...
      throw registryFailure(e, 'upload failed'); // carries moon_VERSION_INCOMPATIBLE
    }
  }

  /**
   * DEV-ONLY: register a plugin from a LOCAL built directory and hot-reload it
   * against real data. Gated by moon_PLUGINS_DEV_LINK on top of admin + kill-switch.
   */
  @Post('link')
  @HttpCode(200)
  async link(@Body() body: { path?: string }) {
    if (!pluginsEnabled()) throw new HttpException({ error: 'Plugins are disabled by server configuration' }, 503);
    if (!devLinkEnabled()) throw new HttpException({ error: 'Dev-link is disabled (set moon_PLUGINS_DEV_LINK=1)' }, 403);
    const dir = body?.path?.trim();
    if (!dir) throw new HttpException({ error: 'path is required' }, 400);
    try {
      return await this.runtime.link(dir);
    } catch (e) {
      throw registryFailure(e, 'link failed'); // carries moon_VERSION_INCOMPATIBLE
    }
  }
