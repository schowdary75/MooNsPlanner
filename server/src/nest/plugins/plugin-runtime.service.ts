...
export class PluginDependencyError extends Error {
  constructor(
    ...
    readonly detail: { addons?: string[]; missing?: PluginDependency[]; versionMismatch?: VersionMismatch[]; moonRange?: string | null; hostVersion?: string } = {},
  ) {
    super(message);
...
export class PluginRuntimeService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly supervisor = new PluginSupervisor((id, granted) => createRealRpcHost(id, granted, this), {
    ...
    onLog: (id, level, msg) => {
      ...
      try {
        ...
        // uncapped table grows without bound in the shared moon.db. Keep only the
  ...
  async exportUserData(userId: number): Promise<Array<{ pluginId: string; data?: unknown; pending?: boolean; settings?: Record<string, unknown>; oauthConnected?: boolean }>> {
    ...
    // Fold in the host-side per-user data moon stores itself (what erasePluginUserData
   ...
   * not activate. Checks run most- to least-severe: moon-version compatibility →
  ...
  private assertActivatable(id: string, installed: Map<string, PluginDepRow>, consentWiden: boolean): void {
    const row = db.prepare('SELECT permissions, granted_permissions, dependencies, moon_range FROM plugins WHERE id = ?').get(id) as
      | { permissions: string; granted_permissions: string; dependencies: string | null; moon_range: string | null }
      | undefined;
    ...
    // run on this moon must never be offered a permission dialog, because consenting to
    // it would not make the plugin start. It is also the gate that catches the case
    // install can't — moon was upgraded PAST the plugin's declared upper bound, so code
    // that was legitimately installed no longer supports the host it's sitting on.
    if (!row.moon_range) {
      throw new PluginDependencyError(
        `plugin ${id} does not declare which moon versions it supports`,
        'moon_VERSION_UNKNOWN',
        { moonRange: null, hostVersion: hostVersion() },
      );
    }
    if (!hostSatisfies(row.moon_range)) {
      throw new PluginDependencyError(
        `plugin ${id} requires moon ${row.moon_range} — this is moon ${hostVersion()}`,
        'moon_VERSION_INCOMPATIBLE',
        { moonRange: row.moon_range, hostVersion: hostVersion() },
      );
    }
  ...
  async update(id: string, opts?: { version?: string; retrustKey?: string }): Promise<{ version: string; activated: boolean; newPermissions: string[]; newEgress: string[] }> {
    ...
    // Pick the newest version that this moon can actually run, not simply the newest one
  ...
  /**
   * The version an "update" should install: the newest one compatible with this moon.
  ...
  private async resolveUpdateTarget(id: string, installedVersion: string | null): Promise<string> {
    if (!this.registry) throw new Error('registry service unavailable');
    const target = await this.registry.resolveVersion(id); // throws moon_VERSION_INCOMPATIBLE if nothing fits
    if (installedVersion && semver.valid(installedVersion) && !semver.gt(target.version, installedVersion)) {
      throw new RegistryError(
        `no update available for ${id} on moon ${hostVersion()} — ${installedVersion} is already the newest compatible version`,
   ...
   * and starts an fs.watch that re-forks on rebuild. Gated behind moon_PLUGINS_DEV_LINK
  ...
  async link(sourceDir: string): Promise<{ id: string; version: string; replaced: boolean }> {
    if (!devLinkEnabled()) throw new Error('dev-link is disabled (set moon_PLUGINS_DEV_LINK=1)');
    if (!path.isAbsolute(sourceDir)) throw new Error('the dev-link path must be absolute');
    const manifestPath = path.join(sourceDir, 'moon-plugin.json');
    if (!fs.existsSync(manifestPath)) throw new Error(`no moon-plugin.json at ${sourceDir}`);
    const manifest = parseManifest(parseJsonText(fs.readFileSync(manifestPath, 'utf8')), { requiremoon: true });
  ...
  async uninstall(id: string, deleteData: boolean): Promise<void> {
    ...
    // same reason as the settings fields: these are moon's config ABOUT the plugin, and
