...
import { isValidmoonRange, minmoonOf } from './host-compat';
import type { NotifEventType } from '../../../services/notificationPreferencesService';

/**
 * Parse + validate a plugin's moon-plugin.json (#plugins, M4). Kept deliberately
...
export interface PluginManifest {
  ...
  /** The raw semver RANGE of moon versions this plugin supports (">=3.2.0 <4.0.0"). */
  moon?: string;
  /** Same range, normalized: a valid+satisfiable one, or null. What the host gates on. */
  moonRange: string | null;
  /** The range's lower bound, for display ("Requires moon 3.2.0+"). Derived, never authored. */
  minmoonVersion?: string;
 ...
 * Validate a manifest.
 *
 * `requiremoon` is the INSTALL front doors (registry, sideload, dev-link): a plugin
 * that doesn't say which moon versions it supports may not be installed at all.
 *
 ...
 */
export function parseManifest(raw: unknown, opts?: { requiremoon?: boolean }): PluginManifest {
  ...
  const moon = optStr(m.moon);
  const moonRange = isValidmoonRange(moon) ? moon : null;
  if (opts?.requiremoon && !moonRange) {
    throw new ManifestError(
      moon
        ? `invalid "moon" version range "${moon}" (expected a satisfiable semver range, e.g. ">=3.2.0 <4.0.0")`
        : 'missing "moon" version range — declare the moon versions this plugin supports, e.g. ">=3.2.0 <4.0.0"',
    );
  }
  return {
    id,
    ...
    minmoonVersion: moonRange ? minmoonOf(moonRange) : undefined,
    nativeModules: false,
    permissions,
    egress,
