 ...
 * disk"). Reads each subdir's moon-plugin.json and upserts a registry row as
...
export function discoverPlugins(db: BetterSqlite3.Database): { discovered: string[]; skipped: string[] } {
  ...
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(dir, 'moon-plugin.json');
  ...
  if (existing) {
    db.prepare(
      `UPDATE plugins SET name = ?, description = ?, type = ?, icon = ?, version = ?, api_version = ?,
         min_moon_version = ?, moon_range = ?, permissions = ?, capabilities = ?, dependencies = ?, operator_egress = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
    ).run(m.name, m.description ?? null, m.type, m.icon ?? 'Blocks', m.version, m.apiVersion, m.minmoonVersion ?? null, m.moonRange, JSON.stringify(m.permissions), JSON.stringify(m.capabilities), dependencies, m.operatorEgress ? 1 : 0, m.id);
  } else {
    db.prepare(
      ...
      `INSERT INTO plugins (id, name, description, type, icon, version, api_version, min_moon_version, moon_range, permissions, capabilities, dependencies, operator_egress, granted_permissions, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '', 'inactive')`,
    ).run(m.id, m.name, m.description ?? null, m.type, m.icon ?? 'Blocks', m.version, m.apiVersion, m.minmoonVersion ?? null, m.moonRange, JSON.stringify(m.permissions), JSON.stringify(m.capabilities), dependencies, m.operatorEgress ? 1 : 0);
  }
