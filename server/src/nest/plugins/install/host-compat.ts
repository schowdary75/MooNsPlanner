...
/**
 * Host-version compatibility for plugins (#plugins). A manifest declares the moon
 * versions it supports as a semver RANGE (`"moon": ">=3.2.0 <4.0.0"`); this module
 * is the single place that answers "what moon is running" and "does that range admit
...
/** The running moon version (same source as the rest of the app). */
 ...
 *    incompatible the moment moon shipped a release candidate.
 *  - `includePrerelease` fixes that but orders 4.0.0-rc.1 BEFORE 4.0.0, so it satisfies
 *    "<4.0.0" — letting a plugin that disclaims moon 4 load on a build that already
 *    carries moon 4's breaking changes. That is the precise failure this gate exists to
 *    prevent.
 *
...
export function normalizedHost(): string | null {
  ...
  if (!coerced && !warnedUnparseable) {
    ...
    console.warn(`[plugins] APP_VERSION "${raw}" is not a semver version — plugin moon-compatibility checks are disabled`);
  }
  return coerced;
}
 ...
 */
export function isValidmoonRange(r: unknown): r is string {
...
/**
 * The lowest moon version a range admits — what the UI shows as "Requires moon x+".
 * Undefined for a range with no real lower bound (`*`, `<4.0.0`), because "requires
 * moon 0.0.0+" is noise rather than information.
 */
export function minmoonOf(range: string): string | undefined {
  if (!isValidmoonRange(range)) return undefined;
...
/**
 * Whether the running moon satisfies `range`. The host arrives here already coerced to a
