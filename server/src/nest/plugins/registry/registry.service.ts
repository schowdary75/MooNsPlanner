...
/**
 * moon-side of the plugin registry (#plugins, M5). Fetches the single aggregated
...
const REGISTRY_URL =
  process.env.moon_PLUGIN_REGISTRY_URL ||
  'https://raw.githubusercontent.com/schowdary75/MooNsPlanner-Plugins/main/dist/index.json';
...
interface RegistryVersion {
  ...
  sha256: string;
  /**
   * The version's declared moon range, mirroring its manifest's `moon`. Authoritative
   * when present. `minmoonVersion`/`maxmoonVersion` are the legacy shape — a lower bound
  ...
  minmoonVersion?: string | null;
  maxmoonVersion?: string | null;
  size?: number;
  apiVersion?: number;
...
/** A registry entry's standing against the running moon, resolved server-side. */
export interface HostCompat {
  /** The latest version's declared moon range, or null for a legacy (min/max-only) entry. */
  moon: string | null;
  hostVersion: string;
  /** Whether the LATEST version can be installed on this moon. */
  compatible: boolean;
...
export class PluginRegistryService {
  ...
  async fetchRegistry(force = false): Promise<Registry> {
    ...
    try {
      ...
      const headers: Record<string, string> = { 'User-Agent': 'moon-Server' };
      if (force) {
  ...
  > {
    ...
    return reg.plugins.map((p) => {
      ...
      return {
        ...
        minmoonVersion: latest?.minmoonVersion ?? null,
  ...
  /**
   * How this entry stands against the running moon, computed SERVER-side. The client has
   ...
   *
   * `latestCompatible` is the useful half: when the newest version has outrun this moon,
    ...
    if (latest) {
      try {
        const { bytes } = await safeDownload(
          rawFileUrl(entry.repo, latest.commitSha, 'moon-plugin.json'),
          MANIFEST_MAX_BYTES,
        );
   ...
   * `constraint` (any version if omitted) AND is compatible with the running moon
   * version (`minmoonVersion`/`maxmoonVersion`). Throws if the plugin isn't in the
  ...
  async resolveVersion(id: string, constraint?: string): Promise<RegistryVersion> {
    ...
    if (!v) {
      throw new RegistryError(
        constraint
          ? `no version of ${id} satisfies "${constraint}" and moon ${hostVersion()}`
          : `no version of ${id} is compatible with moon ${hostVersion()}`,
        'moon_VERSION_INCOMPATIBLE',
      );
    }
    return v;
  }

  /** The newest version of `entry` that satisfies `constraint` AND admits the running moon. */
   ...
   * happily installed a plugin that declared it doesn't support this moon.
   */
  private selectVersion(entry: RegistryEntry, opts?: { version?: string; constraint?: string }): RegistryVersion {
    if (opts?.version) {
      ...
      if (!hostCompatible(ver, normalizedHost())) {
        throw new RegistryError(
          `${entry.id} ${ver.version} requires moon ${moonRequirement(ver)} — this is moon ${hostVersion()}`,
          'moon_VERSION_INCOMPATIBLE',
        );
      }
      return ver;
    ...
    // shown, rather than a bare "not found" — the fix is a moon upgrade, not a retry.
    const latest = entry.versions[0];
    if (latest && !opts?.constraint) {
      throw new RegistryError(
        `${entry.id} ${latest.version} requires moon ${moonRequirement(latest)} — this is moon ${hostVersion()}`,
        'moon_VERSION_INCOMPATIBLE',
      );
    }
    throw new RegistryError(
      opts?.constraint
        ? `no version of ${entry.id} satisfies "${opts.constraint}" and moon ${hostVersion()}`
        : `no version of ${entry.id} is compatible with moon ${hostVersion()}`,
      'moon_VERSION_INCOMPATIBLE',
    );
  }
  ...
  async install(
    ...
    const entry = reg.plugins.find((p) => p.id === id);
    if (!entry) throw new RegistryError(`plugin ${id} not in registry`);
    const ver = this.selectVersion(entry, opts); // throws moon_VERSION_INCOMPATIBLE before we fetch a byte
    ...
    try {
      ...
      if (!pluginRoot) throw new RegistryError('archive contains no moon-plugin.json');

      // 4. re-validate the bundled manifest + 5. native re-scan
      const manifest = parseManifest(
        parseJsonText(fs.readFileSync(path.join(pluginRoot, 'moon-plugin.json'), 'utf8')),
        { requiremoon: true },
      );
      ...
      // plugin that declares "<4.0.0" passes the pre-download filter on moon 4 and is
      // caught only here.
      assertHostCompatible(manifest.moonRange, id);
  ...
  stageUpload(bytes: Buffer): { id: string; version: string; root: string; stagingDir: string } {
    ...
    try {
      ...
      if (!root) throw new RegistryError('archive contains no moon-plugin.json');
      const manifest = parseManifest(parseJsonText(fs.readFileSync(path.join(root, 'moon-plugin.json'), 'utf8')), {
        requiremoon: true,
      });
      assertHostCompatible(manifest.moonRange, manifest.id);
...
/**
 * Whether a registry version admits the running moon.
 *
 * The version's `moon` range is authoritative when the entry carries one. Entries
 * published before that field existed only carry min/max bounds, so those remain the
 * fallback — but they are a strictly weaker check (maxmoonVersion is inclusive and is
...
function hostCompatible(v: RegistryVersion, host: string | null): boolean {
  ...
  if (v.moon) return hostSatisfies(v.moon, host);
  if (v.minmoonVersion && semver.valid(v.minmoonVersion) && semver.lt(host, v.minmoonVersion)) return false;
  if (v.maxmoonVersion && semver.valid(v.maxmoonVersion) && semver.gt(host, v.maxmoonVersion)) return false;
  return true;
}

/**
 * Refuse an artifact whose declared moon range doesn't admit the running host. Shared by
...
export function assertHostCompatible(range: string | null, id: string): void {
  ...
  throw new RegistryError(`${id} requires moon ${range} — this is moon ${hostVersion()}`, 'moon_VERSION_INCOMPATIBLE');
}

/**
 * How a version's moon requirement reads in an error/UI string. Each bound is optional —
 * an entry may declare only a ceiling, or (once `moon` is absent too) nothing at all — so
 * this composes whatever bounds exist rather than interpolating a missing one as "null".
 */
function moonRequirement(v: RegistryVersion): string {
  if (v.moon) return v.moon;
  const bounds = [
    v.minmoonVersion ? `>=${v.minmoonVersion}` : null,
    v.maxmoonVersion ? `<=${v.maxmoonVersion}` : null,
  ].filter(Boolean);
...
function locateManifestDir(staging: string): string | null {
  if (fs.existsSync(path.join(staging, 'moon-plugin.json'))) return staging;
  ...
  for (const s of subs) {
    ...
    if (fs.existsSync(path.join(p, 'moon-plugin.json'))) return p;
  }
  return null;
}
