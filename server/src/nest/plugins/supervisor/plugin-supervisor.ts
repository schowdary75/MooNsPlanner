...
const DEFAULTS: Required<SupervisorTuning> = {
  ...
  maxRssBytes: (Number(process.env.moon_PLUGIN_MAX_RSS_MB) || 300) * 1024 * 1024,
};
...
export class PluginSupervisor {
  ...
  private spawn(sup: Supervised): void {
    ...
    const child = fork(entry, [sup.id, codeDir], {
      ...
      env: {
        ...
        ...(process.env.moon_PLUGIN_ALLOW_PRIVATE_EGRESS
          ? { moon_PLUGIN_ALLOW_PRIVATE_EGRESS: process.env.moon_PLUGIN_ALLOW_PRIVATE_EGRESS }
          : {}),
      },
    });
    sup.child = child;
  ...
  private recordLog(sup: Supervised, level: string, msg: string, meta?: unknown): void {
    ...
    if (sup.droppedLogs > 0) {
      ...
      sup.droppedLogs = 0;
      this.hooks.onLog?.(sup.id, 'warn', `[moon] ${dropped} log line(s) dropped (plugin log rate limit exceeded)`);
    }
