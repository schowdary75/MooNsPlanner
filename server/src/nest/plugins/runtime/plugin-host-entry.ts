...
const pluginId = process.argv[2] || process.env.moon_PLUGIN_ID || 'unknown';
...
/**
 * Make `require('moon-plugin-sdk')` resolve inside the child WITHOUT the plugin
 ...
 * `const { definePlugin } = require('moon-plugin-sdk')` working in production.
 * Subpaths (`moon-plugin-sdk/testing`) are build/test-time tools and fail with a
...
function installSdkInjection(requirePlugin: NodeJS.Require): void {
  ...
  nodeModule._load = function (request: string, parent: unknown, isMain: boolean): unknown {
    if (request === 'moon-plugin-sdk') return shim;
    if (request.startsWith('moon-plugin-sdk/')) {
      throw new Error(`${request} is a build/test-time module — only 'moon-plugin-sdk' itself is injected inside moon`);
    }
...
async function handleInvoke(req: { id: string; method: string; params: Record<string, unknown> }): Promise<void> {
  ...
  try {
    ...
    if (req.method === 'invoke.route') {
      ...
      // A moon account was erased. Userless (like a job) — the handler only learns
 ...
 * moon_PLUGIN_ALLOW_PRIVATE_EGRESS=on to permit private/internal targets (e.g. a
...
function installEgressGuard(egress: string[]): void {
  ...
  const blockPrivate = (process.env.moon_PLUGIN_ALLOW_PRIVATE_EGRESS ?? '').toLowerCase() !== 'on';
