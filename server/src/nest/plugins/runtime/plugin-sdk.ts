 ...
 * version. The published `@moon/plugin-sdk` (M6) will re-export these types; for
...
export interface NotificationMessage {
  /** The moon event that produced this, e.g. `trip_invite`. */
  event: string;
  title: string;
  body: string;
  /** Absolute deep link back into moon, when the event has one. */
  url?: string;
  tripName?: string;
}

/**
 * Deliver moon notifications over a channel moon doesn't ship (Gotify, Pushover, …).
...
/** One row of extra place info moon renders natively (reviews/ratings/links/…). */
...
/** A validation/warning a plugin raises on a trip; moon surfaces it in the planner. */
...
/** One row of extra info moon renders under a journal entry (same shape as PlaceDetailItem). */
...
export interface PluginDefinition {
  ...
  /** GDPR erasure: a moon account was deleted — remove everything you hold about it
