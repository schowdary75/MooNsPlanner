...
// moon's REAL origin, so we must never let the browser RENDER it as a document
...
// so a forwarded header can never leak a moon session or be replayed. Signature +
...
const REDIRECT_BASE = 'https://moon.invalid';
...
export class PluginsProxyController {
  ...
  async proxy(@Param('pluginId') pluginId: string, @Req() req: Request, @Res() res: Response): Promise<void> {
    ...
    try {
      ...
      // Never let a non-redirect reply render as a document at moon's origin
