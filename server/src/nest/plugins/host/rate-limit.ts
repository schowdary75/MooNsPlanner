 ...
 * in-flight counter caps concurrency, turning "one plugin freezes moon for every
...
export const DEFAULT_RPC_LIMIT: RpcLimitConfig = {
  burst: Number(process.env.moon_PLUGIN_RPC_BURST) || 60,
  perSec: Number(process.env.moon_PLUGIN_RPC_PER_SEC) || 20,
  maxInFlight: Number(process.env.moon_PLUGIN_RPC_INFLIGHT) || 16,
};
...
export const DEFAULT_LOG_LIMIT = {
  burst: Number(process.env.moon_PLUGIN_LOG_BURST) || 50,
  perSec: Number(process.env.moon_PLUGIN_LOG_PER_SEC) || 10,
};
