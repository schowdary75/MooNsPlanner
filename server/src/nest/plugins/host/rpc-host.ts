 ...
 * the host is the sole holder of the moon.db handle and the broadcast fns.
 *
...
export interface HostDeps {
  ...
  data: PluginDataDb;
  /** Read-only handle to the core moon.db, used ONLY through the typed readers here. */
  db: CoreDb;
...
// Mirrors the STRING_LIMITS the places REST controller enforces (the @moon/shared
...
// Same idea for trips — the @moon/shared schema leaves the title/description open,
...
export class PluginRpcHost {
  ...
  constructor(
    ...
    // input against the SAME @moon/shared schema the web app uses, then gate on
    ...
    if (has('notify:send')) {
      ...
      this.methods.set('notify.send', (p, uid) => {
        ...
        const input = asPayload(p.input);
        // Emoji-stripped so a bell/push notification matches moon's lucide-only UI (also
  ...
  /**
   * The @moon/shared write schemas don't carry the string-length caps the REST
