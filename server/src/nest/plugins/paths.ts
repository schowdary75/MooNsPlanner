...
export function pluginsCodeRoot(): string {
  return process.env.moon_PLUGINS_DIR || path.join(DATA_ROOT, 'plugins');
}
export function pluginsDataRoot(): string {
  return process.env.moon_PLUGINS_DATA_DIR || path.join(DATA_ROOT, 'plugins-data');
}

/** A plugin's installed code directory (contains moon-plugin.json + server/index.js). */
 ...
 * NOT be granted the data root (moon.db, .jwt_secret, .encryption_key live there).
 */
 ...
 * dir). Result: the child can no longer read moon.db / the secret files or shell
 * out, closing the direct-filesystem and RCE escapes that bypass the RPC layer.
 * Empty (opt-out) when moon_PLUGIN_PERMISSIONS=off.
 */
let warnedPermissionsOff = false;
export function pluginPermissionArgs(pluginId: string): string[] {
  if ((process.env.moon_PLUGIN_PERMISSIONS ?? 'on').toLowerCase() === 'off') {
    // The OS permission jail is the boundary that stops an installed plugin from
    // reading moon.db / the secret files / shelling out. Turning it off makes plugin
    ...
    if (!warnedPermissionsOff) {
      ...
      console.warn('[plugins] moon_PLUGIN_PERMISSIONS=off — the OS permission jail is DISABLED; installed plugins run with full Node access to this process. Only use this on a machine you fully trust.');
    }
    return [];
  }
  ...
  return [
    ...
    // which also holds the `data` symlink to moon.db and the secret files.
