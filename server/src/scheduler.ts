...
// Version check: daily at 9 AM — notify admins if a new moon release is available
...
// The TTL must exceed any realistic offline window: the moon client replays
...
// moon photo cache cleanup: every 2 hours — evict disk files and DB rows past their 1h TTL
let moonsPhotoCacheTask: ScheduledTask | null = null;

function startMoonsPhotoCacheCleanup(): void {
  if (moonsPhotoCacheTask) { moonsPhotoCacheTask.stop(); moonsPhotoCacheTask = null; }

  // Run once immediately on startup to evict any entries left over from a previous run
  try {
    const { sweepExpired } = require('./services/memories/moonsPhotoCache');
    sweepExpired();
  } catch { /* cache dir may not exist yet — harmless */ }

  moonsPhotoCacheTask = cron.schedule('0 */2 * * *', () => {
    try {
      const { sweepExpired } = require('./services/memories/moonsPhotoCache');
      sweepExpired();
    } catch (err: unknown) {
      logError(`moon photo cache cleanup: ${err instanceof Error ? err.message : err}`);
    }
  });
}
...
export { start, stop, startDemoReset, startTripReminders, startTodoReminders, startVersionCheck, startIdempotencyCleanup, purgeExpiredIdempotencyKeys, startMoonsPhotoCacheCleanup, startPlacePhotoCacheCleanup, startAirTrailSync, loadSettings, saveSettings, VALID_INTERVALS };
