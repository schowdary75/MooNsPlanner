 ...
 *   docker exec -it moons node server/reset-admin.js
 *   docker exec -it -e RESET_ADMIN_EMAIL=me@example.com -e RESET_ADMIN_PASSWORD=secret moons node server/reset-admin.js
 *
 * Defaults to admin@moons.local with a generated password (printed below). The
 * account is flagged must_change_password, so you are prompted to set a new one
 * on first login. Honours moons_DB_FILE the same way the server does.
 */
...
const email = process.env.RESET_ADMIN_EMAIL || 'admin@moons.local';
...
const dbPath = process.env.moons_DB_FILE || path.join(__dirname, 'data/travel.db');
