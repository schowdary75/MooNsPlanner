 ...
 *
 * Re-encrypts all at-rest secrets in the moons database from one ENCRYPTION_KEY
 ...
 *
 * Usage (Docker):
 *   docker exec -it moons node --import tsx scripts/migrate-encryption.ts
 *
...
async function main() {
  console.log('=== moons Encryption Key Migration ===\n');
  ...
  db.transaction(() => {
    ...
    // --- moons_photos: passphrase ---
    const photos = db.prepare('SELECT id, passphrase FROM moons_photos WHERE passphrase IS NOT NULL').all() as { id: number; passphrase: string }[];
    for (const row of photos) {
      const newVal = migrateApiKeyValue(row.passphrase, `moons_photos[${row.id}].passphrase`);
      if (newVal !== null) {
        db.prepare('UPDATE moons_photos SET passphrase = ? WHERE id = ?').run(newVal, row.id);
      }
    }
  })();
