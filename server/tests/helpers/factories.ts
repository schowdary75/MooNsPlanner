...
export function createMcpToken(
  ...
  const rawToken = overrides.rawToken ?? `moons_test_${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
export function addTripPhoto(
  ...
  // Insert into moons_photos first (central registry)
  db.prepare(
    'INSERT OR IGNORE INTO moons_photos (provider, asset_id, owner_id) VALUES (?, ?, ?)'
  ).run(provider, assetId, userId);
  const moonsPhoto = db.prepare(
    'SELECT id FROM moons_photos WHERE provider = ? AND asset_id = ? AND owner_id = ?'
  ...
  ).run(tripId, userId, moonsPhoto.id, opts.shared ? 1 : 0, opts.albumLinkId ?? null);
  return db.prepare(`
