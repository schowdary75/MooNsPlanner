...
const { testDb } = vi.hoisted(() => {
  ...
  db.exec(`CREATE TABLE plugins (
    ...
    -- Activation refuses a plugin that declares no moons range, so the fixtures default to a
    ...
    -- itself is exercised in "moons host-version gating" below, with explicit ranges.
    moons_range TEXT DEFAULT '>=3.0.0',
...
beforeAll(() => {
  codeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'moonsplug-rt-code-'));
  dataRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'moonsplug-rt-data-'));
  process.env.moons_PLUGINS_DIR = codeRoot;
  process.env.moons_PLUGINS_DATA_DIR = dataRoot;
  process.env.moons_PLUGINS_ENABLED = 'true';
...
afterAll(async () => {
  ...
  delete process.env.moons_PLUGINS_ENABLED;
...
describe('PluginRuntimeService (M2 end-to-end)', () => {
  ...
  it('onModuleInit is a no-op when the runtime is disabled', () => {
    process.env.moons_PLUGINS_ENABLED = 'false';
...
describe('PluginRuntimeService.update (re-consent gate)', () => {
  // update() now asks the registry which version this moons can actually run before it
 ...
 *
 * Install alone can't be the whole answer: a plugin installed legitimately on moons 3.3
 ...
 */
describe('moons host-version gating', () => {
  const seedRanged = (id: string, moonsRange: string | null, enabled = 0) => {
    fs.mkdirSync(path.join(codeRoot, id, 'server'), { recursive: true });
    fs.writeFileSync(path.join(codeRoot, id, 'server', 'index.js'), 'module.exports = { async onLoad() {} };');
    testDb.prepare("INSERT INTO plugins (id, status, enabled, permissions, granted_permissions, config, moons_range) VALUES (?, ?, ?, '[]', '[]', '{}', ?)")
      .run(id, enabled ? 'active' : 'inactive', enabled, moonsRange);
  };
  const cleanup = (...ids: string[]) => { for (const id of ids) testDb.prepare('DELETE FROM plugins WHERE id = ?').run(id); };

  afterEach(() => { delete process.env.APP_VERSION; });

  it('refuses to activate a plugin this moons has outgrown, and says so with a code', async () => {
    ...
    expect(err.detail).toMatchObject({ moonsRange: '>=3.2.0 <4.0.0', hostVersion: '4.0.0' });
  ...
  it('refuses a plugin that never declared a range at all', async () => {
    ...
    await expect(new PluginRuntimeService().activate('undeclared')).rejects.toMatchObject({ code: 'moons_VERSION_UNKNOWN' });
    cleanup('undeclared');
  });
  ...
  it('checks the version BEFORE permissions — an unrunnable plugin gets no consent dialog', async () => {
    ...
    testDb.prepare("INSERT INTO plugins (id, status, enabled, permissions, granted_permissions, config, moons_range) VALUES ('both-wrong','inactive',0,'[\"db:own\"]','[]','{}','>=3.2.0 <4.0.0')").run();
    await expect(new PluginRuntimeService().activate('both-wrong')).rejects.toMatchObject({ code: 'moons_VERSION_INCOMPATIBLE' });
    cleanup('both-wrong');
  });
});
