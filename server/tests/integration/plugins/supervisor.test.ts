...
beforeAll(() => {
  codeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'moonsplug-code-'));
  dataRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'moonsplug-pdata-'));
...
describe('PluginSupervisor — isolated runtime', () => {
  ...
  it('injects require(moons-plugin-sdk) — a scaffold-style plugin loads with no node_modules', async () => {
    const events: Array<{ topic: string; data: unknown }> = [];
    sup = makeSupervisor(events);

    writePlugin(
      'scaffolded',
      `const { definePlugin, PLUGIN_API_VERSION } = require('moons-plugin-sdk');
      let testingError = '';
