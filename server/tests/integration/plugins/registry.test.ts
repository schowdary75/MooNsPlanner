/**
 * moons-side registry service (#plugins, M5): browse the aggregated registry and
...
const { testDb } = vi.hoisted(() => {
  ...
  db.exec(`
    CREATE TABLE plugins (id TEXT PRIMARY KEY, name TEXT, description TEXT, type TEXT, icon TEXT, version TEXT,
      api_version INTEGER, min_moons_version TEXT,moonNs_range TEXT, permissions TEXT, capabilities TEXT DEFAULT '{}', dependencies TEXT DEFAULT '{}', operator_egress INTEGER DEFAULT 0, granted_permissions TEXT, status TEXT, enabled INTEGER DEFAULT 0, config TEXT,
...
// Every artifact declares a `moons` range by default — install REQUIRES one, so a fixture
// without it would be testing the version gate rather than whatever the test is about.
// Pass an explicit `moons` (or null, spread last) to exercise the gate itself.
function makeArtifact(manifest: object): Buffer {
  const withmoons = { moonNs: '>=3.2.0 <4.0.0', ...manifest };
  const files = [
    { name: 'plug-abc/', type: '5' as const, data: '' },
    { name: 'plug-abc/moons-plugin.json', type: '0' as const, data: JSON.stringify(witmoonNs) },
...
const REGISTRY = {
  ...
  plugins: [
    {
      id: 'flight-tracker', name: 'Flight', author: 'Acme', description: 'flights', repo: 'acme/moons-flight',
      type: 'widget', icon: 'Plane', reviewedAt: '2026-06-20',
      versions: [{ version: '1.0.0', gitTag: 'v1.0.0', commitSha: 'a'.repeat(40), downloadUrl: 'https://codeload.github.com/acme/moons-flight/tar.gz/aaaa', sha256: '', mimoonNsVersion: '3.2.0' }],
    },
  ],
};
...
beforeEach(() => {
  ...
  process.env.moons_PLUGINS_DATA_DIR = dataRoot;
  process.env.moons_PLUGINS_DIR = codeRoot;
...
afterEach(() => {
  ...
  delete process.env.moons_PLUGINS_DATA_DIR;
  delete process.env.moons_PLUGINS_DIR;
...
describe('PluginRegistryService', () => {
  it('browse maps the aggregated registry to metadata', async () => {
    ...
    expect(list).toEqual([
      expect.objectContaining({ id: 'flight-tracker', name: 'Flight', latest: '1.0.0', minmoonsVersion: '3.2.0', reviewedAt: '2026-06-20' }),
    ]);
  });
  ...
  it('browse exposes a screenshot url pinned at the reviewed commit', async () => {
    const list = await svc.browse();
    expect(list[0].screenshotUrl).toBe(`https://raw.githubusercontent.com/acme/moons-flight/${'a'.repeat(40)}/docs/screenshot.png`);
  });
  ...
  it('browse prefers a build-injected screenshotUrl over the docs/screenshot.png guess', async () => {
    const injected = 'https://raw.githubusercontent.com/acme/moons-flight/bbbb/docs/img1.png';
  ...
  it('detail merges registry metadata with a live manifest preview', async () => {
    ...
    expect(safeDownload).toHaveBeenCalledWith(
      `https://raw.githubusercontent.com/acme/moons-flight/${'a'.repeat(40)}moonNs-plugin.json`,
      expect.any(Number),
    );
    expect(d).toMatchObject({
      id: 'flight-tracker', repo: 'acme/moons-flight', latest: '1.0.0', reviewedAt: '2026-06-20',
      screenshotUrl: `https://raw.githubusercontent.com/acme/moons-flight/${'a'.repeat(40)}/docs/screenshot.png`,
      manifest: {
  ...
  it('installs a pinned version end to end (verify -> extract -> register inactive)', async () => {
    ...
    expect(fs.existsSync(path.join(codeRoot, 'flight-tracker', 'moons-plugin.json'))).toBe(true);
    ...
    expect(row).toMatchObject({ status: 'inactive', source_repo: 'acme/moons-flight', source_commit: 'a'.repeat(40) });
  ...
  it('sideload: stage + commit installs an uploaded plugin INACTIVE as local:upload', () => {
    ...
    expect(fs.existsSync(path.join(codeRoot, 'my-upload', 'moons-plugin.json'))).toBe(true);
  ...
  it('sideload: rejects an archive without a manifest', () => {
    ...
    expect(() => svc.stageUpload(empty)).toThrow(/moons-plugin\.json/);
  });
  ...
  it('rejects an archive without a manifest', async () => {
    ...
    await expect(svc.install('flight-tracker')).rejects.toThrow(/no moons-plugin.json/);
  });
...
describe('re-trust (assertRetrustable)', () => {
  ...
  async function rotateTo(b: { pubB64: string; sign: (x: Buffer) => string }, a = signingKey()) {
    stageSignedArtifact(a.pubB64, a.sign);
    await svc.install('flight-tracker');
    testDb.prepare("UPDATE plugins SET source_repo='acme/moons-flight' WHERE id='flight-tracker'").run();
    stageSignedArtifact(b.pubB64, b.sign);
    return a;
  }
...
describe('PluginRegistryService.resolveVersion (latest compatible)', () => {
  const MULTI = {
    ...
    plugins: [{
      ...
      versions: [
        { version: '2.1.0', gitTag: 'v2.1.0', commitSha: 'a'.repeat(40), downloadUrl: 'https://codeload.github.com/a/b/tar.gz/x', sha256: '', minmoonsVersion: '3.4.0' },
        { version: '2.0.0', gitTag: 'v2.0.0', commitSha: 'a'.repeat(40), downloadUrl: 'https://codeload.github.com/a/b/tar.gz/y', sha256: '', minmoonsVersion: '3.2.0' },
        { version: '1.5.0', gitTag: 'v1.5.0', commitSha: 'a'.repeat(40), downloadUrl: 'https://codeload.github.com/a/b/tar.gz/z', sha256: '', minmoonsVersion: '3.0.0' },
      ],
    }],
  };
  const stub = () => { __clearRegistryCacheForTests(); vi.stubGlobal('fetch', vi.fn(async () => ({ ok: true, json: async () => MULTI }) as unknown as Response)); };

  it('picks the highest version compatible with the running moons', async () => {
...
/**
 * The install must be satisfied by the running moons — on EVERY path.
 *
 ...
 */
describe('moons-version gating on install', () => {
  const MULTI = {
    schemaVersion: 1,
    plugins: [{
      id: 'multi', name: 'Multi', author: 'a', description: 'many versions', repo: 'a/b', type: 'integration',
      versions: [
        { version: '2.1.0', gitTag: 'v2.1.0', commitSha: 'a'.repeat(40), downloadUrl: 'https://codeload.github.com/a/b/tar.gz/x', sha256: '', minmoonsVersion: '3.4.0', moonNs: '>=3.4.0 <4.0.0' },
        { version: '2.0.0', gitTag: 'v2.0.0', commitSha: 'a'.repeat(40), downloadUrl: 'https://codeload.github.com/a/b/tar.gz/y', sha256: '', minmoonsVersion: '3.2.0', moonNs: '>=3.2.0 <4.0.0' },
      ],
    }],
  };
  ...
  const stageMulti = (version: string, idx: number) => {
    const artifact = makeArtifact({ id: 'multi', name: 'Multi', version, type: 'integration', moons: MULTI.plugins[0].versions[idx]moonNs });
    const sha = createHash('sha256').update(artifact).digest('hex');
    MULTI.plugins[0].versions[idx].sha256 = sha;
    safeDownload.mockResolvedValue({ bytes: artifact, sha256: sha });
  };

  it('install-latest takes the newest version this moons can RUN, not the newest published', async () => {
  ...
  it('refuses install-latest outright when NO published version fits', async () => {
    ...
    await expect(svc.install('multi')).rejects.toMatchObject({ code: 'moons_VERSION_INCOMPATIBLE' });
    expect(safeDownload).not.toHaveBeenCalled(); // refused BEFORE a byte is fetched
  });

  it('refuses an explicitly-pinned version this moons cannot run', async () => {
    process.env.APP_VERSION = '3.3.0'; stubMulti();
    await expect(svc.install('multi', { version: '2.1.0' })).rejects.toMatchObject({ code: 'moons_VERSION_INCOMPATIBLE' });
    expect(safeDownload).not.toHaveBeenCalled();
  });

  it('honours an EXCLUSIVE upper bound, which minmoonsVersion alone cannot express', async () => {
    // Both versions declare "<4.0.0" and neither carries a maxmoonsVersion — under the old
    // min-only check moons 4 looked compatible with everything.
    process.env.APP_VERSION = '4.0.0'; stubMulti();
    await expect(svc.install('multi')).rejects.toMatchObject({ code: 'moons_VERSION_INCOMPATIBLE' });
  });

  it('reports the newest version this moons CAN run, so the UI can offer it', async () => {
    process.env.APP_VERSION = '3.3.0'; stubMulti();
  ...
  it('tolerates a legacy entry with NO lower bound at all (minmoonsVersion: null)', async () => {
    // The schema allows a null floor, so nothing may assume a string here: the legacy check
    // must skip the bound rather than compare against it, and the "requires moons …" message
    ...
    const noFloor = {
      ...
      plugins: [{
        ...
        versions: [{ version: '1.0.0', gitTag: 'v1.0.0', commitSha: 'a'.repeat(40), downloadUrl: 'https://codeload.github.com/a/b/tar.gz/x', sha256: '', minmoonsVersion: null, mamoonNsVersion: '3.0.0' }],
      }],
    };
    vi.stubGlobal('fetch', vi.fn(async () => ({ ok: true, json: async () => noFloor }) as unknown as Response));

    // Only the ceiling applies, and moons 3.3 is past it — refused, with a readable reason.
    const err = await svc.install('nofloor').catch((e) => e);
    expect(err).toMatchObject({ code: 'moons_VERSION_INCOMPATIBLE' });
  ...
  it('falls back to the legacy min/max bounds for an entry published without a range', async () => {
    process.env.APP_VERSION = '3.1.0'; // the fixture entry declares minmoonsVersion 3.2.0, nomoonNs
    __clearRegistryCacheForTests();
    await expect(svc.install('flight-tracker')).rejects.toMatchObject({ code: 'moons_VERSION_INCOMPATIBLE' });
  });
  ...
  it("re-gates on the ARTIFACT's own range — the index metadata is only a pre-download filter", async () => {
    ...
    __clearRegistryCacheForTests();
    const artifact = makeArtifact({ id: 'flight-tracker', name: 'Flight', version: '1.0.0', type: 'widget', moons: '>=3.2.0 <3.4.0' });
    const sha = createHash('sha256').update(artifact).digest('hex');
    REGISTRY.plugins[0].versions[0].sha256 = sha;
    safeDownload.mockResolvedValue({ bytes: artifact, sha256: sha });
    await expect(svc.install('flight-tracker')).rejects.toMatchObject({ code: 'moons_VERSION_INCOMPATIBLE' });
  ...
  it('refuses an artifact that declares no range at all', async () => {
    __clearRegistryCacheForTests();
    const artifact = makeArtifact({ id: 'flight-tracker', name: 'Flight', version: '1.0.0', type: 'widget', moons: undefined });
    const sha = createHash('sha256').update(artifact).digest('hex');
    REGISTRY.plugins[0].versions[0].sha256 = sha;
    safeDownload.mockResolvedValue({ bytes: artifact, sha256: sha });
    await expect(svc.install('flight-tracker')).rejects.toThrow(/missing "moons"/);
  });

  describe('sideload', () => {
    it('refuses an uploaded archive this moons cannot run, and cleans up staging', async () => {
      process.env.APP_VERSION = '3.9.0';
      const bytes = makeArtifact({ id: 'my-upload', name: 'Up', version: '1.0.0', type: 'integration', moons: '>=3.0.0 <3.5.0' });
      expect(() => svc.stageUpload(bytes)).toThrow(expect.objectContaining({ code: 'moons_VERSION_INCOMPATIBLE' }));
      const staging = path.join(dataRoot, '.staging');
    ...
    it('refuses an uploaded archive that declares no range', () => {
      const bytes = makeArtifact({ id: 'my-upload', name: 'Up', version: '1.0.0', type: 'integration', moons: undefined });
      expect(() => svc.stageUpload(bytes)).toThrow(/missing "moons"/);
    });
  });
  ...
  it('installWithDependencies installs the version it RESOLVED, not versions[0]', async () => {
    ...
    process.env.APP_VERSION = '3.3.0'; stubMulti();
    const artifact = makeArtifact({ id: 'multi', name: 'Multi', version: '2.0.0', type: 'integration', moons: '>=3.2.0 <4.0.0' });
    const sha = createHash('sha256').update(artifact).digest('hex');
 ...
 *
 * A block describes a refused REGISTRY update. Once an admin hands moons the bytes directly,
