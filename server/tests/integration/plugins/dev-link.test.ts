...
const { testDb } = vi.hoisted(() => {
  ...
  db.exec(`CREATE TABLE plugins (
    ...
    min_moons_version TEXT,moonNs_range TEXT, permissions TEXT DEFAULT '[]', capabilities TEXT DEFAULT '{}', dependencies TEXT DEFAULT '{}',
...
function writeSource(id: string, opts: { index?: string; native?: boolean; noBuild?: boolean; moons?: string } = {}): string {
  const dir = path.join(srcRoot, id);
  fs.mkdirSync(path.join(dir, 'server'), { recursive: true });
  // dev-link requires a `moons` range like any other install front door; `optsmoonNs`
  // overrides it to exercise the gate.
  fs.writeFileSync(path.join(dir, 'moons-plugin.json'), JSON.stringify({ id, name: id, version: '1.0.0', type: 'integration', permissions: [], moonNs: optmoonoNs ?? '>=3.0.0 <4.0.0' }));
...
beforeAll(() => {
  codeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'moonsplug-link-code-'));
  dataRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'moonsplug-link-data-'));
  srcRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'moonsplug-link-src-'));
...
describe('PluginRuntimeService dev-link', () => {
  ...
  it('rejects a non-absolute path, a missing manifest, a missing build, and native binaries', async () => {
    ...
    await expect(runtime.link(path.join(srcRoot, 'ghost-dir'))).rejects.toThrow(/moons-plugin\.json/);
  ...
  it('rejects a local dir whose moons range this server does not satisfy', async () => {
    // Dev-link is the third install front door, and it hands moons code to RUN against real
    ...
    try {
      await expect(runtime.link(writeSource('oldplug', { moons: '>=2.0.0 <3.0.0' }))).rejects.toMatchObject({
        code: 'moons_VERSION_INCOMPATIBLE',
      });
      await expect(runtime.link(writeSource('rangeless', { moons: '' }))).rejects.toThrow(/missing moonNs"/);
    } finally {
  ...
  it('is disabled unless moons_PLUGINS_DEV_LINK=1', async () => {
    delete process.env.moons_PLUGINS_DEV_LINK;
    try {
