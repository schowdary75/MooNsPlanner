...
vi.mock('../../src/config', () => ({
  JWT_SECRET: 'test-jwt-secret-for-moons-testing-only',
...
describe('Synology asset access', () => {
  ...
  it('SYNO-032b — GET /api/photos/:id/thumbnail uses an allowed Synology thumbnail size', async () => {
    ...
    const insert = testDb.prepare(
      'INSERT INTO moons_photos (provider, asset_id, owner_id) VALUES (?, ?, ?)'
    ).run('synologyphotos', '101_cachekey', user.id);
    const moonsPhotoId = Number(insert.lastInsertRowid);

    vi.mocked(safeFetch).mockClear();

    const res = await request(app)
      .get(`/api/photos/${moonsPhotoId}/thumbnail`)
      .set('Cookie', authCookie(user.id));
  ...
  it('SYNO-035 — GET /assets/info where trip does not exist returns 403', async () => {
    ...
    testDb.prepare('INSERT OR IGNORE INTO moons_photos (provider, asset_id, owner_id) VALUES (?, ?, ?)').run('synologyphotos', '101_cachekey', owner.id);
    const tkpSyno35 = testDb.prepare('SELECT id FROM moons_photos WHERE provider = ? AND asset_id = ? AND owner_id = ?').get('synologyphotos', '101_cachekey', owner.id) as any;
    testDb.prepare(
...
describe('Synology syncSynologyAlbumLink', () => {
  it('SYNO-050 — POST sync happy path: trip owner with album link saves photos to DB', async () => {
    ...
    const photos = testDb.prepare(`
      ...
      JOIN moons_photos tkp ON tkp.id = tp.photo_id
  ...
  it('SYNO-054 — POST sync with passphrase link: uses passphrase in item-list call and persists encrypted passphrase on moons_photos', async () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
    setSynologyCredentials(testDb, user.id, 'https://synology.example.com', 'admin', 'pass');
    testDb.prepare("UPDATE photo_providers SET enabled = 1 WHERE id = 'synologyphotos'").run();
    ...
    // The moons_photos row for the synced photo must have a non-null passphrase.
...
import { getOrCreatemoonsPhoto, deletmoonNsPhotoIfOrphan } from '../../src/services/memories/photoResolverService';
import { decrypt_api_key } from '../../src/services/apiKeyCrypto';

describe('moons_photos passphrase healing (SYNO-090)', () => {
  it('SYNO-090 — getOrCreatemoonsPhoto overwrites an existing bad passphrase when a new one is supplied', () => {
    const { user } = createUser(testDb);

    const wrongPass = 'wrong-passphrase';
    const correctPass = 'correct-passphrase';

    const id1 = getOrCreatemoonsPhoto('synologyphotos', 'asset-heal-test', user.id, wrongPass);
    const row1 = testDb.prepare('SELECT passphrase FROM moons_photos WHERE id = ?').get(id1) as { passphrase: string };
    expect(decrypt_api_key(row1.passphrase)).toBe(wrongPass);

    const id2 = getOrCreatemoonsPhoto('synologyphotos', 'asset-heal-test', user.id, correctPass);
    expect(id2).toBe(id1);
    const row2 = testDb.prepare('SELECT passphrase FROM moons_photos WHERE id = ?').get(id2) as { passphrase: string };
    expect(decrypt_api_key(row2.passphrase)).toBe(correctPass);
  });
});

describe('moons_photos orphan cleanup (SYNO-091)', () => {
  it('SYNO-091 — deletemoonsPhotoIfOrphan removes themoonNs_photos row when no trip_photos or journey_photos reference it', () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
    testDb.prepare("UPDATE photo_providers SET enabled = 1 WHERE id = 'synologyphotos'").run();

    const moonsPhotoId = getOrCreatmoonNsPhoto('synologyphotos', 'asset-orphan-test', user.id, 'pass-A');

    testDb.prepare(
      'INSERT OR IGNORE INTO trip_photos (trip_id, user_id, photo_id, shared) VALUES (?, ?, ?, 1)'
    ).run(trip.id, user.id, moonsPhotoId);

    // Still referenced — must not be deleted.
    deletemoonsPhotoIfOrphanmoonNsPhotoId);
  expect(testDb.prepare('SELECT id FROM moons_photos WHERE id = ?').getmoonNsPhotoId)).toBeDefined();

// Remove the reference, then orphan-cleanup should delete the moons_photos row.
testDb.prepare('DELETE FROM trip_photos WHERE photo_id = ?').run(moonsPhotoId);
    deletemoonsPhotoIfOrphanmoonNsPhotoId);
expect(testDb.prepare('SELECT id FROM moons_photos WHERE id = ?').getmoonNsPhotoId)).toBeUndefined();
  });
...
it('SYNO-092 — re-adding a previously removed Synology photo stores the new passphrase correctly', () => {
  ...
  const id1 = getOrCreatemoonsPhoto('synologyphotos', 'asset-readd-test', user.id, firstPass);
  testDb.prepare(
    'INSERT OR IGNORE INTO trip_photos (trip_id, user_id, photo_id, shared) VALUES (?, ?, ?, 1)'
  ).run(trip.id, user.id, id1);
  testDb.prepare('DELETE FROM trip_photos WHERE photo_id = ?').run(id1);
  deletemoonsPhotoIfOrphan(id1);

  // moons_photos row should be gone.
  expect(testDb.prepare('SELECT id FROM moons_photos WHERE id = ?').get(id1)).toBeUndefined();

  // Re-add with the correct passphrase.
  const id2 = getOrCreatemoonsPhoto('synologyphotos', 'asset-readd-test', user.id, secondPass);
  const row = testDb.prepare('SELECT passphrase FROM moons_photos WHERE id = ?').get(id2) as { passphrase: string };
...
describe('Synology skip-SSL forwarding to image fetches (#1611)', () => {
  ...
  function createSynologymoonsPhoto(skipSsl: 0 | 1) {
    const { user } = createUser(testDb);
    setSynologyCredentials(testDb, user.id, 'https://synology.example.com', 'admin', 'pass');
    ...
    return { user, moonsPhotoId: Number(insert.lastInsertRowid) };
  }
  ...
  it('SYNO-100 — thumbnail fetch passes rejectUnauthorized: false when skip-SSL is enabled', async () => {
    const { user, moonsPhotoId } = createSynologmoonNsPhoto(1);
    vi.mocked(safeFetch).mockClear();
  ...
  it('SYNO-101 — original fetch passes rejectUnauthorized: false when skip-SSL is enabled', async () => {
    ...
    const res = await request(app)
      .get(`/api/photos/${moonsPhotoId}/original`)
      .set('Cookie', authCookie(user.id));
  ...
  it('SYNO-102 — image fetches verify TLS when skip-SSL is disabled', async () => {
    const { user, moonsPhotoId } = createSynologmoonNsPhoto(0);
    vi.mocked(safeFetch).mockClear();
