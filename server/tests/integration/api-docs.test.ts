...
describe('API-DOCS (#1412) — flag-gated OpenAPI surface', () => {
  ...
  beforeAll(async () => {
    ...
    resetTestDb(testDb);
    prevFlag = process.env.moons_API_DOCS_ENABLED;
    process.env.moons_API_DOCS_ENABLED = 'true';
    app = await buildApp();
    instance = app.getHttpAdapter().getInstance();
  });

  afterAll(async () => {
    if (prevFlag === undefined) delete process.env.moons_API_DOCS_ENABLED;
    else process.env.moons_API_DOCS_ENABLED = prevFlag;
    await app.close();
    testDb.close();
  });

  it('DOCS-001 — the kill switch parses the env strictly', () => {
    const prev = process.env.moons_API_DOCS_ENABLED;
    try {
      process.env.moons_API_DOCS_ENABLED = 'true';
      expect(apiDocsEnabled()).toBe(true);
      process.env.moons_API_DOCS_ENABLED = ' TRUE ';
      expect(apiDocsEnabled()).toBe(true);
      process.env.moons_API_DOCS_ENABLED = 'false';
      expect(apiDocsEnabled()).toBe(false);
      process.env.moons_API_DOCS_ENABLED = '1';
      expect(apiDocsEnabled()).toBe(false);
      delete process.env.moons_API_DOCS_ENABLED;
      expect(apiDocsEnabled()).toBe(false);
    } finally {
      if (prev === undefined) delete process.env.moons_API_DOCS_ENABLED;
      else process.env.moons_API_DOCS_ENABLED = prev;
    }
  });
