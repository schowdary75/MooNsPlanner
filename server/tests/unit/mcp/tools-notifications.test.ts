 ...
 * Also covers the resource moons://notifications/in-app.
 */
...
// ---------------------------------------------------------------------------
// Resource: moons://notifications/in-app
// ---------------------------------------------------------------------------

describe('Resource: moons://notifications/in-app', () => {
  it('returns notifications list', async () => {
    const { user } = createUser(testDb);
    createNotification(testDb, user.id, { title_key: 'notif.test' });
    await withResourceHarness(user.id, async (h) => {
      const result = await h.client.readResource({ uri: 'moons://notifications/in-app' });
