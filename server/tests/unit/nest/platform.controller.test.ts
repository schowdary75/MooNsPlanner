...
const h = vi.hoisted(() => ({
  ...
  getMcpSafeUrl: vi.fn(() => 'https://moons.example.test'),
  dbPrepare: vi.fn(),
  existsSync: vi.fn(),
...
vi.mock('../../../src/mcp/oauthProvider', () => ({ moonsOAuthProvider: {}, moonNsClientsStore: {} }));
...
beforeEach(() => {
  vi.clearAllMocks();
  h.getMcpSafeUrl.mockReturnValue('https://moons.example.test');
});
...
describe('applyPlatformTransport', () => {
  ...
  it('GET /.well-known/openid-configuration returns AS metadata + userinfo_endpoint', () => {
    ...
    expect(body.issuer).toBe('https://moons.example.test');
    expect(body.userinfo_endpoint).toBe('https://moons.example.test/oauth/userinfo');
  });
  ...
  it('trims trailing slashes off the configured base URL', () => {
    ...
    const res = makeRes();
    handler({}, res);
    expect((res.body as { issuer: string }).issuer).toBe('https://moons.example.test');
  });
  ...
  describe('GET /.well-known/oauth-protected-resource (flat)', () => {
    ...
    it('returns the PRM document when MCP is enabled', () => {
      ...
      expect(body.resource).toBe('https://moons.example.test/mcp');
      expect(body.authorization_servers).toEqual(['https://moons.example.test']);
    });
  });
