...
vi.mock('../../../src/services/notifications', () => ({ getAppUrl: () => 'https://moons.example' }));
...
describe('PluginOAuthService', () => {
  ...
  it('startConnect builds a PKCE authorize URL + persists a single fresh state per user', () => {
    ...
    expect(url.searchParams.get('redirect_uri')).toBe('https://moons.example/api/plugin-oauth/p/callback');
