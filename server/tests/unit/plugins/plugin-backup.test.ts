...
let root: string;
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'moons-pbackup-'));
  process.env.moons_PLUGINS_DIR = path.join(root, 'plugins');
  process.env.moons_PLUGINS_DATA_DIR = path.join(root, 'plugins-data');
});
afterEach(() => {
