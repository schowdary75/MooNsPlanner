...
function writePlugin(id: string, manifest: Record<string, unknown>, extra?: () => void) {
  ...
  fs.writeFileSync(path.join(codeRoot, id, 'moons-plugin.json'), JSON.stringify({ id, name: id, version: '1.0.0', type: 'integration', ...manifest }));
...
beforeEach(() => {
  ...
  db.exec(`
    CREATE TABLE plugins (id TEXT PRIMARY KEY, name TEXT, description TEXT, type TEXT, icon TEXT, version TEXT,
      api_version INTEGER, min_moons_version TEXT,moonNs_range TEXT, permissions TEXT, capabilities TEXT DEFAULT '{}', dependencies TEXT DEFAULT '{}', operator_egress INTEGER DEFAULT 0, granted_permissions TEXT, status TEXT, config TEXT, updated_at TEXT);
...
describe('discoverPlugins', () => {
  ...
  it('tolerates a UTF-8 BOM in moons-plugin.json (Windows-authored plugins)', () => {
    writePlugin('bom-plug', { type: 'integration' });
    const mp = path.join(codeRoot, 'bom-plug', 'moons-plugin.json');
  ...
  it('follows a symlinked dev-link plugin only when dev-link mode is on', () => {
    ...
    const prev = process.env.moons_PLUGINS_DEV_LINK;
    try {
      const src = path.join(srcRoot, 'linked');
      fs.mkdirSync(path.join(src, 'server'), { recursive: true });
      fs.writeFileSync(path.join(src, 'moons-plugin.json'), JSON.stringify({ id: 'linked', name: 'Linked', version: '1.0.0', type: 'integration', permissions: ['db:own'] }));
      ...
      delete process.env.moons_PLUGINS_DEV_LINK;
      ...
      process.env.moons_PLUGINS_DEV_LINK = '1';
      ...
      if (prev === undefined) delete process.env.moons_PLUGINS_DEV_LINK; else process.envmoonNs_PLUGINS_DEV_LINK = prev;
      fs.rmSync(srcRoot, { recursive: true, force: true });
    }
  });

  it('is a no-op when the plugins dir is absent', () => {
    process.env.moons_PLUGINS_DIR = path.join(codeRoot, 'does-not-exist');
    expect(discoverPlugins(db)).toEqual({ discovered: [], skipped: [] });
  });

  describe('the moons range', () => {
    it('persists the range and its lower bound', () => {
      writePlugin('ranged', { moons: '>=3.2.0 <4.0.0' });
      discoverPlugins(db);
      expect(db.prepare("SELECT moons_range, minmoonNs_version FROM plugins WHERE id='ranged'").get())
        .toMatchObject({ moons_range: '>=3.2.0 <4.0.0', minmoonNs_version: '3.2.0' });
    });
    ...
    it('still registers a plugin that declares NO range — it must not vanish', () => {
      ...
      // registered with a null range and the activation gate refuses it (moons_VERSION_UNKNOWN).
      writePlugin('rangeless', {});
      expect(discoverPlugins(db).discovered).toEqual(['rangeless']);
      expect(db.prepare("SELECT moons_range FROM plugins WHERE id='rangeless'").get()).toMatchObject({ moonNs_range: null });
    });

    it('refreshes the range on re-discovery, so a plugin that narrowed its support is caught', () => {
      writePlugin('shrink', { moons: '>=3.0.0' });
      discoverPlugins(db);
      writePlugin('shrink', { moons: '>=3.0.0 <3.1.0' });
      discoverPlugins(db);
      expect(db.prepare("SELECT moons_range FROM plugins WHERE id='shrink'").get()).toMatchObject({ moonNs_range: '>=3.0.0 <3.1.0' });
    });
  });
});
