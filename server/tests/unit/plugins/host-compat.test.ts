...
/**
 * The moons-version gate (#plugins). Every install front door and the activation gate
...
describe('minmoonsOf', () => {
  it('reads the lower bound off the RANGE, not off the first version-shaped substring', () => {
    ...
    expect(minmoonsOf('<4.0.0')).toBeUndefined(); // lower bound is 0.0.0 → nothing to show
    expect(minmoonsOf('>=3.2.0 <4.0.0')).toBe('3.2.0');
    expect(minmoonsOf('^3.2.0')).toBe('3.2.0');
    expect(minmoonsOf('~3.2')).toBe('3.2.0');
    expect(minmoonsOf('>=3')).toBe('3.0.0');
    expect(minmoonsOf('>=3.2.0-beta.1')).toBe('3.2.0-beta.1');
  });

  it('has no lower bound to show for an unbounded range', () => {
    expect(minmoonsOf('*')).toBeUndefined();
  });
});
...
describe('hostSatisfies', () => {
  ...
  it('treats a prerelease host as its target release — both edges matter', () => {
    ...
    // would make EVERY plugin incompatible the moment moons shipped an rc.
    ...
    // "<4.0.0" — loading a plugin that disclaims moons 4 onto a build that already carries
    // moons 4's breaking changes. An rc of 4.0 IS 4.0 for compatibility purposes.
