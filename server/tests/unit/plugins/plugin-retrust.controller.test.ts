 ...
 *
 * The code is tested through the real moonsExceptionFilter rather than on the thrown
...
function throughFilter(e: unknown): { status: number; body: unknown } {
  ...
  new moonsExceptionFilter().catch(e, { switchToHttp: () => ({ getResponse: () => res }) } as unknown as ArgumentsHost);
  return { status, body };
}
