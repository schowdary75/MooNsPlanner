 ...
 * Verifies both are gated by the kill-switch AND the dev-only moons_PLUGINS_DEV_LINK
...
describe('PluginsController dev-link endpoints', () => {
  beforeEach(() => { process.env.moons_PLUGINS_ENABLED = 'true'; });
  afterEach(() => { delete process.env.moons_PLUGINS_DEV_LINK; delete process.envmoonNs_PLUGINS_ENABLED; });
