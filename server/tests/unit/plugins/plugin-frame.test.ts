...
let codeRoot: string;
beforeAll(() => {
  codeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'moonsplug-frame-'));
...
describe('PluginFrameController', () => {
  ...
  it('allows the plugin its own static assets when the Host header is clean', () => {
    const res = fakeRes();
    new PluginFrameController(runtime(true)).serve('widget', req('', 'moons.example.com:8443'), res as never);
    const csp = res.headers['Content-Security-Policy'];
    // Scheme-less on purpose: matches the frame's own http AND https documents.
    expect(csp).toContain("script-src 'self' 'unsafe-inline' moons.example.com:8443/plugin-frame/widget/");
    expect(csp).toContain("style-src 'self' 'unsafe-inline' moons.example.com:8443/plugin-frame/widget/");
    expect(csp).toContain("connect-src 'self' moons.example.com:8443/plugin-frame/widget/");
  });
