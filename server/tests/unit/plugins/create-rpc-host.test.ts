...
vi.mock('../../../src/db/database', () => {
  ...
  d.prepare("INSERT INTO users (id, role, email) VALUES (77, 'user', 'demo@moons.app')").run(); // a demo-account member for the DEMO_MODE upload guard
...
const { testFilesDir } = vi.hoisted(() => {
  ...
  return { testFilesDir: pm.join(osm.tmpdir(), 'moons-crh-files-test') };
});
...
let tmp: string;
beforeAll(() => {
  tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'moonsplug-crh-'));
  process.env.moons_PLUGINS_DATA_DIR = tmp;
});
afterAll(() => {
...
describe('create-rpc-host wiring', () => {
  ...
  it('db.tx dispatches an atomic batch through to the plugin db under db:own', async () => {
    ...
    const res = await host.dispatch({
      k: 'req', id: '2', method: 'db.tx', params: {
        ops: [
...
describe('create-rpc-host — Wave 3 wiring (files write / collab / member-add)', () => {
  ...
  it('files.create blocks a demo user while DEMO_MODE is on, but not other members', async () => {
    ...
    try {
      const h = host('db:write:files')
      // user 77 is the demo account (demo@moons.app) → the plugin upload is refused
