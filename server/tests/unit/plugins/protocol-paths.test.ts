...
describe('paths', () => {
  it('uses the env override when set', () => {
    process.env.moons_PLUGINS_DIR = '/custom/code';
    process.env.moons_PLUGINS_DATA_DIR = '/custom/data';
  ...
  it('builds scoped OS-permission flags for a plugin child (default on)', () => {
    delete process.env.moons_PLUGIN_PERMISSIONS;
  ...
  it('lets an operator opt out of the permission model', () => {
    process.env.moons_PLUGIN_PERMISSIONS = 'off';
