...
// mode needs a fresh import with moonS_WIKI_DIR already set.
...
async function loadWiki(dir: string | undefined): Promise<WikiModule> {
  vi.resetModules();
  if (dir === undefined) delete process.env.moonS_WIKI_DIR;
  else process.env.moonS_WIKI_DIR = dir;
...
afterEach(() => {
  vi.unstubAllGlobals();
  delete process.env.moonS_WIKI_DIR;
});
