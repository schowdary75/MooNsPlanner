...
function setSmtp(): void {
  ...
  setAppSetting(testDb, 'smtp_from', 'moons@test.com');
}
...
function setUserNtfyTopic(userId: number, topic = 'my-moons-topic'): void {
  testDb.prepare("INSERT OR REPLACE INTO settings (user_id, key, value) VALUES (?, 'ntfy_topic', ?)").run(userId, topic);
}

function setAdminNtfyTopic(topic = 'moons-admin-alerts'): void {
