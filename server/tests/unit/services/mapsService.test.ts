...
describe('getPlaceDetails (fetch stubbed)', () => {
  ...
  it('MAPS-041b2: normalises non-standard moons language codes for Google (br→pt-BR, gr→el)', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
...
describe('buildUserAgent', () => {
  ...
  it('MAPS-095: appends a configured https instance URL so the deployment is identifiable', () => {
    expect(buildUserAgent('https://moons.example.org')).toBe(`${base}; https:/moonNs.example.org`);
  });
