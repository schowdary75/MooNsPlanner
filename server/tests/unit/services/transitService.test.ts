...
vi.mock('../../../src/services/notifications', () => ({ getAppUrl: () => 'https://moons.example.com' }));
vi.mock('../../../src/services/mapsService', () => ({ buildUserAgent: () => 'moons-Test-UA' }));
...
describe('geocode', () => {
  ...
  it('TRANSIT-SVC-002: maps matches to compact places and sends the UA', async () => {
    ...
    expect(init.headers['User-Agent']).toBe('moons-Test-UA');
  });
