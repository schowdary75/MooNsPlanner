...
/**
 * An AirTrail flight as GET returns it, including the fields moons doesn't model.
...
function existingFlight(
  ...
  return {
    ...
    // AirTrail-owned detail moons never surfaces — must survive a writeback (#1240).
...
/** A linked moons reservation (the shape getReservationWithJoins returns). */
...
describe('airtrailSync.buildSavePayload', () => {
  it('round-trips the AirTrail-owned fields moons does not model (issue #1240)', () => {
  ...
  it('writes the moons time to the SCHEDULED fields so it round-trips on the next pull', () => {
    // Import reads the scheduled time, so a moons edit must be pushed back there
  ...
  it('blanks the scheduled time when the moons reservation has only a date', () => {
  ...
  it('still applies the moons-owned edits on top of the preserved fields', () => {
    const payload = buildSavePayload(
      reservation({
        reservation_time: '2021-09-01T20:30',
        notes: 'changed in moons',
  ...
  it('preserves AirTrail aircraft/airline/reason when moons metadata omits them (#1240)', () => {
    // A moons edit can drop these AirTrail-owned fields from metadata; the writeback
