...
const { photoSvc, helperSvc } = vi.hoisted(() => ({
  photoSvc: { streamPhoto: vi.fn(), getPhotoInfo: vi.fn(), resolvemoonsPhoto: vi.fn() },
  helperSvc: { canAccessmoonsPhoto: vi.fn() },
}));
...
describe('Files + photos e2e (real auth guard + temp SQLite)', () => {
  ...
  beforeEach(() => {
    ...
    helperSvc.canAccessmoonsPhoto.mockReturnValue(true);
  });
  ...
  it('403 on a photo the user cannot access', async () => {
    helperSvc.canAccessmoonsPhoto.mockReturnValue(false);
