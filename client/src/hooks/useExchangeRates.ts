...
function readCache(base: string): { rates: Record<string, number>; ts: number } | null {
  ...
  if (m) return m
  try {
    const raw = localStorage.getItem('moon_fx_' + base)
    if (raw) {
...
export async function fetchExchangeRates(base: string): Promise<Record<string, number> | null> {
  ...
  try {
    ...
    mem.set(upper, entry)
    try { localStorage.setItem('moon_fx_' + upper, JSON.stringify(entry)) } catch { /* ignore */ }
    return rates
  } catch {
...
export function clearExchangeRateCache(): void {
  ...
  try {
    for (const k of Object.keys(localStorage)) {
      if (k.startsWith('moon_fx_')) localStorage.removeItem(k)
    }
  } catch { /* ignore */ }
}
