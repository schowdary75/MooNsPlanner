 ...
 */
export function useRouteCalculation(tripStore: TripStoreState, selectedDayId: number | 'all' | null, enabled: boolean = true, profile: 'driving' | 'walking' | 'cycling' = 'driving', accommodations: Accommodation[] = NO_ACCOMMODATIONS) {
  ...
  const updateRouteForDay = useCallback(async (dayId: number | 'all' | null) => {
    if (routeAbortRef.current) routeAbortRef.current.abort()
    // Route is manual: only compute when explicitly enabled (the "show route" toggle).
    if (!dayId || (!enabled && dayId !== 'all')) { setRoute(null); setRouteSegments([]); return }
    ...
    for (const currentDayId of dayIdsToProcess) {
      ...
      const dayTransports = thisOrder == null ? [] : allReservations.filter(r => {
        ...
        if (startId === endId) {
          if (startId !== currentDayId) return false
        } else {
        ...
        const pos = r.day_positions?.[currentDayId] ?? r.day_positions?.[String(currentDayId)] ?? r.day_plan_position
        return pos != null
      })
      ...
      const entries: Entry[] = [
        ...
        ...dayTransports.map(r => {
          const { from, to } = getTransportRouteEndpoints(r, currentDayId)
          return {
            kind: 'transport' as const,
            from,
            to,
            pos: (r.day_positions?.[currentDayId] ?? r.day_positions?.[String(currentDayId)] ?? r.day_plan_position) as number,
          }
        }),
    ...
    if (allRunsWithHotel.length === 0) { setRoute(null); setRouteSegments([]); return }
    ...
    try {
      ...
      for (const run of allRunsWithHotel) {
        try {
  ...
  // Stable signature for transport reservations on the selected day(s) — changes when a transport
  ...
  const transportSignature = useMemo(() => {
    ...
    return reservationsForSignature
      ...
      .map(r => {
        // If 'all', we don't have a single dayId, so we just include its positions
        const posStr = JSON.stringify(r.day_positions || {}) + (r.day_plan_position || '')
        const eps = (r.endpoints || []).map(e => `${e.role}@${e.lat ?? ''},${e.lng ?? ''}`).join(';')
        return `${r.id}:${r.day_id ?? ''}:${r.end_day_id ?? ''}:${r.reservation_time ?? ''}:${posStr}:${eps}`
      })
      .sort()
      .join('|')
  ...
  const selectedDayAssignments = selectedDayId && selectedDayId !== 'all' ? tripStore.assignments?.[String(selectedDayId)] : null
  const allAssignments = selectedDayId === 'all' ? tripStore.assignments : null
  useEffect(() => {
    if (!selectedDayId) { setRoute(null); setRouteSegments([]); return }
    updateRouteForDay(selectedDayId)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedDayId, selectedDayAssignments, allAssignments, transportSignature, enabled, profile, accommodations, optimizeFromAccommodation, distanceUnit])
