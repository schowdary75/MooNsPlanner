...
function buildLocationEl(): { root: HTMLDivElement; cone: HTMLDivElement } {
  ...
  pulse.style.cssText = `
    ...
    animation:moon-location-pulse 2s ease-out infinite;
  `
...
function ensurePulseStyle() {
  if (document.getElementById('moon-location-style')) return
  const s = document.createElement('style')
  s.id = 'moon-location-style'
  s.textContent = `
    @keyframes moon-location-pulse {
...
export function attachLocationMarker(map: mapboxgl.Map, MarkerCtor: MarkerConstructor): LocationMarkerHandle {
  ...
  const ensureAccuracyLayer = () => {
    if (map.getSource('moon-location-accuracy')) return
    try {
      map.addSource('moon-location-accuracy', {
        type: 'geojson',
      ...
      map.addLayer({
        id: 'moon-location-accuracy',
        type: 'fill',
        source: 'moon-location-accuracy',
        paint: {
          'fill-color': '#3b82f6',
          'fill-opacity': 0.14,
  ...
  const setAccuracy = (p: GeoPosition) => {
    const src = map.getSource('moon-location-accuracy') as mapboxgl.GeoJSONSource | undefined
    if (!src) return
  ...
  const handle: LocationMarkerHandle = {
    ...
    destroy: () => {
      ...
      try {
        if (map.getLayer('moon-location-accuracy')) map.removeLayer('moon-location-accuracy')
        if (map.getSource('moon-location-accuracy')) map.removeSource('moon-location-accuracy')
      } catch { /* noop */ }
    },
  }
