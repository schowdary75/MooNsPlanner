...
function ensureJourneyPopupStyle() {
  if (document.getElementById('moon-journey-popup-style')) return
  const s = document.createElement('style')
  s.id = 'moon-journey-popup-style'
  s.textContent = `
    .mapboxgl-popup.moon-journey-popup,
    .maplibregl-popup.moon-journey-popup { pointer-events: none; animation: moon-journey-popup-in 180ms ease-out; }
    .mapboxgl-popup.moon-journey-popup .mapboxgl-popup-content,
    .maplibregl-popup.moon-journey-popup .maplibregl-popup-content {
    ...
    .mapboxgl-popup.moon-journey-popup.moon-dark .mapboxgl-popup-content,
    .maplibregl-popup.moon-journey-popup.moon-dark .maplibregl-popup-content {
      background: rgba(24, 24, 27, 0.88);
      border-color: rgba(255, 255, 255, 0.08);
      color: #FAFAFA;
    }
    .mapboxgl-popup.moon-journey-popup .mapboxgl-popup-tip,
    .maplibregl-popup.moon-journey-popup .maplibregl-popup-tip {
      border-top-color: rgba(255, 255, 255, 0.94);
      border-bottom-color: rgba(255, 255, 255, 0.94);
    }
    .mapboxgl-popup.moon-journey-popup.moon-dark .mapboxgl-popup-tip,
    .maplibregl-popup.moon-journey-popup.moon-dark .maplibregl-popup-tip {
      border-top-color: rgba(24, 24, 27, 0.88);
      border-bottom-color: rgba(24, 24, 27, 0.88);
    }
    .mapboxgl-popup.moon-journey-popup .mapboxgl-popup-close-button,
    .maplibregl-popup.moon-journey-popup .maplibregl-popup-close-button { display: none; }
    .moon-journey-popup-title {
      font-size: 13.5px;
      font-weight: 600;
    ...
    .mapboxgl-popup.moon-journey-popup.moon-dark .moon-journey-popup-title,
    .maplibregl-popup.moon-journey-popup.moon-dark .moon-journey-popup-title { color: #FAFAFA; }
    .moon-journey-popup-sub {
      display: flex;
    ...
    .mapboxgl-popup.moon-journey-popup.moon-dark .moon-journey-popup-sub,
    .maplibregl-popup.moon-journey-popup.moon-dark .moon-journey-popup-sub { color: #A1A1AA; }
    .moon-journey-popup-place {
      min-width: 0;
      overflow: hidden;
      text-overflow: ellipsis;
    }
    .moon-journey-popup-sep {
      flex: 0 0 auto;
      opacity: 0.55;
      font-weight: 500;
    }
    .moon-journey-popup-date { flex: 0 0 auto; }
    @keyframes moon-journey-popup-in {
      from { opacity: 0; }
      to { opacity: 1; }
    }
  `
...
function markerHtml(dayColor: string, dayLabel: number, highlighted: boolean): HTMLDivElement {
  ...
  inner.className = 'moon-journey-marker-inner'
...
const JourneyMapGL = forwardRef<JourneyMapGLHandle, Props>(function JourneyMapGL(
  ...
  const showPopup = useCallback((id: string) => {
    ...
    if (place) subParts.push(`<span class="moon-journey-popup-place">${place}</span>`)
    if (date) subParts.push(`<span class="moon-journey-popup-date">${date}</span>`)
    const subline = subParts.length === 2
      ? `${subParts[0]}<span class="moon-journey-popup-sep">\u00B7</span>${subParts[1]}`
      : subParts.join('')

    const html = `
      <div class="moon-journey-popup-title">${primary}</div>
      ${subline ? `<div class="moon-journey-popup-sub">${subline}</div>` : ''}
    `
    ...
    if (popupRef.current) {
      ...
      if (el) el.classList.toggle('moon-dark', !!darkRef.current)
    } else {
      popupRef.current = new gl.Popup({
        ...
        anchor: 'bottom',
        offset,
        className: `moon-journey-popup${darkRef.current ? ' moon-dark' : ''}`,
        maxWidth: '280px',
      })
  ...
  const setMarkerStyle = useCallback((id: string, highlighted: boolean) => {
    ...
    const currentInner = el.querySelector('.moon-journey-marker-inner') as HTMLDivElement | null
    ...
    const nextInner = next.querySelector('.moon-journey-marker-inner') as HTMLDivElement
