...
const MFA_BACKUP_SESSION_KEY = 'moon_mfa_backup_codes_pending'
...
export default function AccountTab(): React.ReactElement {
  ...
  const downloadBackupCodes = () => {
    ...
    a.href = url
    a.download = 'moon-mfa-backup-codes.txt'
  ...
  const printBackupCodes = () => {
    if (!backupCodesText) return
    const html = `<!doctype html><html><head><meta charset="utf-8"/><title>moon MFA Backup Codes</title>
      <style>body{font-family:Arial,sans-serif;padding:32px}h1{font-size:20px}pre{font-size:16px;line-height:1.6}</style>
      </head><body><h1>moon MFA Backup Codes</h1><p>${new Date().toLocaleString()}</p><pre>${backupCodesText}</pre></body></html>`
