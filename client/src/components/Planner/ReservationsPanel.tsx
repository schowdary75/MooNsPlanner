...
export default function ReservationsPanel({ tripId, reservations, days, assignments, files = [], onAdd, onImport, bookingImportAvailable, onAirTrailImport, airTrailAvailable, onEdit, onDelete, onNavigateToFiles, titleKey = 'reservations.title', addManualKey = 'reservations.addManual', contributionView = 'reservations' }: ReservationsPanelProps) {
  ...
  const storageKey = `moon-reservation-filters-${tripId}`
        ...
        {total === 0 && reservations.length === 0 ? (
          ...
          <>
            {transitEntries.length > 0 && (
              <Section title={t('transit.sectionTitle')} count={transitEntries.length} accent="gray" storageKey={`moon:bookings-transit-open:${tripId}`}>
            ...
            {allPending.length > 0 && (
              <Section title={t('reservations.pending')} count={allPending.length} accent="gray" storageKey={`moon:bookings-pending-open:${tripId}`}>
            ...
            {allConfirmed.length > 0 && (
              <Section title={t('reservations.confirmed')} count={allConfirmed.length} accent="green" storageKey={`moon:bookings-confirmed-open:${tripId}`}>
