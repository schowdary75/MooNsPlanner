...
function plugin(over: Record<string, unknown> = {}) {
  return {
    id: 'moon-gotify', name: 'Gotify', description: 'Push notifications', type: 'integration',
...
describe('AdminPluginsPanel — allowed-hosts chip', () => {
  ...
  it('FE-COMP-PLUGINS-EGRESS-004: clicking the chip opens the allowed-hosts dialog', async () => {
    ...
    server.use(
      http.get('*/api/admin/plugins/moon-gotify/egress-hosts', () =>
...
function mockDetail(manifest: Record<string, unknown> | null) {
  server.use(
    ...
    http.get('*/api/admin/plugins/registry', () =>
      HttpResponse.json([{ id: 'moon-gotify', name: 'Gotify', author: 'MooN', description: 'Push', repo: 'MooN/moon-gotify', type: 'integration', tags: [] }])),
    http.get('*/api/admin/plugins/registry/moon-gotify', () =>
      HttpResponse.json({
        id: 'moon-gotify', name: 'Gotify', author: 'MooN', description: 'Push', repo: 'MooN/moon-gotify',
...
describe('AdminPluginsPanel — row ⋯ menu is never clipped (#1523)', () => {
  const withRepo = plugin({ source_repo: 'moon/gotify', operatorEgress: false })
  ...
  async function openRowMenu() {
    ...
    fireEvent.click(await screen.findByTestId('plugin-row-menu-btn-moon-gotify'))
    return { container, menu: screen.getByTestId('plugin-row-menu-moon-gotify') }
  }
...
/**
 * Signature status (#plugins). moon has always verified author signatures and TOFU-pinned
...
function registryEntry(over: Record<string, unknown> = {}) {
  return {
    id: 'moon-gotify', name: 'Gotify', author: 'Acme', description: 'Push', repo: 'acme/gotify',
    type: 'integration', latest: '2.0.0', minmoonVersion: null, reviewedAt: null,
...
describe('AdminPluginsPanel — a refused update', () => {
  ...
  it('FE-COMP-PLUGINS-SIG-010: confirming a re-trust re-pins AND updates in ONE call', async () => {
    ...
    mockPanel(blocked('SIGNATURE_KEY_CHANGED'))
    server.use(
      http.post('*/api/admin/plugins/moon-gotify/retrust', async ({ request }) => {
...
describe('AdminPluginsPanel — update consent', () => {
  it('FE-COMP-PLUGINS-SIG-011: says an unsigned update is untied to its author, and still activates in one click', async () => {
    ...
    server.use(
      http.post('*/api/admin/plugins/moon-gotify/update', () =>
        HttpResponse.json({ version: '2.0.0', activated: false, newPermissions: ['db:read:trips'], newEgress: [] }),
      ),
      http.post('*/api/admin/plugins/moon-gotify/activate', () => { activated = true; return HttpResponse.json({ status: 'active' }) }),
    )
    render(<AdminPluginsPanel />)
    await screen.findByText('Gotify')
  ...
  it('FE-COMP-PLUGINS-SIG-015: the unsigned warning survives an unreachable registry', async () => {
    server.use(
      ...
      http.post('*/api/admin/plugins/moon-gotify/activate', () =>
...
describe('AdminPluginsPanel — a refusal with no installed row', () => {
  ...
  it('FE-COMP-PLUGINS-SIG-014: a refusal while downloading a DEPENDENCY opens the dialog too', async () => {
    const parent = plugin({ id: 'moon-parent', name: 'Parent', source_repo: 'acme/parent', enabled: 0, status: 'inactive', operatorEgress: false })
    server.use(
      ...
      http.post('*/api/admin/plugins/moon-parent/activate', () =>
        HttpResponse.json({ error: 'missing dependency', code: 'DEPENDENCY_MISSING', missing: [{ id: 'moon-gotify', version: '^1.0.0' }], versionMismatch: [] }, { status: 409 })),
...
/**
 * moon-version compatibility. The SERVER owns the semver — a second implementation in the
 ...
 */
describe('AdminPluginsPanel — moon-version compatibility', () => {
  ...
  it('blocks Install when no published version runs on this moon, and says why', async () => {
    await openDiscover({ moon: '>=4.0.0', hostVersion: '3.3.0', compatible: false, latestCompatible: null })
  ...
  it('offers the newest version that DOES run here rather than a dead button', async () => {
    await openDiscover({ latest: '2.0.0', moon: '>=3.4.0', hostVersion: '3.3.0', compatible: false, latestCompatible: '1.5.0' })
    const btn = await screen.findByRole('button', { name: /^install 1\.5\.0$/i })
    expect(btn).toBeEnabled()
  })

  it('installs normally when the latest version fits', async () => {
    await openDiscover({ moon: '>=3.2.0 <4.0.0', hostVersion: '3.3.0', compatible: true, latestCompatible: '2.0.0' })
  ...
  it('an installed plugin the server has outgrown shows the blocker on its card', async () => {
    ...
    mockPanel(plugin({
      dependencyStatus: 'hostIncompatible', moonRange: '>=3.2.0 <4.0.0', hostVersion: '4.0.0', enabled: 0, status: 'inactive',
    }))
    render(<AdminPluginsPanel />)
    expect(await screen.findByText(/needs moon >=3\.2\.0 <4\.0\.0/i)).toBeInTheDocument()
  })
})
