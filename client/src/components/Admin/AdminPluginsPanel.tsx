...
interface PluginRow {
  ...
  /** The moon versions the plugin says it supports; null if it never declared any. */
  moonRange?: string | null
  /** The moon this server is running — the server does the semver, the client just shows it. */
  hostVersion?: string
...
interface RegistryItem {
  ...
  /** The latest version's declared moon range; null on a legacy registry entry. */
  moon?: string | null
  hostVersion?: string
  /** Whether the LATEST version can be installed on this moon. Computed server-side. */
  compatible?: boolean
...
// or the moon version itself (amber when this server has outgrown the plugin's range,
...
function deriveDeps(p: PluginRow, t: T): DepChip[] {
  ...
  if (p.dependencyStatus === 'hostIncompatible') {
    out.push({
      icon: AlertTriangle,
      label: p.moonRange
        ? t('admin.plugins.dep.moonIncompatible', { range: p.moonRange, host: p.hostVersion ?? '?' })
        : t('admin.plugins.dep.moonUnknown'),
      blocked: true,
    })
  }
 ...
 * has outrun this moon but an older one still fits, offer THAT version rather than a dead
...
function installOffer(item: RegistryItem, t: T): { blocked: boolean; version?: string; label: string; title?: string } {
  ...
  const title = item.moon
    ? t('admin.plugins.dep.moonIncompatible', { range: item.moon, host: item.hostVersion ?? '?' })
    : t('admin.plugins.dep.moonUnknown')
...
export default function AdminPluginsPanel() {
  ...
  const [devLink, setDevLink] = useState(false) // dev-link enabled server-side (moon_PLUGINS_DEV_LINK)
        ...
        <div className="px-4 sm:px-5 pt-4 pb-5">
          ...
          <div className="mt-5">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-3 mt-2.5">
              ...
              {/* The range, not just its lower bound: "moon 3.2.0+" reads as "and anything
                  newer", which is exactly the claim a `<4.0.0` upper bound denies. */}
              {(item.moon || item.minmoonVersion) && (
                <Meta k={t('admin.plugins.metaRequires')} v={item.moon ? `moon ${item.moon}` : `moon ${item.minmoonVersion}+`} />
              )}
