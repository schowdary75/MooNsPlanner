...
import type { CollectionStatus } from '@moons/shared'
 ...
 * with utility classes only, so it works both inside and outside `.moon-dash`.
 */
export default function StatusBadge({ status, onChange, showLabel = true, size = 13, onCover = false, t }: StatusBadgeProps): React.ReactElement {
  ...
  // the .moon-dash scope the global `.moon-dash button` reset would strip the
