...
// values, resolved for the CURRENT theme, so a plugin can match moon exactly (and
...
// accent, high-contrast) flows through here live. The glassy `.moon-dash` layer
...
/**
 * Renders a plugin's sandboxed page/widget iframe and hosts the moonBridge
 * (#plugins, M3).
 *
 * The frame is served same-origin from /plugin-frame/:id but sandboxed WITHOUT
 * allow-same-origin, so it runs at an OPAQUE origin: no access to the moon_session
...
interface PluginFrameProps {
   ...
   * (moon:resize) and the page collapses to a floating island with dead space
...
type Inbound =
  | { type: 'moon:ready' }
  | { type: 'moon:context:request' }
  | { type: 'moon:navigate'; to: string }
  | { type: 'moon:notify'; level?: 'info' | 'success' | 'warning' | 'error'; message?: string; duration?: number }
  | { type: 'moon:resize'; height?: number }
  | { type: 'moon:invoke'; requestId: string; sub: string; method?: string; body?: unknown }
  | { type: 'moon:confirm'; requestId: string; title?: string; message?: string; confirmLabel?: string; cancelLabel?: string; danger?: boolean }
  | { type: 'moon:openExternal'; url?: string }
...
export default function PluginFrame({ pluginId, tripId = null, placeId = null, dayId = null, reservationId = null, fill = false, className, title, path = 'index.html' }: PluginFrameProps) {
  ...
  // The exposure is bounded (only this plugin's own routes + the moon:context
  ...
  // same native confirm every moon feature uses. One at a time; extras are refused.
  ...
  // deliver the context too: the moon:ready handshake alone is racy — if the frame
  ...
  const buildContext = useCallback(() => ({
    ...
    // how moon formats things, the resolved theme tokens, and the appearance state
  ...
  useEffect(() => {
      ...
      switch (msg.type) {
        ...
        case 'moon:confirm': {
          ...
          if (confirmReqRef.current) {
            ...
            post({ type: 'moon:confirm:result', requestId: msg.requestId, confirmed: false })
            break
          }
        ...
        case 'moon:invoke': {
          ...
          try {
            ...
            post({ type: 'moon:response', requestId: msg.requestId, data })
          } catch (e) {
            const err = e as { response?: { status?: number }; message?: string }
            post({ type: 'moon:error', requestId: msg.requestId, code: err.response?.status ?? 'error', message: err.message ?? 'invoke failed' })
          }
          break
        }
      }
    ...
    if (tripId) {
      wsForward = (ev) => {
        ...
        post({ type: 'moon:event', event: ev.type, tripId })
      }
      addListener(wsForward)
    }

    // The frame is opaque-origin and can't read our DOM, and we otherwise send the
    // context (incl. theme + tokens) only once on moon:ready — so a plugin can't
  ...
  const answerConfirm = (confirmed: boolean) => {
    ...
    if (req) postFrame({ type: 'moon:confirm:result', requestId: req.requestId, confirmed })
    confirmReqRef.current = null
    setConfirmReq(null)
  }

  // The dialog title always leads with the host-controlled plugin name, so a
  // plugin cannot dress its confirm up as a moon system dialog.
  ...
  return (
    <>
      <iframe
        ...
        // message listener during parse), closing the moon:ready race so the theme is
