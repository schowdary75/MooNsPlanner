...
export default function Navbar({ tripTitle, tripId, onBack, showBack, onShare }: NavbarProps): React.ReactElement {
  ...
  const toggleDarkMode = () => {
    document.documentElement.classList.add('moon-theme-transitioning')
    ...
    themeTransitionTimer.current = window.setTimeout(() => {
      document.documentElement.classList.remove('moon-theme-transitioning')
      themeTransitionTimer.current = null
    }, 360)
  }
      ...
      <div className="flex items-center gap-3 min-w-0">
        {showBack && (
          <button onClick={onBack}
            className="moon-back-btn p-1.5 rounded-lg transition-colors flex items-center gap-1.5 text-sm flex-shrink-0 text-content-muted"
            onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-hover)'}
            onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            <ArrowLeft className="moon-back-icon w-4 h-4" />
        ...
        <div className="relative">
          ...
          {userMenuOpen && ReactDOM.createPortal(
            <>
              ...
              <div className="moon-menu-enter w-52 rounded-xl shadow-xl border overflow-hidden bg-surface-card border-edge" style={{ position: 'fixed', top: 'var(--nav-h)', right: 8, zIndex: 9999 }}>
                    ...
                    <div className="px-4 pt-2 pb-2.5 text-center border-t border-edge-secondary" style={{ marginTop: 4 }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--bg-tertiary)', borderRadius: 99, padding: '4px 12px' }}>
                          <img src={dark ? '/text-light.svg' : '/text-dark.svg'} alt="moon" style={{ height: 10, opacity: 0.5 }} />
