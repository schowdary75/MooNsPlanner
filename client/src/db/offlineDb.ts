 ...
 * the base name; a logged-in user uses `moon-offline-u<userId>`.
 */
const ANON_DB_NAME = 'moon-offline';

function userDbName(userId: number | string): string {
  return `moon-offline-u${userId}`;
}
...
function initialDbName(): string {
  try {
    const raw = typeof localStorage !== 'undefined' ? localStorage.getItem('moon_auth_snapshot') : null;
...
class moonOfflineDb extends Dexie {
...
let _db = new moonOfflineDb(initialDbName());

export const offlineDb = new Proxy({} as moonOfflineDb, {
  get(_target, prop) {
...
export async function deleteCurrentUserDb(): Promise<void> {
  ...
  _db = new moonOfflineDb(ANON_DB_NAME);
  await _db.open();
}
