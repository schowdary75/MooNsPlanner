...
import type { TripCreateRequest } from '@moons/shared'
import {
  type DashboardTrip,
  type TravelStats,
...
export function useDashboard() {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>(() => (localStorage.getItem('moon_dashboard_view') as 'grid' | 'list') || 'grid')
  ...
  const toggleViewMode = () => {
    setViewMode(prev => {
      ...
      localStorage.setItem('moon_dashboard_view', next)
      return next
    })
  }
