...
export default function DashboardPage(): React.ReactElement {
  ...
  return (
    <>
      {/* Navbar lives outside .moon-dash so it keeps the app-wide font + button
          styling instead of inheriting the dashboard scope's font and the
          `.moon-dash button` reset (which shifted the bell icon + menu items). */}
      <Navbar />
      <div className="moon-dash moon-dash-shell">
      {demoMode && <DemoBanner />}
      <div className="moon-dash-scroll">
        <MobileTopBar />
...
function CurrencyTool(): React.ReactElement {
  ...
  useEffect(() => {
    if (!isLoaded) return
    const lf = localStorage.getItem('moon_fx_from')
    const lt = localStorage.getItem('moon_fx_to')
    if (!lf && !lt) return
    ...
    Promise.all(writes).then(() => {
      localStorage.removeItem('moon_fx_from')
      localStorage.removeItem('moon_fx_to')
...
function TimezoneTool({ locale }: { locale: string }): React.ReactElement {
  ...
  useEffect(() => {
    if (!isLoaded) return
    const raw = localStorage.getItem('moon_dashboard_tz')
    if (!raw) return
    let parsed: unknown
    // A malformed/non-array value can never be written, so drop it now to avoid retrying forever.
    try { parsed = JSON.parse(raw) } catch { localStorage.removeItem('moon_dashboard_tz'); return }
    if (!Array.isArray(parsed)) { localStorage.removeItem('moon_dashboard_tz'); return }
    ...
    updateSetting('dashboard_timezones', parsed)
      .then(() => { localStorage.removeItem('moon_dashboard_tz') })
      .catch(() => { /* keep localStorage; retry on next load */ })
  }, [isLoaded, updateSetting])
