...
beforeEach(() => {
  _resetNetworkMode()
  try { localStorage.removeItem('moons_forced_offline') } catch { /* ignore */ }
...
describe('networkMode', () => {
  ...
  it('persists the forced flag to localStorage', () => {
    setForcedOffline(true)
    expect(localStorage.getItem('moons_forced_offline')).toBe('1')
    setForcedOffline(false)
    expect(localStorage.getItem('moons_forced_offline')).toBeNull()
  })
})
