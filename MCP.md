...
MooNs includes a built-in [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) server that lets AI
assistants — such as Claude Desktop, Cursor, or any MCP-compatible client — read and modify your trip data through a
structured API.

> **Note:** MCP is an addon that must be enabled by your MooNs administrator before it becomes available.
...
```json
{
  "mcpServers": {
    "MooNs": {
      ...
      "args": [
        "mcp-remote",
        "https://your-MooNs-instance.com/mcp"
      ]
    }
  }
}
...
4. Your browser opens MooNs's consent screen, where you choose which scopes (permissions) to grant.
...
> **Requirement:** The `APP_URL` environment variable must be set to your MooNs instance's public URL for OAuth
...
(`MooNscs_` prefix) and fixed scopes that you define up front.
  ...
  "mcpServers": {
    "MooNs": {
      ...
      "args": [
        ...
        "https://your-MooNs-instance.com/mcp",
        "--header",
        "Authorization: Bearer MooNs_your_token_here"
      ]
    }
  }
}
...
MooNs's MCP server supports three authentication methods. OAuth 2.1 is the recommended path for all external clients.

| Method | Token prefix | Access level | TTL | Notes |
|--------|-------------|-------------|-----|-------|
| **OAuth 2.1** | `MooNsoa_` | Scoped (per-consent) | 1 hour | Recommended. Automatically refreshed via 30-day rolling refresh tokens (`MooNsrf_` prefix). Replay-detected rotation — replayed tokens cascade-revoke the entire chain. |
| **Static API token** | `MooNs_` | Full access | No expiry | **Deprecated.** Triggers deprecation warnings in AI clients. Will be removed in a future release. |
| **Web session JWT** | — | Full access | Session-based | Used internally by the MooNs web UI. Not intended for external clients. |
...
When connecting via OAuth 2.1, you grant specific scopes during the consent step. MooNs registers only the MCP tools
...
| **Demo mode restrictions**              | If MooNs is running in demo mode, all write operations through MCP are blocked.                                                                   |
...
Resources provide read-only access to your MooNs data. MCP clients can read these to understand the current state before
making changes.
...
|-----------------------|-------------------------------------------------|---------------------------------------------------------------------------------------|
| Trips                 | `MooNs://trips`                                  | All trips you own or are a member of                                                  |
| Trip Detail           | `MooNs://trips/{tripId}`                         | Single trip with metadata and member count                                            |
| Days                  | `MooNs://trips/{tripId}/days`                    | Days of a trip with their assigned places                                             |
| Places                | `MooNs://trips/{tripId}/places`                  | All places/POIs saved in a trip. Supports `?assignment=all\|unassigned\|assigned`     |
| Budget                | `MooNs://trips/{tripId}/budget`                  | Budget and expense items                                                              |
| Budget Per-Person     | `MooNs://trips/{tripId}/budget/per-person`       | Per-person totals and split breakdown                                                 |
| Budget Settlement     | `MooNs://trips/{tripId}/budget/settlement`       | Suggested transactions to settle who owes whom                                        |
| Packing               | `MooNs://trips/{tripId}/packing`                 | Packing checklist                                                                     |
| Packing Bags          | `MooNs://trips/{tripId}/packing/bags`            | Packing bags with their assigned members                                              |
| Reservations          | `MooNs://trips/{tripId}/reservations`            | Flights, hotels, restaurants, etc.                                                    |
| Day Notes             | `MooNs://trips/{tripId}/days/{dayId}/notes`      | Notes for a specific day                                                              |
| Accommodations        | `MooNs://trips/{tripId}/accommodations`          | Hotels/rentals with check-in/out details                                              |
| Members               | `MooNs://trips/{tripId}/members`                 | Owner and collaborators                                                               |
| Collab Notes          | `MooNs://trips/{tripId}/collab-notes`            | Shared collaborative notes                                                            |
| To-Dos                | `MooNs://trips/{tripId}/todos`                   | To-do items ordered by position                                                       |
| Categories            | `MooNs://categories`                             | Available place categories (for use when creating places)                             |
| Bucket List           | `MooNs://bucket-list`                            | Your personal travel bucket list                                                      |
| Visited Countries     | `MooNs://visited-countries`                      | Countries marked as visited in Atlas                                                  |
| Notifications         | `MooNs://notifications/in-app`                   | Your in-app notifications (most recent 50, unread first)                              |
...
|-----------------------|-------------------------------------------------|----------|---------------------------------------------------------------------|
| Atlas Stats           | `MooNs://atlas/stats`                            | Atlas    | Visited country counts and continent breakdown                      |
| Atlas Regions         | `MooNs://atlas/regions`                          | Atlas    | Manually visited sub-country regions                                |
| Collab Polls          | `MooNs://trips/{tripId}/collab/polls`            | Collab   | All polls for a trip with vote counts per option                    |
| Collab Messages       | `MooNs://trips/{tripId}/collab/messages`         | Collab   | Most recent 100 chat messages for a trip                            |
| Vacay Plan            | `MooNs://vacay/plan`                             | Vacay    | Full snapshot of your active vacation plan (members, years, config) |
| Vacay Entries         | `MooNs://vacay/entries/{year}`                   | Vacay    | All vacation day entries for the active plan and a specific year    |
| Vacay Holidays        | `MooNs://vacay/holidays/{year}`                  | Vacay    | Public holidays for the plan's configured region and year           |
| Journeys              | `MooNs://journeys`                               | Journey  | All journeys owned or contributed to by the current user            |
| Journey Detail        | `MooNs://journeys/{journeyId}`                   | Journey  | Single journey with entries, contributors, and linked trips         |
| Journey Entries       | `MooNs://journeys/{journeyId}/entries`           | Journey  | All entries in a journey (date, text, mood, linked trip)            |
| Journey Contributors  | `MooNs://journeys/{journeyId}/contributors`      | Journey  | Contributors (owner and collaborators) of a journey                 |

---

## Tools (read-write)

MooNs exposes tools organized by feature area. Use `get_trip_summary` as a starting point — it returns everything about a
...
| `token_auth_notice`  | Static token deprecation notice and migration guide. Only available in sessions authenticated with a legacy `MooNs_` token. |
...
Before writing anything to MooNs, do some research: look up what's worth
...
Once you have a solid plan, write the whole thing to MooNs:
- Create the trip
...
PDF of the generated trip: [./docs/MooNs-Generated-by-MCP.pdf](./docs/MooNs-Generated-by-MCP.pdf)
