...
Single source of truth for MooNs's API contracts, expressed as [Zod](https://zod.dev) schemas
...
Both apps resolve `@moons/shared` to this package's TypeScript source:
...
The canonical **error envelope** is finalised in card **F5** (it must match MooNs's current
