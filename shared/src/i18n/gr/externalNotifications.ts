...
const gr: NotificationLocale = {
  email: {
    footer: 'Λάβατε αυτό το μήνυμα επειδή έχετε ενεργοποιήσει τις ειδοποιήσεις στο moon.',
    manage: 'Διαχείριση προτιμήσεων στις Ρυθμίσεις',
    madeWith: 'Δημιουργήθηκε με',
    openmoon: 'Άνοιγμα moon',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `Ο/Η ${p.actor} σας προσκάλεσε να συγχωνεύσετε τα σχέδια διακοπών σας. Ανοίξτε το moon για να αποδεχτείτε ή να απορρίψετε.`,
    }),
    collection_invite: (p) => ({
      title: 'Πρόσκληση σε συλλογή',
      body: `Ο/Η ${p.actor} σας προσκάλεσε να μοιραστείτε μια συλλογή. Ανοίξτε το moon για να αποδεχτείτε ή να απορρίψετε.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Νέα έκδοση moon διαθέσιμη',
      body: `Η έκδοση moon ${p.version} είναι τώρα διαθέσιμη. Επισκεφθείτε τον πίνακα διαχείρισης για να ενημερώσετε.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Γεια σας',
    body: 'Λάβαμε ένα αίτημα επαναφοράς του κωδικού πρόσβασης για τον λογαριασμό σας στο moon. Κάντε κλικ στο παρακάτω κουμπί για να ορίσετε νέο κωδικό πρόσβασης.',
