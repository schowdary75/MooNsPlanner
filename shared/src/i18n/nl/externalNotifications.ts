...
const nl: NotificationLocale = {
  email: {
    footer: 'Je ontvangt dit omdat je meldingen hebt ingeschakeld in moon.',
    manage: 'Voorkeuren beheren',
    madeWith: 'Made with',
    openmoon: 'moon openen',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} nodigt je uit om vakantieplannen te fuseren. Open moon om te accepteren of af te wijzen.`,
    }),
    collection_invite: (p) => ({
      title: 'Collectie-uitnodiging',
      body: `${p.actor} nodigt je uit om een collectie te delen. Open moon om te accepteren of af te wijzen.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nieuwe moon-versie beschikbaar',
      body: `moon ${p.version} is nu beschikbaar. Bezoek het beheerderspaneel om bij te werken.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hallo',
    body: 'We hebben een verzoek ontvangen om het wachtwoord voor je moon-account te resetten. Klik op de knop hieronder om een nieuw wachtwoord in te stellen.',
