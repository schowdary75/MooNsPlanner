...
const uk: NotificationLocale = {
  email: {
    footer: 'Ви отримали це, оскільки увімкнули сповіщення в moon.',
    manage: 'Керувати налаштуваннями у Налаштуваннях',
    madeWith: 'Made with',
    openmoon: 'Відкрити moon',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} запрошує вас об'єднати плани відпустки. Відкрийте moon, щоб прийняти або відхилити.`,
    }),
    collection_invite: (p) => ({
      title: 'Запрошення до колекції',
      body: `${p.actor} запрошує вас поділитися колекцією. Відкрийте moon, щоб прийняти або відхилити.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Доступна нова версія moon',
      body: `moon ${p.version} тепер доступний. Перейдіть до панелі адміністратора для оновлення.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Привіт',
    body: 'Ми отримали запит на скидання пароля вашого облікового запису moon. Натисніть кнопку нижче, щоб встановити новий пароль.',
