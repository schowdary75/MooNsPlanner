...
const en: NotificationLocale = {
  email: {
    footer: 'You received this because you have notifications enabled in moon.',
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} invited you to fuse vacation plans. Open moon to accept or decline.`,
    }),
    collection_invite: (p) => ({
      title: 'Collection invite',
      body: `${p.actor} invited you to share a collection. Open moon to accept or decline.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'New moon version available',
      body: `moon ${p.version} is now available. Visit the admin panel to update.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hi',
    body: 'We received a request to reset the password for your moon account. Click the button below to set a new password.',
