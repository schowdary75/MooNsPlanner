...
const br: NotificationLocale = {
  email: {
    footer: 'Você recebeu isso porque tem as notificações ativadas no moon.',
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} convidou você para fundir planos de férias. Abra o moon para aceitar ou recusar.`,
    }),
    collection_invite: (p) => ({
      title: 'Convite para coleção',
      body: `${p.actor} convidou você para compartilhar uma coleção. Abra o moon para aceitar ou recusar.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nova versão do moon disponível',
      body: `O moon ${p.version} está disponível. Acesse o painel de administração para atualizar.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Olá',
    body: 'Recebemos um pedido para redefinir a senha da sua conta moon. Clique no botão abaixo para definir uma nova senha.',
