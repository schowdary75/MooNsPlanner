...
const tr: NotificationLocale = {
  email: {
    footer: "moon'te bildirimleri etkinleştirdiğiniz için bunu aldınız.",
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} sizi tatil planlarını birleştirmeye davet etti. Kabul etmek veya reddetmek için moon'i açın.`,
    }),
    collection_invite: (p) => ({
      title: 'Koleksiyon daveti',
      body: `${p.actor} sizi bir koleksiyonu paylaşmaya davet etti. Kabul etmek veya reddetmek için moon’i açın.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Yeni moon sürümü mevcut',
      body: `moon ${p.version} artık mevcut. Güncellemek için yönetici panelini ziyaret edin.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Merhaba',
    body: 'moon hesabınızın şifresini sıfırlamak için bir istek aldık. Yeni bir şifre belirlemek için aşağıdaki butona tıklayın.',
