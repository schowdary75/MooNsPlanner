...
const hu: NotificationLocale = {
  email: {
    footer: 'Ezt az értesítést azért kaptad, mert engedélyezted az értesítéseket a moon-ben.',
    manage: 'Beállítások kezelése',
    madeWith: 'Made with',
    openmoon: 'moon megnyitása',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} meghívott a nyaralási tervek összevonásához. Nyissa meg a moon-et az elfogadáshoz vagy elutasításhoz.`,
    }),
    collection_invite: (p) => ({
      title: 'Gyűjtemény meghívó',
      body: `${p.actor} meghívott egy gyűjtemény megosztására. Nyissa meg a moon-et az elfogadáshoz vagy elutasításhoz.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Új moon verzió érhető el',
      body: `A moon ${p.version} elérhető. Látogasson el az adminisztrációs panelre a frissítéshez.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Szia',
    body: 'Kérést kaptunk a moon-fiókod jelszavának visszaállítására. Kattints az alábbi gombra az új jelszó beállításához.',
