...
const es: NotificationLocale = {
  email: {
    footer: 'Recibiste esto porque tienes las notificaciones activadas en moon.',
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} te invitó a fusionar planes de vacaciones. Abre moon para aceptar o rechazar.`,
    }),
    collection_invite: (p) => ({
      title: 'Invitación a colección',
      body: `${p.actor} te invitó a compartir una colección. Abre moon para aceptar o rechazar.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nueva versión de moon disponible',
      body: `moon ${p.version} ya está disponible. Visita el panel de administración para actualizar.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hola',
    body: 'Recibimos una solicitud para restablecer la contraseña de tu cuenta de moon. Haz clic en el botón de abajo para establecer una nueva contraseña.',
