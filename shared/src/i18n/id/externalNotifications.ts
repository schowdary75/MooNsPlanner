...
const id: NotificationLocale = {
  email: {
    footer: 'Anda menerima ini karena Anda telah mengaktifkan notifikasi di moon.',
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} mengundang Anda untuk menggabungkan rencana liburan. Buka moon untuk menerima atau menolak.`,
    }),
    collection_invite: (p) => ({
      title: 'Undangan koleksi',
      body: `${p.actor} mengundang Anda untuk berbagi koleksi. Buka moon untuk menerima atau menolak.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Versi moon baru tersedia',
      body: `moon ${p.version} sekarang tersedia. Kunjungi panel admin untuk memperbarui.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Halo',
    body: 'Kami menerima permintaan untuk menyetel ulang kata sandi akun moon Anda. Klik tombol di bawah untuk menetapkan kata sandi baru.',
