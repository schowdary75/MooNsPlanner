...
const ru: NotificationLocale = {
  email: {
    footer: 'Вы получили это, потому что у вас включены уведомления в moon.',
    manage: 'Управление настройками',
    madeWith: 'Made with',
    openmoon: 'Открыть moon',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} приглашает вас объединить планы отпуска. Откройте moon для подтверждения.`,
    }),
    collection_invite: (p) => ({
      title: 'Приглашение в коллекцию',
      body: `${p.actor} приглашает вас поделиться коллекцией. Откройте moon для подтверждения.`,
    }),
    photos_shared: (p) => ({
      title: `${p.count} фото`,
    ...
    version_available: (p) => ({
      title: 'Доступна новая версия moon',
      body: `moon ${p.version} теперь доступен. Перейдите в панель администратора для обновления.`,
    }),
  ...
  passwordReset: {
    ...
    body: 'Мы получили запрос на сброс пароля вашего аккаунта moon. Нажмите кнопку ниже, чтобы установить новый пароль.',
