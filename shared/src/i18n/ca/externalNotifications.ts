...
const ca: NotificationLocale = {
  email: {
    footer: 'Has rebut això perquè tens les notificacions activades a moon.',
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} et va convidar a fusionar plans de vacances. Obre moon per acceptar o rebutjar.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Versió nova de moon disponible',
      body: `moon ${p.version} ja està disponible. Visita el panell d'administració per actualitzar.`,
    }),
    ...
    collection_invite: (p) => ({
      ...
      body: `${p.actor} t'ha convidat a compartir una col·lecció. Obre moon per acceptar-la o rebutjar-la.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hola',
    body: 'Vam rebre una sol·licitud per restablir la contrasenya del teu compte de moon. Fes clic al botó de sota per establir una contrasenya nova.',
