...
const admin: TranslationStrings = {
  ...
  'admin.plugins.disabledBody':
    'プラグインランタイムがオフになっています（moon_PLUGINS_ENABLED）。管理者がサーバー設定で有効にするまで、プラグインは実行できません。',
  ...
  'admin.plugins.requiresmoon': 'moon {version} 以上が必要',
  ...
  'admin.plugins.reviewedMeaning':
    '「レビュー済み」とは、moonのメンテナーが各バージョンでこのプラグインをマルウェアの観点から確認したという意味です — 品質や動作の可否を確認したものではありません。プラグインが無害であることを保証するものではありません。',
  ...
  'admin.plugins.security.isolationBody':
    'プラグインは、自身のファイルしか読み取れない厳重に制限された独立プロセスとして実行されます。データベース、ログイン用のシークレット、暗号鍵を読み取ることはできず、他のプログラムを起動することも、どこかにファイルを書き込むこともできません。その画面は隔離されたブラウザフレーム内で動作し、セッションCookieを読み取ったり、周囲のmoonページに干渉したりすることはできません。',
  'admin.plugins.security.permsTitle': '権限の意味',
  'admin.plugins.security.permsBody':
    'インストール前に表示される権限は、プラグインの実行中にmoonが強制する厳格な制限です — リストにないことは一切実行できません。ただし、これらはプラグインが何をできるかを示すもので、実際に何をするかを示すものではありません。旅行の読み取りとサーバーへの接続を許可されたプラグインは、あなたの旅行をそのサーバーに送信できます。説明文だけでなく、権限と接続先ホストも確認してください。',
  'admin.plugins.security.limitsTitle': '保証できないこと',
  'admin.plugins.security.limitsBody':
    'この分離は強固なソフトウェア上の境界ですが、絶対的なものではありません。プラグインは、あなたが承認した権限のとおりに動作するため、その権限の範囲内では説明とは異なる振る舞いをする場合があり、正当に保持しているデータを宣言済みのホストへ送信することもできます。moonは、プラグインのコードが実際に何をするかを読み取ったり判断したりはしません。',
  ...
  'admin.plugins.security.reviewedBody':
    'レビュー済みのプラグインは、moonのメンテナーが各バージョンで手動でマルウェアの有無を確認したものです — 悪意のあるコードの有無を確認したものであり、正しく動作するかどうかを確認したものではありません。プラグインが無害であることを約束するものではありません。',
  'admin.plugins.security.signedTitle': '「署名済み」の意味',
  'admin.plugins.security.signedBody':
    'moonがインストールのたびに検証するチェックサムは、ファイルがレジストリの保証するものとまったく同一であることを証明します。署名が証明するのは別のことです。そのファイルが作者本人から届いたこと、そして作者だけが持つ鍵で署名されていることです。署名済みのプラグインはその両方を備えています。署名がないプラグインが危険というわけではありません — 保証がひとつ少ないだけであり、現時点ではレジストリのプラグインの多くは署名されていません。',
  ...
  'admin.plugins.sig.missingBody':
    'このプラグインはインストール時には署名されていましたが、新しいバージョンには署名がありません。moonはこの後退を黙って受け入れることはしません。これを上書きすることはできません。',
  ...
  'admin.plugins.sig.confirmOutOfBand':
    'moonには、正当な鍵の更新と乗っ取りを見分けることはできません — ここからは両者はまったく同じに見えます。受け入れる前に、すでに信頼している経路で作者に新しい鍵を確認してください。受け入れると、プラグインが更新され、新しい鍵が記憶されます。',
  ...
  'admin.plugins.security.trustBody':
    'プラグインのインストールは、サードパーティ製アプリのインストールと同じです。信頼できる作者のコードのみを追加し、少しでも疑わしい場合は、まずご自身で内容を確認してください。moonはサードパーティ製プラグインについて一切の責任を負いません。',
  ...
  'admin.plugins.dep.moonIncompatible': 'moon {range} が必要です — このサーバーは {host} です',
  'admin.plugins.dep.moonUnknown': '対応する moon バージョンが宣言されていません',
  ...
  'admin.addons.subtitle': '機能を有効／無効にしてmoonをカスタマイズします。',
  ...
  'admin.weather.description':
    'moonは天気データにOpen‑Meteoを使用しています。無料でオープンソース、APIキーは不要です。',
  ...
  'admin.github.by': '作成者',
  'admin.github.support': 'moonの開発を支援',
  'admin.update.available': '更新があります',
  'admin.update.text': 'moon {version} が利用可能です。現在は {current} を使用しています。',
  'admin.update.button': 'GitHubで見る',
  'admin.update.install': '更新をインストール',
  'admin.update.confirmTitle': '更新をインストールしますか？',
  'admin.update.confirmText': 'moonを {current} から {version} に更新します。更新後、サーバーは自動的に再起動します。',
  ...
  'admin.update.dockerText':
    'moonはDockerで実行されています。{version} に更新するには、サーバーで次のコマンドを実行してください：',
  'admin.update.nonDockerText':
    'このmoonインスタンスはDockerで実行されていません。{version} に更新するには、使用したインストールまたは更新方法をもう一度実行してください。たとえばProxmox Community Scriptsの場合は、LXCコンソールから更新を実行します：',
  ...
  'admin.passkey.rpIdHint':
    'パスキーが紐づくドメイン名のみ（例：moon.example.org）。空欄の場合は APP_URL から導出されます。後で変更すると既存のパスキーは無効になります。',
  'admin.passkey.origins': '許可するオリジン',
  'admin.passkey.originsHint':
    'カンマ区切りの完全なオリジン（例：https://moon.example.org）。空欄の場合は APP_URL を使用します。',
