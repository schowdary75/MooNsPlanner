...
const ja: NotificationLocale = {
  email: {
    footer: 'moonで通知を有効にしているため、このメールが届きました。',
    manage: '設定で通知設定を管理',
    madeWith: 'Made with',
    openmoon: 'moonを開く',
  },
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor}が休暇プランの統合に招待しています。moonを開いて承認または拒否してください。`,
    }),
    collection_invite: (p) => ({
      title: 'コレクション招待',
      body: `${p.actor}がコレクションの共有に招待しています。moonを開いて承認または拒否してください。`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: '新しいmoonバージョンが利用可能',
      body: `moon ${p.version}が利用可能になりました。管理パネルからアップデートしてください。`,
    }),
  ...
  passwordReset: {
    subject: 'パスワードをリセット',
    greeting: 'こんにちは',
    body: 'moonアカウントのパスワードリセットリクエストを受け付けました。以下のボタンをクリックして新しいパスワードを設定してください。',
    ctaIntro: 'パスワードをリセット',
