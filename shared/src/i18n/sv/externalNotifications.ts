...
const en: NotificationLocale = {
  email: {
    footer: 'Du har fått detta eftersom du har aktiverat aviseringar i moon.',
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} bjöd in dig att slå samman semesterplaner. Öppna moon för att acceptera eller avvisa.`,
    }),
    collection_invite: (p) => ({
      title: 'Inbjudan till samling',
      body: `${p.actor} bjöd in dig att dela en samling. Öppna moon för att acceptera eller avvisa.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Ny moon version tillgänglig',
      body: `moon ${p.version} är nu tillgänglig. Gå till adminpanelen för att uppdatera.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hej',
    body: 'Vi har fått en begäran om att återställa lösenordet till ditt moon konto. Klicka på knappen nedan för att ange ett nytt lösenord.',
