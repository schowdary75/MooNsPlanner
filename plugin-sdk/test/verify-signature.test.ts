 ...
 * point: moons's host verifier (server/src/nest/plugins/install/verify-signature.ts) and the
 * registry's port of it (moons-Plugins scripts/lib/verify-signature.mjs). A gate that is
...
function keypair(): { key: crypto.KeyObject; pub: string } {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'moons-sig-'));
...
describe('checkSignatureShape', () => {
  ...
  // Both halves of a half-signed entry. moons refuses to install either ("incomplete
