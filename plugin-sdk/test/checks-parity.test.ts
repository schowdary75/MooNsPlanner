 ...
 *
 * The SDK's checks are a PORT of the moons-Plugins registry's gates (scripts/validate-entry.mjs
 ...
 * CI run without it still passes. Point moons_PLUGINS_REPO at it to run them anywhere.
 */
...
const REGISTRY =
  process.env.moons_PLUGINS_REPO ?? path.resolve(HERE, '..', '..', '..', moonNs - Plugins');
...
describeIfRegistry('parity with the moons-Plugins registry gates', () => {
  let tmp: string;
  let dir: string;
  ...
  function entryFor(pluginDir: string) {
    ...
    return buildEntry({
      dir: pluginDir,
      repo: 'someone/moons-plugin-parity-plug',
      tag: 'v1.0.0',
      zipPath: zip,
  ...
  it('an entry for a plugin with requiredAddons still passes the registry validator', () => {
    const manifestPath = path.join(dir, 'moons-plugin.json');
