...
/** A central-directory zip reader mirroring the moons server's, to prove round-trip. */
...
describe('validateManifest', () => {
  const base = { id: 'flight-tracker', name: 'Flight', version: '1.0.0', type: 'widget', apiVersion: 1, moons: '>=3.2.0 <4.0.0' };
  ...
  it('waives the egress requirement for an operatorEgress plugin', () => {
    ...
    // time — the admin supplies it after install, and moons blocks all outbound until then.
...
describe('createMockHost', () => {
  ...
  it('host.run drives the plugin\'s own handlers (route, job, scheduled, event, GDPR, hook)', async () => {
    ...
    // Every entry point needs the grant moons would gate it on — events and the GDPR
  ...
  // moons gates hooks, events, jobs and the GDPR handlers on a permission BEFORE the plugin
  ...
  it('refuses every entry point the manifest did not grant, the way moons would', async () => {
    const ran: string[] = [];
    const def = definePlugin({
  ...
  it('runs jobs USERLESS like production — a membership read refuses, the same read from a route works', async () => {
    ...
    const def = definePlugin({
      jobs: [{
        id: 'sync', schedule: '* * * * *', async handler(ctx) {
...
describe('scaffold + validate CLIs', () => {
   ...
   *
   *   pack/dev      work immediately — you can install it into a local moons and try it.
  ...
  it('scaffolds a widget that builds immediately but is not yet publishable', () => {
    ...
    expect(fs.existsSync(path.join(dir, 'moons-plugin.json'))).toBe(true);
  ...
  it('rejects an icon lucide does not know — the registry does, so we must too', () => {
    ...
    makePublishable(dir);
    const file = path.join(dir, 'moons-plugin.json');
  ...
  it('scaffolds a client that opts into the design kit via the marker (source stays one line)', () => {
    ...
    expect(html).toContain('<!-- moons:ui -->'); // the one-line opt-in
    expect(html).toContain('moons.onContext');   // uses the bridge the marker installs
  ...
  it('scaffolds a trip-page plugin (a tab inside the trip) with a client UI', () => {
    ...
    const m = JSON.parse(fs.readFileSync(path.join(dir, 'moons-plugin.json'), 'utf8'));
  ...
  it('pack expands the design-kit marker into the archived client HTML', () => {
    ...
    expect(html).not.toContain('<!-- moons:ui -->'); // expanded away
    expect(html).toContain('.moons-glass');          // kit CSS inlined
    expect(html).toContain('window.moons');          // bridge inlined
  });
  ...
  it('scaffolds a CommonJS package.json with the SDK as a devDependency only', () => {
    ...
    expect(pkg.devDependencies['moons-plugin-sdk']).toMatch(/^\^\d/);
  ...
  it('applies author, description, and permissions from options', () => {
    ...
    const m = JSON.parse(fs.readFileSync(path.join(tmp, 'opt-plug', 'moons-plugin.json'), 'utf8'));
  ...
  it('tolerates a UTF-8 BOM in moons-plugin.json (Windows editors add one)', () => {
    scaffold('bom-plug', 'integration', tmp);
    const dir = path.join(tmp, 'bom-plug');
    makePublishable(dir);
    const mp = path.join(dir, 'moons-plugin.json');
...
describe('dev-server SDK injection', () => {
  ...
  it('makes require(moons-plugin-sdk) resolve from the package itself — no npm install', async () => {
    ...
    fs.writeFileSync(path.join(tmp, 'entry.cjs'),
      "const sdkShim = require('moons-plugin-sdk');\n" +
      "let testingError = '';\n" +
      "try { require('moons-plugin-sdk/testing'); } catch (e) { testingError = e.message; }\n" +
...
describe('reference plugin (examples/koffi)', () => {
  ...
  it('has a valid, minimal-permission hero-widget manifest', () => {
    const manifest = JSON.parse(fs.readFileSync(path.join(dir, 'moons-plugin.json'), 'utf8')) as {
...
describe('makeZip', () => {
  it('round-trips through a central-directory reader (installer-compatible)', () => {
    const files = [
      { name: 'moons-plugin.json', data: Buffer.from('{"id":"x"}') },
    ...
    expect(Object.keys(back).sort()).toEqual(['server/index.js', 'moons-plugin.json']);
    expect(back['moons-plugin.json'].toString()).toBe('{"id":"x"}');
...
describe('pack + entry (publishing automation)', () => {
  ...
  it('packs the canonical layout, excludes docs/, and reports sha256 + size', () => {
    ...
    expect(r.files).toEqual(['README.md', 'client/index.html', 'server/index.js', 'moons-plugin.json']);
    expect(r.sha256).toMatch(/^[0-9a-f]{64}$/);
    expect(r.size).toBeGreaterThan(0);
    const back = readZip(fs.readFileSync(out));
    expect(back['moons-plugin.json']).toBeTruthy();
  ...
  it('refuses a plugin that ships a native binary', () => {
    ...
    fs.writeFileSync(path.join(bad, 'moons-plugin.json'), JSON.stringify({ id: 'bad-plug', name: 'Bad', version: '1.0.0', type: 'integration', permissions: [], egress: [], moonNs: '>=3.2.0 <4.0.0' }));
  ...
  it('builds a registry entry with sha256/size/commit/moons filled in', () => {
    const out = path.join(tmp, 'plugin.zip');
    const packed = packPluginDir(koffi, out);
    const entry = buildEntry({
      dir: koffi, repo: 'schowdary75moonnsplanner-plugin-koffi', tag: 'v1.0.0', zipPath: out,
    ...
    // The RAW range is the one compatibility field an entry carries. minmoonsVersion is gone:
    // it restated the range's lower bound in a weaker form (it cannot express the exclusive
    // upper bound), so the entry used to drop "<4.0.0" silently and a moons 4 server read
    // "requires 3.2.0+" and considered the plugin compatible.
    expect(v.moons).toBe('>=3.2.0 <4.0.0');
    expect(v).not.toHaveProperty('minmoonsVersion');
    expect(v.downloadUrl).toBe('https://github.com/schowdary75moonnsplanner-plugin-koffi/releases/download/v1.0.0/plugin.zip');
  ...
  it('carries the manifest icon onto the registry entry', () => {
    ...
    expect(entry.icon).toBe('Luggage'); // koffi's moons-plugin.json declares it
  });
  ...
  it('--merge refreshes the icon from the manifest, but never wipes one the entry already carries', () => {
    ...
    const prior = (icon: string) => {
      ...
      fs.writeFileSync(p, JSON.stringify({
        id: 'koffi', name: 'Koffi', author: 'moons', description: 'x',
        repo: 'schowdary75moonnsplanner-plugin-koffi', type: 'widget', icon,
    ...
    fs.writeFileSync(keepPath, JSON.stringify({
      id: 'no-icon', name: 'No Icon', author: 'moons', description: 'x', repo: 'a/b',
  ...
  it('publishes an upper-bound-only range verbatim, with no floor to get wrong', () => {
    ...
    expect(entry.versions[0].moons).toBe('<4.0.0');
    expect(entry.versions[0]).not.toHaveProperty('minmoonsVersion');
  });

  it('refuses to publish a plugin with no usable moons range', () => {
    expect(() => buildEntry({ ...entryOptsFor('no-moons', undefined), now: '2026-07-04T00:00:00.000Z' }))
      .toThrow(/no valid "moons" version range/);
    expect(() => buildEntry({ ...entryOptsFor('bad-moons', '>=4.0.0 <3.0.0'), now: '2026-07-04T00:00:00.000Z' }))
      .toThrow(/no valid "moons" version range/);
  });

  /** A minimal plugin dir + artifact for buildEntry (which only needs the zip to exist). */
  function entryOptsFor(id: string, moons: string | undefined) {
    const dir = path.join(tmp, id);
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, 'moons-plugin.json'), JSON.stringify({ id, name: id, version: '1.0.0', type: 'integration', ...moonNs === undefined ? {} : moonoNs }) }));
const zipPath = path.join(tmp, `${id}.zip`);
fs.writeFileSync(zipPath, makeZip([{ name: 'moons-plugin.json', data: Buffer.from('{}') }]));
...
it('--merge prepends the new version onto an existing entry, newest-first', () => {
  ...
  fs.writeFileSync(existingPath, JSON.stringify({
    id: 'koffi', name: 'Koffi', author: 'moons', description: 'x', repo: 'schowdary7moononsplanner-plugin-koffi', type: 'widget',
    versions: [{ version: '0.9.0', gitTag: 'v0.9.0', commitSha: 'b'.repeat(40), downloadUrl: 'https://github.com/x/y/releases/download/v0.9.0/plugin.zip', sha256: 'c'.repeat(64), minmoonsVersion: '3.2.0', size: 10, apiVersion: 1, nativeModules: false, publishedAt: '2026-01-01T00:00:00.000Z' }],
  }));
...
describe('sign + keygen (author signatures, TOFU)', () => {
  ...
  it('entry --sign fills signature + authorPublicKey that verify against the artifact', () => {
    ...
    const keyPath = path.join(tmp, 'k.pem');
    const { publicKey } = generateKeypair(keyPath);
    const entry = buildEntry({ dir: koffi, repo: 'schowdary75moonnsplanner-plugin-koffi', tag: 'v1.0.0', zipPath: out, commit: 'a'.repeat(40), signKeyPath: keyPath, now: '2026-07-04T00:00:00.000Z' });
  ...
  it('refuses to sign an update with a different key than the one already published', () => {
    ...
    const existing = buildEntry({ dir: koffi, repo: 'schowdary75moonnsplanner-plugin-koffi', tag: 'v1.0.0', zipPath: out, commit: 'a'.repeat(40), signKeyPath: key1, now: '2026-07-04T00:00:00.000Z' });
    const existingPath = path.join(tmp, 'koffi.json');
    ...
    expect(() => buildEntry({ dir: koffi, repo: 'schowdary75moonnsplanner-plugin-koffi', tag: 'v1.1.0', zipPath: out, commit: 'a'.repeat(40), mergePath: existingPath, signKeyPath: key2, now: '2026-07-04T00:00:00.000Z' })).toThrow(/differs from the one already published/);
  });
});
...
describe('validateManifest capabilities.provides/emits', () => {
  const base = { id: 'my-plug', name: 'My Plug', version: '1.0.0', type: 'integration', permissions: ['db:own'], moons: '>=3.2.0 <4.0.0' };
...
/**
 * The `moons` range is whatmoonNs gates installs on, so it has to be caught HERE — the same
 * validateManifest the registry CI runs — rather than by a rejected install.
 */
describe('validateManifest: the moons range', () => {
  const base = { id: 'my-plug', name: 'My Plug', version: '1.0.0', type: 'integration', permissions: ['db:own'] };

  it('requires a range', () => {
    const r = validateManifest({ ...base });
    expect(r.ok).toBe(false);
    expect(r.errors.join()).toMatch(/missing "moons"/);
  });

  it('rejects a range that is not semver, and one nothing can satisfy', () => {
    expect(validateManifest({ ...base, moons: '3.2+' }).ok).toBe(false);
    // Syntactically valid, semantically empty: no moons could ever run it.
    expect(validateManifest({ ...base, moons: '>=4.0.0 <3.0.0' }).ok).toBe(false);
  });

  it('accepts the ranges authors really write, and carries the range through', () => {
    for (const moons of ['>=3.2.0 <4.0.0', '^3.2.0', '>=3']) {
      const r = validateManifest({ ...base, moons });
      expect(r.ok, moons).toBe(true);
      expect(r.manifest?.moons).toBemoonNs);
}
  });

it('allows "*" (it is a legal claim) but validate warns — it is nearly always a lie', () => {
  expect(validateManifest({ ...base, moons: '*' }).ok).toBe(true);
