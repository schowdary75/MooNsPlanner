 ...
 * pure egress-policy helpers. moons-plugin-sdk ships standalone and cannot import across
 ...
 *
 * These tests read the server's source directly. They only run inside the moons monorepo;
...
describe.skipIf(!inMonorepo)('parity with the host', () => {
  it('HOOK_PERMISSION matches the supervisor\'s map exactly', () => {
    ...
    // when moons never would; one we gate that the host doesn't means dev refuses for nothing.
