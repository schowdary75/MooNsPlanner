...
describe('manifest checks', () => {
  ...
  it('allows no icon at all (moons falls back to Blocks) but says so', () => {
    const m = { ...GOOD_MANIFEST };
  ...
  it('warns — but does not fail — on an unbounded moons range', () => {
    const r = runOffline(ctx({ manifest: { ...GOOD_MANIFEST, moons: '*' } }));
    expect(warned(r, 'manifest.moons-range')).toBe(true);
    expect(failed(r, 'manifest.moons-range')).toBe(false);
  });
});
 ...
 * The egress trap.
 *
 * moons builds the child's network allow-list and the iframe CSP from the `http:outbound:<host>`
...
describe('README checks', () => {
  ...
  it('fails a screenshot link whose file does not exist', () => {
    ...
    expect(r.errors.find((e) => e.id === 'docs.screenshot')?.next).toBe('moons-plugin shot');
  });
 ...
 * install a plugin into a local moons to try it, and an unwritten README does not stop it running.
 */
describe('what blocks what', () => {
  ...
  it('a broken manifest blocks the artifact too — that plugin could never load', () => {
    const broken = runOffline(ctx({ manifest: { ...GOOD_MANIFEST, moons: 'not-a-range' } }));
...
describe('README helpers mirror the registry exactly', () => {
  ...
  it('spots each template placeholder the registry spots', () => {
    ...
    expect(placeholders('see your-name/moons-plugin-x')).toHaveLength(1);
