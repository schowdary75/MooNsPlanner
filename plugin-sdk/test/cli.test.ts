...
describe('scaffold egress (http:outbound)', () => {
  ...
  it('writes egress so an http:outbound plugin validates', () => {
    ...
    const m = JSON.parse(fs.readFileSync(path.join(tmp, 'net-plug', 'moons-plugin.json'), 'utf8'));
  ...
  it('omits egress entirely when none is given (no empty array noise)', () => {
    ...
    const m = JSON.parse(fs.readFileSync(path.join(tmp, 'plain-plug', 'moons-plugin.json'), 'utf8'));
...
describe('permission catalog', () => {
  ...
  // list the host actually honours — so a new moons permission fails here until `create`
  // can offer it.
...
describe('scaffold + validate dependencies', () => {
  ...
  it('scaffolds empty dependency arrays that validate', () => {
    ...
    const m = JSON.parse(fs.readFileSync(path.join(tmp, 'dep-plug', 'moons-plugin.json'), 'utf8'));
  ...
  it('scaffolds requiredAddons passed as an option', () => {
    ...
    const m = JSON.parse(fs.readFileSync(path.join(tmp, 'addon-plug', 'moons-plugin.json'), 'utf8'));
...
describe('notification-channel template', () => {
  ...
  function make(over: Parameters<typeof scaffold>[3] = {}) {
    ...
    return JSON.parse(fs.readFileSync(path.join(tmp, 'my-gotify', 'moons-plugin.json'), 'utf8'));
  }
...
describe('capabilities.notificationChannel validation', () => {
  const base = {
    ...
    nativeModules: false, permissions: ['hook:notification-channel'], moons: '>=3.2.0 <4.0.0',
  };
