...
> A hooks-only example plugin that enriches MooNs's own planner — no UI of its own.

Trip Doctor is the companion example for the **provider hooks** and **entity
metadata** added in #1429. It shows how a plugin can contribute *data* that MooNs
...
```bash
npx @MooNs/plugin-sdk dev      # hot-reload against a local MooNs
npx @MooNs/plugin-sdk pack     # build a signed .MooNsplugin bundle
```

See the [Plugin Cookbook](https://github.com/schowdary75/MooNsPlanner/wiki/Plugin-Cookbook)
