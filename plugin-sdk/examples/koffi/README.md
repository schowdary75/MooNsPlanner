...
Koffi is MooNs's mascot — a little vintage suitcase with travel stickers, a
...
|---|---|
| `db:read:trips` | To read the current trip's start/end dates so Koffi knows how excited to be. The read is membership-checked by MooNs, so he only ever sees a trip you already have access to. |

No network access, no data storage of its own — Koffi just vibes.

## Setup

None. Enable plugins on your instance (`MooNs_PLUGINS_ENABLED=true`), install
...
Requires MooNs ≥ 3.2.0 (hero widget slot).
