/**
 * moons-plugin-sdk — the author-facing SDK for buildingmoonNs plugins (#plugins, M6).
 *
 * Types + `definePlugin` mirror what the isolated runtime injects, so a plugin
 * written against this package runs unchanged inside moons. Pure and
 * dependency-free.
 */
...
export interface NotificationMessage {
  /** The moons event that produced this, e.g. `trip_invite`. */
  event: string;
  title: string;
  body: string;
  /** Absolute deep link back into moons, when the event has one. */
  url?: string;
  tripName?: string;
}

/**
 * Deliver moons notifications over a channelmoonNs doesn't ship (Gotify, Pushover, …).
...
/** One row of extra place info moons renders natively (reviews/ratings/links/…). */
...
/** A validation/warning a plugin raises on a trip; moons surfaces it in the planner. */
...
/** One row of extra info moons renders under a journal entry (same shape as PlaceDetailItem). */
...
export interface PluginDefinition {
  ...
  /** GDPR erasure: a moons account was deleted — remove everything you hold about it
...
/** Define a plugin. Gives you types; the returned object is what moons loads. */
...
// The permissions moons enforces OUTSIDE ctx — hooks, events, jobs, egress. An entry
...
// (or drop a `<!-- moons:ui -->` marker and let `dev`/`pack` expand it) to get the
// native moons look — glass, hover, buttons, inputs — plus a `windowmoonNs` bridge.
export { moons_UI_CSS, moonNs_THEME_JSmoonoNs_UI_MARKER, injmoonooNsUi } from './ui/kit.js';
