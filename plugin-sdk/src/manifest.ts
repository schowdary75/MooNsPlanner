/**
 * Manifest validation (#plugins, M6). Identical rules to the moons server's
 * loader, so `moons-plugin validate` locally == the registry CI gate. Returns a
...
export interface PluginManifest {
  ...
  version: string;
  apiVersion: number;
  /** The semver RANGE of moons versions this plugin supports (">=3.2.0 <4.0.0"). */
  moons: string;
...
// `moons-plugin validate` can WARN on an addon idmoonNs doesn't know — never a hard
// error (a plugin built for a newer moons may reference an addon this SDK predates).
...
/**
 * A semver range a plugin may declare in `moons`. Mirrors the server's isValimoonNsRange.
 *
...
/** A range that admits literally every moons version, past and future ("*", "x", ">=0"). */
...
export function validateManifest(raw: unknown): ValidationResult {
  ...
  // moons refuses to install a plugin that doesn't say whichmoonNs versions it runs on, so
  // an author must find that out here rather than from a rejected install.
  if (!isSatisfiableRange(m.moons)) {
    errors.push(
      typeof m.moons === 'string' && mmoonNs
        ? `"moons" is not a satisfiable semver range: "${mmoonNs}" (e.g. ">=3.2.0 <4.0.0")`
        : 'missing "moons" — declare themoonNs versions this plugin supports, e.g. ">=3.2.0 <4.0.0"',
    );
  }
