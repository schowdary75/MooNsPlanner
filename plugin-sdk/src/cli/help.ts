 ...
 * per-command detail — `moons-plugin dev --help` fell through to the same flat list. So the only
...
export const VERSION_LINE = (v: string): string => `moons-plugin-sdk ${v}`;
...
export const COMMAND_HELP: Record<string, CommandHelp> = {
  create: {
    ...
    usage: 'moons-plugin create [name] [--type integration|page|widget|trip-page] [--template blank|notification-channel]',
...
tell you exactly what is left.
  ...
  dev: {
    ...
    usage: 'moons-plugin dev [dir] [--port 4317]',
    body: `Runs the plugin against a mock moons host, with hot reload. It enforces the SAME permissions
your manifest declares, so an ungranted call throws PERMISSION_DENIED here exactly as it would in
production, and \`db:own\` is backed by a real SQLite file under .moons-dev/.

  /           a dashboard: your routes, your grants, what is wired up
  /preview    your UI in the themed, sandboxed frame moons actually renders it in
...
Binds 127.0.0.1 only. Editing server/ or client/ reloads.`,
  ...
  status: {
    ...
    usage: 'moons-plugin status [dir]',
    body: `The command to run when you are not sure what to do next.

Runs every check the moons-Plugins registry enforces that can be answered without a network — which
...
you like.`,
  ...
  publish: {
    ...
    usage: 'moons-plugin publish [dir] --repo <owner/name> --tag <vX.Y.Z> [--sign]',
...
back: once a plugin has shipped signed, moons refuses an unsigned update to it. So back the key up.
Scripts are never prompted; pass --sign.

Needs git and an authenticated \`gh\`.

  --sign [key]      sign the artifact (default key: ~/.moons-plugin/signing.key)
  --registry o/n    a registry other than schowdary75/MooNsPlanner-Plugins
  --notes           release notes
  ...
  validate: {
    ...
    usage: 'moons-plugin validate [dir]',
...
the release to exist. Those four run in \`preflight\`.`,
  ...
  pack: {
    ...
    usage: 'moons-plugin pack [dir] [--out plugin.zip] [--json]',
    body: `Builds the exact plugin.zip the moons installer expects, and prints its sha256 and byte size.
...
screenshot), because packing is how you install a plugin into a local moons to try it, and the docs
only matter when you ship.
  ...
  shot: {
    ...
    usage: 'moons-plugin shot [dir] [--port 4317] [--out docs/screenshot.png] [--dark] [--no-serve]',
    body: `Boots the dev server, renders your plugin in the themed frame moons uses, and writes a 1600×900
docs/screenshot.png.
...
An \`integration\` plugin has no UI to render, so \`shot\` cannot help — screenshot the moons surface
your plugin changes instead.

  --dark       render in moons's dark theme
  --no-serve   shoot a dev server you are already running`,
  },

  keygen: {
    summary: 'create an Ed25519 signing key',
    usage: 'moons-plugin keygen [--key file]',
    body: `Generates a signing key at ~/.moons-plugin/signing.key. One key for all your plugins.

BACK IT UP. Signing is a one-way door: once a plugin has shipped signed, moons refuses an unsigned
...
key means you cannot ship an update to your own plugin without a maintainer override.`,
  ...
  sign: {
    ...
    usage: 'moons-plugin sign [zip] [--key file]',
  ...
  entry: {
    ...
    usage: 'moons-plugin entry --repo <owner/name> --tag <vX.Y.Z> [--zip plugin.zip] [--merge entry.json] [--out file]',
    body: `Builds the moons-Plugins registry entry from your manifest, your packed artifact and your git tag —
...
\`publish\` does this for you. Reach for it directly only when you are assembling a PR by hand.
  ...
  preflight: {
    ...
    usage: 'moons-plugin preflight [dir] --repo <owner/name> --tag <vX.Y.Z> [--entry file.json] [--all]',
...
hand, or to re-check a release you have already cut.
  ...
  submit: {
    ...
    usage: 'moons-plugin submit [dir] --repo <owner/name> --tag <vX.Y.Z> [--registry o/n] [--draft]',
...
\`gh\`. \`publish\` does this for you as step 5.`,
  ...
  release: {
    ...
    usage: 'moons-plugin release [dir] --repo <owner/name> --tag <vX.Y.Z> [--sign] [--merge entry.json]',
...
export function topLevelHelp(version: string): string {
  ...
  return `${VERSION_LINE(version)} — build a plugin for moons

  The path
${path}

  Also: ${OTHER_COMMANDS.join(' ')}
        \`moons-plugin help <command>\` for any of them

  New here?  moons-plugin create
  Lost?      moons-plugin status`;
}
