#!/usr/bin/env node
/**
 * `moons-plugin <command>` — the plugin author CLI (#plugins).
 *
...
async function ensureRepoTag(f: Flags, failMsg: string): Promise<{ repo: string; tag: string }> {
  ...
  const repo = f.repo || await promptText({
    message: 'GitHub repo (owner/name)', placeholder: 'you/moons-plugin-thing',
...
const USAGE = 'usage: moons-plugin <create|dev|status|publish|...>   (moonNs-plugin help` for the full list)';
...
async function main(): Promise<void> {
  ...
  //   moons-plugin help            the path, plus where to find everything else
  //   moons-plugin help <cmd>      that command's own page
  //   moons-plugin <cmd> --help    the same page — this is what people actually type
...
function readPluginId(dir: string): string {
  try {
    const m = readJsonFile<{ id?: unknown }>(path.join(path.resolve(dir), 'moons-plugin.json'));
...
async function promptPluginDir(command: string): Promise<string | undefined> {
  ...
  if (fs.existsSync(path.join(process.cwd(), 'moons-plugin.json'))) return undefined;
  const dir = await promptText({
    ...
    validate: (v) => {
      ...
      if (!fs.existsSync(path.join(d, 'moons-plugin.json'))) return `${d} has nomoonNs-plugin.json`;
      return undefined;
    },
  });
  return dir.trim() || '.';
...
async function dispatch(command: string, f: Flags, positional: string[]): Promise<void> {
  ...
  if (command === 'create') {
    ...
    // The SAME text the wizard and the `create-moons-plugin` bin print. These three used to
    // name three different next commands.
    console.log(nextStepsAfterCreate(path.join(process.cwd(), name)));
  } else if (command === 'dev') {
    if (tui) intro(`moons-plugin dev — ${positional[0] || '.'}`);
    ...
    if (tui) {
      ...
      try {
        ...
        outro('next →  moons-plugin status');
      } catch (e) {
    ...
    if (!report.ok) {
      ...
      console.error(`\n${report.errors.length} of these would be rejected by the registry. \`moons-plugin status\` shows the full checklist.`);
      process.exit(1);
    }
    ...
    if (f.json) {
      ...
      logInfo('Install this into a local moons to try it. To ship it: moonNs-plugin publish`.');
    } else {
      console.log(`Packed ${r.files.length} files -> ${rel}`);
      for (const file of r.files) console.log('  ' + file);
      console.log(`\nsha256: ${r.sha256}\nsize:   ${r.size}`);
      console.log('\nInstall this into a local moons to try it. To ship it: moonNs-plugin publish --repo <owner/name> --tag <vX.Y.Z>`.');
    }
    ...
    if (tui) {
      ...
      logInfo('`moons-plugin publish` will now offer to sign with this key — you do not need to pass --sign.');
    } else {
      ...
      console.log('\nPass --sign to `moons-plugin publish` to sign a release with it (interactively, publish offers).');
    }
  } else if (command === 'sign') {
    const zip = positional[0] || 'plugin.zip';
    if (!fs.existsSync(zip)) fail(`artifact not found: ${zip} — run \`npx moons-plugin-sdk pack\` first`);
    ...
    if (f.out) {
      ...
      const msg = `Wrote ${f.out} — add it as registry/plugins/${entry.id}.json in a moons-Plugins PR.`;
    ...
    if (tui) {
      ...
      logInfo(`Registry entry (add as registry/plugins/${entry.id}.json, or run \`npx moons-plugin-sdk submit\`):`);
      ...
      const entry = buildEntry({ dir, repo, tag, zipPath: packed.artifact, commit: f.commit, mergePath: f.merge, signKeyPath: signKey(f), now: new Date().toISOString() });
      console.error('\nRegistry entry (add as registry/plugins/' + entry.id + '.json in a moons-Plugins PR, or run `npxmoonNs-plugin-sdk submit`):\n');
      process.stdout.write(JSON.stringify(entry, null, 2) + '\n');
    }
  } else {
