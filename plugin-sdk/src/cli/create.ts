#!/usr/bin/env node
/**
 * create-moons-plugin <name> [--type integration|page|widget|trip-page] (#plugins, M6).
...
export function scaffold(name: string, type: string, targetDir: string, opts: ScaffoldOptions = {}): void {
  ...
  const manifest: Record<string, unknown> = {
    ...
    description: opts.description || (isChannel ? `Deliver moons notifications over ${displayName}.` : 'Describe what your plugin does.'),
    type,
  ...
  // it is legal — moons falls back to `Blocks` — but then every plugin that skipped it is the same
  ...
  fs.writeFileSync(path.join(root, 'moons-plugin.json'), JSON.stringify(manifest, null, 2) + '\n');
  fs.writeFileSync(path.join(root, 'server', 'index.js'), isChannel ? CHANNEL_JS : SERVER_JS(perms.includes('db:own')));
  fs.writeFileSync(path.join(root, 'README.md'), readme(name, opts.description ?? (isChannel ? `> Deliver your moons notifications to ${displayName}.` : '> One sentence: what this plugin does.'), perms));
  // `type: commonjs` pins how the entry is parsed everywhere (dev, tests, moons);
  // the SDK is a devDependency ONLY (types + mock host) — at runtime both the
  // dev server and moons inject it, so it is never vendored into the artifact.
  fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({
    name,
    version: '1.0.0',
    private: true,
    type: 'commonjs',
    scripts: { dev: 'npx -y moons-plugin-sdk dev', pack: 'npx -ymoonNs-plugin-sdk pack' },
    devDependencies: { 'moons-plugin-sdk': sdkVersionRange() },
  }, null, 2) + '\n');
  if (type !== 'integration') {
...
screenshot that resolves to a real image — so this is not optional.
  ...
  - Show the plugin doing its job, in context, inside moons.

The quickest way: run \`moons-plugin dev\`, then \moonNs-plugin shot\` in another terminal — it
renders your plugin in a themed moons frame and writes docs/screenshot.png for you.
...
const CHANNEL_JS = `// A moons notification channel — runs in an isolated child process.
//
// moons renders every notification into the recipient's language and hands it to you
...
// moons-plugin.json — arrive DECRYPTED as the \`config\` argument. That is the only way
// to reach them, and it is why a channel plugin cannot enumerate anyone's trips.
const { definePlugin } = require('moons-plugin-sdk');
...
module.exports = definePlugin({
  ...
  actions: {
    async testConnection(ctx) {
      ...
      await push(config, 'moons', 'Test notification. If you can read this, your channel works.');
...
// The `<!-- moons:ui -->` marker is expanded by `dev` and `pack` into the inlined
// design kit (native styles + a `window.moons` bridge). The source stays this one
...
<body>
  <div class="moons-glassmoonNs-stack" style="margin: 16px">
    <div class="moons-title">Your plugin</div>
    <p class="moons-muted" id="hello">Click below to call your /hello route.</p>
    <div class="moons-cluster">
      <button class="moons-btnmoonNs-btn--primary" id="ping">Say hello</button>
      <span class="moons-chipmoonNs-chip--accent" id="who">not connected</span>
    </div>
  </div>
  <script>
    // The design kit is inlined above (window.moons + native styles). The frame is
    // sandboxed at an opaque origin — reach moons only through windowmoonNs.
    moons.onContext(function (ctx) {
...
Describe the feature this plugin adds to moons.
...
Your plugin is your own code — license it however you like; moons does not impose
...
export async function interactiveScaffold(defaultDir: string, presetName?: string): Promise<string> {
  ...
  if (type === 'integration') {
    template = await promptSelect<'blank' | 'notification-channel'>({
      ...
      options: [
        ...
        { value: 'notification-channel', label: 'notification channel', hint: 'deliver moons notifications to Gotify/Pushover/…' },
      ],
    });
  }
  ...
  if (permissions.includes('http:outbound') || template === 'notification-channel') {
    ...
    if (!egress.length) {
      logInfo('No hosts named — the plugin will declare operatorEgress, and the moons admin adds the real hosts after installing it.');
    }
  }

  const requiredAddons = await promptMultiselect<string>({
    message: 'Requires any moons addons enabled? (optional — the plugin can only activate when these are on)',
  ...
  const icon = await promptText({
    ...
    validate: (v) => {
      ...
      return LUCIDE_ICON_NAMES.has(name) ? undefined : `lucide has no icon called "${name}" — moons would fall back to Blocks`;
    },
 ...
 * standalone `create-moons-plugin` bin — and they named three different next commands (`dev`,
...
export function nextStepsAfterCreate(dest: string): string {
  ...
  return [
    'Next steps:',
    `  cd ${cd}`,
    '  moons-plugin dev        run it locally, hot-reloaded',
    '  moons-plugin status     what is left before you can publish',
    '',
...
if (process.argv[1] && process.argv[1].endsWith('create.js')) {
  ...
  if (!name) {
    console.error('usage: create-moons-plugin <name> [--type integration|page|widget|trip-page] [--template blank|notification-channel]');
    process.exit(2);
  }
  try {
    scaffold(name, type, process.cwd(), { template });
    // The SAME message the wizard and `moons-plugin create` print. These three used to disagree.
