/**
 * moons-plugin shot — capture docs/screenshot.png.
 *
 ...
 *
 * So: boot the dev server, render the plugin in the same themed, sandboxed frame moons uses
...
export interface ShotOptions {
  ...
  noServe?: boolean;
  /** Render in moons's dark theme instead of light. */
  dark?: boolean;
}
...
async function loadChromium(pluginDir: string): Promise<{ launch: (o?: unknown) => Promise<Chromium> }> {
  ...
  throw new ShotError(
    'moons-plugin shot needs Playwright, which is not installed.\n\n' +
...
export async function runShot(dir: string, opts: ShotOptions = {}): Promise<{ out: string; width: number; height: number }> {
  ...
  const manifestPath = path.join(root, 'moons-plugin.json');
  if (!fs.existsSync(manifestPath)) throw new ShotError(`no moons-plugin.json in ${dir} — is this a plugin directory?`);
  ...
  if (type === 'integration') {
    throw new ShotError(
      ...
      'moons surface your plugin CHANGES: the notification it sends, the badge it adds to a place, the row\n' +
  ...
  try {
    if (!opts.noServe) {
      ...
      dev = spawn(process.execPath, [path.join(import.meta.dirname, 'moons-plugin.js'), 'dev', root, '--port', String(port)], {
      ...
      if (!(await waitForPort(port))) {
        throw new ShotError(
          ...
          (devErr.trim() ? `\n\n${devErr.trim()}` : '\n\nRun `moons-plugin dev` yourself to see why.'),
        );
      }
