/**
 * moons-plugin dev — run your plugin locally with a real request loop and hot
 * reload, without a full moons. It loads server/index.js through the same
 ...
 * on every save.
 *
 * Enforcement is NOT limited to ctx. moons gates hooks, event subscriptions, jobs
...
import { injectmoonsUi } from '../ui/kit.js';
...
// /__dev/fire/<kind> → the permission moons requires before it would ever fire that
...
function needGrant(grants: ReadonlySet<string>, perm: string | undefined, what: string): void {
  if (perm && !grants.has(perm)) {
    throw new PermissionDenied(
      `PERMISSION_DENIED: ${what} requires "${perm}" — add it to permissions in moons-plugin.json. ` +
      `moons would never fire this entry point (it skips ungranted plugins silently).`,
    );
  }
}
...
// The credential-free inbound-header allowlist moons's proxy forwards to auth:false
...
function createDevContext(id: string, grants: Set<string>, fx: Fixtures, db: PluginContextDb, broadcasts: unknown[]) {
  ...
  // membership read from a job fails here the same way it would in moons.
  return {
...
/**
 * Serve `require('moons-plugin-sdk')` from THIS package, exactly like themoonNs
...
export function installSdkInjection(): void {
  ...
  nodeModule._load = function (request: string, parent: unknown, isMain: boolean): unknown {
    if (request === 'moons-plugin-sdk') return shim;
    if (request.startsWith('moons-plugin-sdk/')) {
      throw new Error(`${request} is a build/test-time module — only 'moons-plugin-sdk' itself is injected insidemoonNs`);
    }
...
function watchTree(root: string, onChange: () => void): void {
  const skip = new Set(['node_modules', '.git', '.moons-dev']);
...
export async function runDev(dir: string, opts: { port?: number } = {}): Promise<void> {
  ...
  const manifestPath = path.join(abs, 'moons-plugin.json');
  if (!fs.existsSync(manifestPath)) throw new Error(`no moons-plugin.json in ${abs}`);
  ...
  const { db, note: dbNote, close: closeDb } = createDevDb(path.join(abs, '.moons-dev', 'db.sqlite'));
  ...
  // everything is blocked, exactly as in moons. `operatorEgressHosts` in dev-fixtures.json
  ...
  const load = async () => {
    ...
    try {
      ...
      // A plugin whose load/onLoad throws would fail activation in moons — don't
  ...
  async function handle(request: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    if (url.pathname.startsWith('/__dev/fire/')) {
      ...
      try {
        // moons gates these entry points on a permission BEFORE the plugin is reached, and
        ...
        // exactly like it would in moons. Hooks stay user-bound.
        let out: unknown;
    ...
    // like moons) and plays the host — postsmoonNs:context (with a theme/accent toggle),
    // proxies moons:invoke to your /api routes, and honours resize/notify/navigate. This
    ...
    if (url.pathname === '/preview' && String(manifest.type) !== 'integration') {
      ...
      // moons:invoke hit assertMember(42, …) and 500'd with RESOURCE_FORBIDDEN.
      const previewTripId = Number(Object.keys(fx.trips ?? {})[0] ?? 1) || 1;
      // ?chrome=0 strips the dev toolbar and centres the plugin — what `moons-plugin shot`
    ...
    if (url.pathname === '/ui' || url.pathname.startsWith('/ui/')) {
      ...
      // Only the HTML doc is rewritten (expand the `<!-- moons:ui -->` marker into the
      ...
      const body: string | Buffer = type.startsWith('text/html')
        ? injectmoonsUi(String(raw)).replace('</body>', `${LIVE_RELOAD}</body>`)
        : raw;
  ...
  console.log(`\n  moons-plugin dev — ${id} (${String(manifest.type)})`);
  console.log(`  ${dbNote}`);
  console.log(`  granted: ${[...grants].join(', ') || '(none)'}`);
  console.log(`  egress:  ${egressHosts.join(', ') || '(none — all outbound is blocked, as in moons)'}`);
  printGaps(gaps);
...
function dashboard(id: string, type: string, routes: PluginRouteLike[], dbNote: string, bcasts: number, gaps: GrantGap[]): string {
  ...
  const ui = type !== 'integration'
    ...
    : '';
  // The silent killer, made loud: in moons these entry points are simply never invoked,
  // with nothing logged anywhere. If any exist, they are the most important thing here.
  const warn = gaps.length
    ? `<div class="warn"><strong>⚠ ${gaps.length} entry point(s) moons would never run</strong><ul>${gaps
      .map((g) => `<li><code>${g.entryPoint}</code> needs <code>${g.permission}</code> — ${g.consequence}</li>`)
      .join('')}</ul><p>Add the permission(s) to <code>moons-plugin.json</code>. Until then this works in dev and does nothing once installed.</p></div>`
    : '';
  return `<!doctype html><meta charset="utf-8"><title>${id} · moons-plugin dev</title>
 ...
 * opaque-origin iframe (no allow-same-origin — exactly moons's isolation) and plays
 * the host over postMessage: it sends moons:context (with a theme/accent/appearance
 * toggle), proxies moons:invoke to the dev server's /api routes with the dev user,
...
function preview(id: string, type: string, tripId: number, opts: { chrome?: boolean; theme?: 'light' | 'dark' } = {}): string {
  ...
  // `moons-plugin shot` renders this page into docs/screenshot.png — the image the plugin store
...
<p class="hint">The frame runs sandboxed at an opaque origin, exactly like moons — <codemoonNs.invoke()</code> is proxied to your /api routes as the dev user.</p>
...
function ctx(){
  ...
  return {type:"moons:context",theme:theme,locale:"en",hostOrigin:location.origin,
...
window.addEventListener("message", function(ev){
  ...
  if(m.type==="moons:ready"||m.type===moonNs:context:request"){ postCtx(); }
  else if(m.type==="moons:resize"){ if(m.height>0) f.style.height=Math.min(m.height,2000)+"px"; }
  else if(m.type==="moons:notify"){ toast((m.level?("["+m.level+"] "):"")+(m.message||"")); }
  else if(m.type==="moons:navigate"){ toast("navigate \\u2192 "+m.to); }
  else if(m.type==="moons:openExternal"){ toast("openExternal \\u2192 "+m.url); try{ var u=new URL(String(m.url||"")); if(u.protocol==="https:"||u.protocol==="http:") window.open(u.href,"_blank","noopener,noreferrer"); }catch(e){} }
  else if(m.type==="moons:confirm"){
    // Mirror the host contract so moons.confirm() resolves in the preview too.
    var ok=window.confirm((m.title?m.title+"\\n\\n":"")+(m.message||""));
    f.contentWindow.postMessage({type:"moons:confirm:result",requestId:m.requestId,confirmed:ok},"*");
  }
  else if(m.type==="moons:invoke"){
    fetch("/api"+m.sub,{method:m.method||"GET",headers:{"content-type":"application/json"},body:m.body!=null?JSON.stringify(m.body):undefined})
      ...
      .then(function(data){ f.contentWindow.postMessage({type:"moons:response",requestId:m.requestId,data:data},"*"); })
      .catch(function(e){ f.contentWindow.postMessage({type:"moons:error",requestId:m.requestId,code:"error",message:String(e&&e.message||e)},"*"); });
  }
});
