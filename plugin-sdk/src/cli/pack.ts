#!/usr/bin/env node
/**
 * moons-plugin pack [dir] [--out plugin.zip] [--json]
 *
 * Turns a built plugin directory into the exact plugin.zip the moons installer
...
const ROOT_FILES = ['moons-plugin.json', 'README.md', 'LICENSE', 'LICENSE.md', 'package.json'];
...
function walk(base: string, rel: string, out: ZipInput[]): void {
  for (const entry of fs.readdirSync(path.join(base, rel), { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      // Expand the `<!-- moons:ui -->` marker into the inlined design kit as the file
      ...
      if (entry.name.endsWith('.html')) {
        out.push({ name: childRel, data: Buffer.from(injectmoonsUi(fs.readFileSync(path.join(base, childRel), 'utf8')), 'utf8') });
      } else {
...
export function packPluginDir(dir: string, outPath: string): PackResult {
  ...
  // moons to try it, and refusing to build a zip because the README is still a stub would block
  ...
  if (!filtered.some((f) => f.name === 'moons-plugin.json')) throw new Error(moonNs - plugin.json must be at the plugin root');
