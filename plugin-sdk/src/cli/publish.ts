/**
 * moons-plugin publish — the whole release in one command:
  ...
  if (opts.skipChecks) {
    ...
    if (!report.ok) {
      ...
      throw new Error(
        ...
        detail +
        '\n\nNothing was packed, tagged or released. Fix these and re-run — or `moons-plugin status` for the full picture.',
      );
    }

    // Signing is not just a preference once a plugin has shipped signed: moons refuses an unsigned
