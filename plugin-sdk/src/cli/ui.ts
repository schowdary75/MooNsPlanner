...
/**
 * Every permission an author can grant, grouped into the areas of moons they touch.
 *
 ...
 * permission to moons ⇒ the test tells you to describe it ⇒ `create` offers it.
 *
...
export const PERMISSION_FAMILIES: PermissionFamily[] = [
  ...
  },
  {
    ...
    permissions: [
      ...
      { value: 'db:read:categories', hint: 'Read the place categories moons ships' },
  ...
  {
    id: 'hooks',
    label: 'Provider hooks (render inside moons natively)',
    hint: 'moons calls YOU. Without the matching grant a hook is NEVER invoked — silently',
    permissions: [
      { value: 'hook:photo-provider', hint: 'Supply place photos to moons' },
      { value: 'hook:calendar-source', hint: 'Supply calendar events to moons' },
      ...
      { value: 'hook:table-contributor', hint: 'Add columns to moons\'s tables' },
      ...
      { value: 'hook:notification-channel', hint: 'Deliver moons notifications over your own channel' },
  ...
  },
  {
    ...
    permissions: [
      ...
      { value: 'events:subscribe', hint: 'Receive moons events (place:created, trip:updated…) as they happen' },
    ],
  },
  {
    id: 'realtime',
  ...
  },
  {
    ...
    hint: 'Reach the outside world, or borrow a moons service',
    permissions: [
      { value: 'http:outbound', hint: 'Call external HTTP hosts (you name the hosts next)' },
      { value: 'weather:read', hint: 'Read forecasts through moons\'s weather service' },
      { value: 'rates:read', hint: 'Read currency exchange rates through moons' },
      { value: 'notify:send', hint: 'Send a moons notification to a user or trip' },
