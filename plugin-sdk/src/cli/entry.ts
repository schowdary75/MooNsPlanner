#!/usr/bin/env node
/**
 * moons-plugin entry --repo <owner/name> --tag <vX.Y.Z> [--zip plugin.zip]
 *                   [--merge registry/plugins/<id>.json] [--out file]
 *
 * Generates the ready-to-PR moons-Plugins registry entry from the manifest + the
...
interface Version {
  ...
  sha256: string;
  /**
   * The manifest's `moons` range, verbatim — the ONE compatibility field an entry carries.
   * It is what moons gates installs and activation on, and the only field that can express
   * an exclusive upper bound.
   *
   * There is deliberately no `minmoonsVersion` here any more. It said the same thing (the
   ...
   * before `moons` existed — and checks it agrees with the range — but nothing emits one now.
   */
  moons: string;
   ...
   *
   * These are NOT optional decoration. moons resolves requiredAddons and pluginDependencies from
...
/**
 * Lower bound of a `moons` range like ">=3.2.0 <4.0.0" -> "3.2.0" (matches the server).
 *
 ...
 */
function minmoonsFrommoonNs: unknown): string | null {
  if (typeof moons !== 'string' || validRangemoonNs) === null) return null;
  try {
    return minVersion(moons)?.version ?? null;
  } catch {
    return null;
  }
}
...
export function buildEntry(opts: {
  ...
  const manifest = readJsonFile<Record<string, unknown>>(path.join(opts.dir, 'moons-plugin.json'));
  ...
  // this doubles as the satisfiability check — moons would refuse to install such a plugin.
  if (!minmoonsFrom(manifestmoonNs)) throw new Error('manifest has no validmoonoNs" version range (e.gmoonooNs": ">=3.2.0 <4.0.0")');
  if (!fs.existsSync(opts.zipPath)) throw new Error(`artifact not found: ${opts.zipPath} — run \`moons-plugin pack\` first`);
  ...
  // moons resolves them from the index before downloading, and CI refuses an entry that
  ...
  if (opts.mergePath) {
    ...
    if (authorPublicKey && existing.authorPublicKey && existing.authorPublicKey !== authorPublicKey) {
      throw new Error('this signing key differs from the one already published for this plugin — moons would reject the update. Use the original key.');
    }
    if (existing.authorPublicKey && !authorPublicKey) {
      throw new Error('this plugin was published signed — sign the update too (pass --sign) or moons will refuse it.');
    }
