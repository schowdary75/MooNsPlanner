/**
 * moons-plugin submit — open themoonNs-Plugins registry PR for you. Forks the
...
const DEFAULT_REGISTRY = 'schowdary75/MooNsPlanner-Plugins';
...
function mergeOnto(existing: EntryLike, fresh: EntryLike): EntryLike {
  ...
  if (existing.authorPublicKey && fresh.authorPublicKey && existing.authorPublicKey !== fresh.authorPublicKey) {
    throw new Error('the signing key differs from the one already published for this plugin — moons would reject the update. Use the original key.');
  }
...
export function submitEntry(entry: EntryLike, opts: { registry?: string; branch?: string; draft?: boolean; keep?: boolean } = {}): { prUrl: string } {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'moons-submit-'));
  try {
    ...
    const body = [
      ...
      '',
      'Generated with `moons-plugin submit`. CI validates the tag, artifact hash, manifest parity and README.',
    ].join('\n');
