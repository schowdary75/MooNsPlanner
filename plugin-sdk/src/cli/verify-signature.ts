/**
 * Author-signature verification for `moons-plugin preflight`.
 *
 * This is a port of moons's installer-side verifier (server/src/nest/plugins/install/
 * verify-signature.ts), and of the registry's port of it (moons-Plugins'
 ...
 *
 * moons refuses to install a half-signed entry ("incomplete signature: an author key and a
...
export function checkSignatureShape(entry: { authorPublicKey?: string; versions?: Array<{ version: string; signature?: string }> }): string[] {
  ...
  for (const v of versions) {
    if (v.signature && !key) {
      problems.push(`${v.version}: has a signature but the entry has no authorPublicKey — moons refuses to install a half-signed entry`);
    }
    if (v.signature) {
      try {
  ...
  // A key with no signed version at all is a publishing mistake: moons pins the key on
