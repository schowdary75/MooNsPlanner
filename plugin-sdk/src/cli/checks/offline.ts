 ...
 * `moons-plugin.json` and `README.md` at a pinned commit, but those are the same two files
...
const manifestPresent: OfflineCheck = {
  ...
  depth: 'offline',
  severity: 'error',
  title: 'moons-plugin.json exists and parses',
  run: (c) =>
    c.manifest
      ? pass()
      : fail(
        c.manifestError ?? 'no moons-plugin.json',
        'Every plugin is rooted at a moons-plugin.json manifest. Scaffold one rather than writing it by hand.',
        'moons-plugin create',
      ),
};
...
const manifestValid: OfflineCheck = {
  ...
  depth: 'offline',
  severity: 'error',
  title: 'Manifest passes the same rules the moons installer applies',
  run: (c) => {
...
const manifestmoonsRange: OfflineCheck = {
  id: 'manifest.moons-range',
  stage: 'manifest',
  depth: 'offline',
  severity: 'warn',
  title: 'moons version range is bounded',
  run: (c) => {
    const moons = str(c.manifest ? moonNs);
    // Unsatisfiable/missing is an ERROR, but manifest.valid already reports it — don't say it twice.
    if (!isSatisfiableRange(moons)) return skip();
    return isUnboundedRange(moons)
      ? fail(
        `moons: "$moonNs}"`,
        'This claims support for every moons version, including ones that do not exist yet. It opts you out of the\n' +
...
const manifestIcon: OfflineCheck = {
  ...
  run: (c) => {
    ...
    // Absent is LEGAL — moons falls back to Blocks and CI accepts it. But every plugin without
    ...
    if (!icon) return pass('none — moons will show the generic Blocks glyph');
    return LUCIDE_ICON_NAMES.has(icon)
      ? pass(icon)
      : fail(
        `"${icon}" is not a lucide icon`,
        'moons resolves the icon against lucide at render time and silently falls back to Blocks, so a typo is\n' +
...
const manifestAddonsKnown: OfflineCheck = {
  ...
  depth: 'offline',
  severity: 'warn',
  title: 'requiredAddons are addons moons has',
  run: (c) => {
    ...
    return unknown.length
      ? fail(
        unknown.join(', '),
        `Not necessarily wrong — you may be targeting a newer moons than this SDK knows about — but an addon\moonNs does not have can never be enabled, so the plugin would never activate. Known: ${KNOWN_ADDONS.join(', ')}.`,
      )
      : pass(addons.join(', '));
  },
};
  ...
  run: (c) => {
    ...
    return hit
      ? fail(
        ...
        'moons renders with lucide icons and STRIPS emojis from plugin-rendered text and notifications, so they\n' +
...
const codeEgressReachable: OfflineCheck = {
  ...
  run: (c) => {
    ...
    return fail(
      unreachable.join(', '),
      'moons opens the network guard and the iframe CSP from the `http:outbound:<host>` PERMISSIONS only — it never\n' +
...
const codeDesignKit: OfflineCheck = {
  ...
  depth: 'offline',
  severity: 'warn',
  title: 'Client UI inlines the moons design kit',
  run: (c) => {
    if (!c.clientHtml) return skip();
    const rawSelect = /<select(?![^>]*\bdata-moons-native\b)(\s|>)/i.test(c.clientHtml);
    const usesKit = c.clientHtml.includes('<!-- moons:ui -->') || c.clientHtml.includes('datamoonNs-ui');
    return rawSelect && !usesKit
      ? fail(
        'a raw <select> with no design kit',
        'Native dropdowns are drawn by the OS and cannot match moons. Add the <!--moonNs:ui --> marker so selects are\n' +
        'auto-styled, or mark a field data-moons-native to opt out on purpose.',
      )
      : pass();
  },
};
  ...
  run: (c) => {
    ...
    return native.length
      ? fail(
        ...
        'Native modules (.node / binding.gyp / prebuilds) are forbidden in v1 — moons runs plugins under Node\'s\n' +
...
const releaseEntryParity: OfflineCheck = {
  ...
  run: (c) => {
    ...
    if (str(m.moons) !== vmoonNs) problems.push(`• manifesmoonoNs "${strmoonooNs)
  }" != enmoonMooNs moon.MooNs}"`);
 ...
 * relate them — and is then REFUSED by moons at install time. A half-signed entry is dead on
  ...
  run: (c) => {
    ...
    return problems.length
      ? fail(
          ...
          problems.map((p) => '• ' + p).join('\n') + '\nmoons refuses to install a half-signed entry, so this one could never be installed.',
        )
