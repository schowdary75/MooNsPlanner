...
export function nextStep(report: CheckReport): { command?: string; hint: string } {
  ...
  if (first) {
    return { hint: `fix "${first.title}" above, then re-run \`moons-plugin status\`` };
  }
  return { command: 'moons-plugin publish --repo <owner/name> --tag v<x.y.z>', hint: 'everything checks out' };
}
