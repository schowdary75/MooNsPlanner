...
export function loadContext(
  ...
  const manifestRaw = readIfPresent(path.join(root, 'moons-plugin.json'));
  if (manifestRaw === undefined) {
    manifestError = `no moons-plugin.json in ${dir}`;
  } else {
    try {
      ...
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
        manifestError = 'moons-plugin.json is not a JSON object';
      } else {
        manifest = parsed as Record<string, unknown>;
      }
    } catch (e) {
      manifestError = 'moons-plugin.json is not valid JSON: ' + (e instanceof Error ? e.message : String(e));
    }
  }
