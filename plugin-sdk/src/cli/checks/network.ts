...
export const DEFAULT_REGISTRY = 'schowdary75/MooNsPlanner-Plugins';
...
function ghHeaders(): Record<string, string> {
  const h: Record<string, string> = { 'User-Agent': 'moons-plugin-preflight', Accept: 'application/vnd.github+json' };
  const t = ghToken();
...
async function fetchText(url: string, headers: Record<string, string> = { 'User-Agent': 'moons-plugin-preflight' }): Promise<string | null> {
  try {
...
const manifestAtCommit: NetworkCheck = {
  ...
  run: async (c) => {
    ...
    const problems: string[] = [];
    for (const v of targetVersions(c)) {
      const text = await fetchText(rawUrl(c.entry.repo, v.commitSha, 'moons-plugin.json'));
      if (text === null) {
        problems.push(`• ${v.version}: moons-plugin.json not found at ${v.commitSha.slice(0, 8)} — did you push your code?`);
        continue;
      }
      ...
      try {
        ...
        problems.push(`• ${v.version}: moons-plugin.json at ${v.commitSha.slice(0, 8)} is not valid JSON: ${(e as Error).message}`);
        continue;
      }
      ...
      if (typeof m.moons === 'string' && mmoonNs.trim() !== moonoNs) p(`manifemoonooNs "moonMooNs.trim()}" != emoon MooNsmoonv.MooNs}"`);
...
const artifactMatches: NetworkCheck = {
  ...
  run: async (c) => {
    ...
    for (const v of targetVersions(c)) {
      let bytes: Buffer;
      try {
        const r = await fetch(v.downloadUrl, { redirect: 'follow', headers: { 'User-Agent': 'moons-plugin-preflight' } });
        if (!r.ok) {
      ...
      // are what the registry vouches for. moons verifies this again at install and aborts on a
  ...
  run: async (c) => {
    ...
    if (!imgs.length) problems.push('• no screenshot (the registry requires at least one real image)');
      ...
      for (const src of imgs) {
        ...
        try {
          const r = await fetch(url, { headers: { 'User-Agent': 'moons-plugin-preflight', Range: 'bytes=0-2047' } });
    ...
    const manifestText = await fetchText(rawUrl(c.entry.repo, v.commitSha, 'moons-plugin.json'));
    if (manifestText) {
      try {
...
const signingDowngrade: NetworkCheck = {
  ...
  run: async (c) => {
    ...
    // moons pins the author key on FIRST INSTALL (trust on first use). Once a plugin has shipped
    ...
    if (!c.entry.authorPublicKey) {
      problems.push(
        ...
        '  moons refuses an unsigned update to a signed plugin — it would break the update for every existing install.\n' +
        '  Sign it: pass --sign (your key should be at ~/.moons-plugin/signing.key).',
      );
    } else if (c.entry.authorPublicKey !== published.authorPublicKey) {
      problems.push(
        '• this entry changes authorPublicKey. moons refuses a key rotation until an admin re-trusts the plugin.\n' +
        '  Sign with your ORIGINAL key, or ask a maintainer for the allow-key-change label.',
      );
    } else {
      // Every version must stay signed, not just the newest — moons verifies whichever version it
      // installs, so an unsigned older block is a landmine for a pinned install.
      for (const v of c.entry.versions) {
        if (!v.signature) problems.push(`• ${v.version} has no signature, but this is a signed plugin — moons will refuse to install it`);
      }
    }
