/**
 * The check registry: every gate the moons-Plugins registry enforces, declared ONCE.
 *
 * The problem this solves: the registry's gates are, almost all of them, functions of two
 * files that are sitting in the author's working directory (`moons-plugin.json`, `README.md`).
 ...
 * author trusts a green. When moons-Plugins' scripts/validate-entry.mjs or check-readme.mjs
 ...
 *
 * The bar for 'error' is: THE REGISTRY WOULD REJECT THIS, or moons WOULD REFUSE TO RUN IT.
 ...
 * into a local moons to try it. Blocking the dev loop on a docs gate that only matters at publish
...
export interface CheckResult {
  ...
  fix?: string;
  /** The literal command that fixes this, e.g. `moons-plugin shot`. Drives "next →". */
  next?: string;
}
...
export interface CheckContext {
  ...
  dir: string;
  /** Parsed `moons-plugin.json`, or undefined when it is missing or unparseable. */
  manifest?: Record<string, unknown>;
  /** Why the manifest is absent, when it is — "no moons-plugin.json" / a JSON parse error. */
...
/** The shape `entry.ts` builds and moons-Plugins stores at registry/plugins/<id>.json. */
