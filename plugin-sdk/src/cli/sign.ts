 ...
 * the registry entry (`authorPublicKey`) and a per-version `signature`, and moons
...
export function defaultKeyPath(): string {
  return path.join(os.homedir(), '.moons-plugin', 'signing.key');
}
...
export function loadPrivateKey(keyPath: string): crypto.KeyObject {
  if (!fs.existsSync(keyPath)) {
    throw new Error(`no signing key at ${keyPath} — run \`moons-plugin keygen\` first (or pass --key <file>)`);
  }
