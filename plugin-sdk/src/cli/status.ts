/**
 * moons-plugin status [dir] — where am I, and what do I do next?
 *
...
export function runStatus(dir: string, opts: StatusOptions = {}): { ok: boolean; text: string } {
  ...
  if (!fs.existsSync(path.join(root, 'moons-plugin.json'))) {
    return {
      ok: false,
      text:
        `  No plugin here.\n\n  ${path.relative(process.cwd(), root) || '.'} has no moons-plugin.json.\n\n` +
        '  next →  moons-plugin create',
    };
  }
