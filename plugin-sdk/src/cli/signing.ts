...
export interface SigningState {
  ...
  hasKey: boolean;
  keyPath: string;
  /** This plugin is ALREADY PUBLISHED SIGNED — moons refuses an unsigned update to it. */
...
export async function inspectSigning(
  ...
  try {
    const r = await fetch(`https://raw.githubusercontent.com/${registry}/main/registry/plugins/${pluginId}.json`, {
      headers: { 'User-Agent': 'moons-plugin' },
    });
...
export function assertSigningAllowed(state: SigningState, signKeyPath: string | undefined): void {
  ...
  throw new Error(
    ...
    'moons pins the author key on first install. An unsigned update to a signed plugin is REFUSED on every\n' +
...
export async function proposeSigning(state: SigningState): Promise<string | undefined> {
  ...
  if (state.publishedSigned) {
    ...
    logWarn('This plugin was published signed — moons refuses an unsigned update, so this release will be signed.');
  ...
  note(
    ...
    'moons pins your key the first time someone installs a signed version, and refuses anything not signed\n' +
    'with it after that.\n\n' +
  ...
  if (!create) {
    logWarn('Publishing unsigned. You can add signing later with `moons-plugin keygen`, and it will break nobody.');
    return undefined;
  }
