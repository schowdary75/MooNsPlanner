 ...
 * VERBATIM from the server's runtime/egress-policy.ts — moons-plugin-sdk ships
 ...
 * refused, so a plugin can't pivot to moons.db's host, the 169.254.169.254
 ...
 *
 * `moons_PLUGIN_ALLOW_PRIVATE_EGRESS=on` permits private/loopback targets — the SAME
...
export function installEgressGuard(hosts: string[]): RestoreEgress {
  ...
  const blockPrivate = (process.env.moons_PLUGIN_ALLOW_PRIVATE_EGRESS ?? '').toLowerCase() !== 'on';
  ...
  const refuse = (host: string): Error =>
    new Error(
      ...
      `add "http:outbound:${host}" to permissions (and "${host}" to egress) in moons-plugin.json`,
    );
