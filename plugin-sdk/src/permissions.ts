/**
 * The permission families moons enforces OUTSIDE the ctx object — and the single
...
/**
 * hooks.<key> → the permission that must ALSO be granted for moons to ever call it.
...
export interface GrantGap {
  ...
  permission: string;
  /** What moons does instead — always a silent no-op, hence the warning. */
  consequence: string;
}

/**
 * Every entry point the plugin implements without the permission that makes moons
...
export function grantGaps(plugin: PluginEntryPoints, grants: ReadonlySet<string>): GrantGap[] {
  ...
  const gap = (entryPoint: string, permission: string, noun: string) => {
    if (!grants.has(permission)) gaps.push({ entryPoint, permission, consequence: `moons will never ${noun}` });
  };
...
/**
 * The hosts a plugin may actually reach. moons builds its runtime network allowlist
