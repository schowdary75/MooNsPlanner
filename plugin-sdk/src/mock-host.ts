...
/**
 * A mock PluginContext for unit-testing a plugin without a running moons
 ...
 * SQL).
 *
 * That applies to the DRIVER too (`run(def)`), not just ctx: moons gates hooks, event
...
export interface MockHostOptions {
   ...
   * definition cannot name them in its manifest. Only `moons-plugin dev` reads this (to
...
export function createMockHost(opts: MockHostOptions = {}): MockHost {
  ...
  // Same field caps the host applies over the open @moons/shared write schemas.
  ...
  // moons gates these entry points BEFORE the plugin is reached: an ungranted hook is
  ...
  const needEntry = (perm: string, what: string) => {
    if (!grants.has(perm)) {
      throw new PermissionDenied(
        `PERMISSION_DENIED: ${what} requires ${perm} — moons never fires an entry point the manifest did not grant`,
      );
    }
  };
...
// `moons-plugin-sdk/testing` resolves to this module, so re-export what a test needs to
