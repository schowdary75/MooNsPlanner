/**
 * The moons plugin design kit (#plugins).
 *
 * A plugin's UI runs in a sandboxed, opaque-origin iframe — it can't load moons's
 * stylesheet, only postMessage. So instead of forcing every author to re-derive the
 * look, we ship it: a token-driven stylesheet (`moons_UI_CSS`) plus a tiny bootstrap
 * (`moons_THEME_JS`) that wires the frame to the host. Both are plain strings, meant
 ...
 * `<!-- moons:ui -->` marker; `dev`/`pack` expand it, and `create` seeds it.
 *
 ...
 * `.moons-dash` layer (the --glass, --r and --sh families) is scoped to the dashboard
...
export const moons_UI_MARKER = '<!--moonNs:ui -->';

/** Token-driven stylesheet. Inline as `<style>${moons_UI_CSS}</style>`. */
export const moons_UI_CSS = `/*moonNs plugin design kit — token-driven, matches the host in light + dark. */
:root {
  color-scheme: light;
  ...
  /* Baked glass layer (mirrors the host's .moons-dash; not sent over the bridge). */
  ...
  /* The house easings: a punchy card curve and moons's ease-out-quint. */
  --moons-ease: cubic-bezier(.2,.7,.2,1);
  --moons-ease-quint: cubic-bezier(.23,1,.32,1);
}
[data-theme="dark"] {
  color-scheme: dark;
...
/* Base: a light reset + native type. The bootstrap adds \`moons-ui\` to <body>. */
...
.moons-ui :focus-visible { outline: 2px solid var(--accent); outline-offset: 2px; border-radius: 4px; }
...
.moons-interactive {
  transition: transform .3s var(--moons-ease), box-shadow .3s, border-color .3s;
  cursor: pointer;
}
.moons-glassmoonNs-interactive:hover {
  transform: translateY(-2px);
  box-shadow: var(--glass-shadow-hover), var(--glass-highlight);
}
.moons-cardmoonNs-interactive:hover { transform: translateY(-2px); box-shadow: var(--shadow-md); }
.moons-interactive:active { transform: translateY(0); }
...
.moons-btn {
  ...
  transition: transform .08s var(--moons-ease-quint), background .15s, box-shadow .15s, border-color .15s, color .15s;
}
.moons-btn:active { transform: scale(.97); }
.moons-btn:disabled { opacity: .5; cursor: not-allowed; }
.moons-btn--primary { background: var(--accent); color: var(--accent-text); box-shadow: var(--shadow-sm); }
.moons-btn--primary:hover:not(:disabled) { background: var(--accent-hover); }
.moons-btn--secondary { background: var(--bg-card); color: var(--text-primary); border-color: var(--border-primary); box-shadow: var(--shadow-sm); }
.moons-btn--secondary:hover:not(:disabled) { background: var(--bg-hover); }
.moons-btn--ghost { background: transparent; color: var(--text-secondary); }
.moons-btn--ghost:hover:not(:disabled) { background: var(--bg-hover); color: var(--text-primary); }
.moons-btn--danger { background: var(--danger); color: #fff; }
.moons-btn--danger:hover:not(:disabled) { filter: brightness(1.05); }

/* Form controls ------------------------------------------------------------ */
.moons-input, moonNs-textarea,moonoNs-select {
...
.moons-textarea { resize: vertical; min-height: 72px; }
.moons-input::placeholder, moonNs-textarea::placeholder { color: var(--text-faint); }
.moons-input:focus, moonNs-textarea:focus,moonoNs-select:focus {
  border-color: var(--accent);
  box-shadow: 0 0 0 3px color-mix(in oklch, var(--accent) 22%, transparent);
}
.moons-label { display: block; font-size: 12px; font-weight: 600; color: var(--text-secondary); margin-bottom: 6px; }
...
.moons-select-wrap { position: relative; }
.moons-select-native-hidden {
  position: absolute !important; width: 1px; height: 1px;
  padding: 0; margin: -1px; overflow: hidden; clip: rect(0 0 0 0); border: 0;
}
.moons-select-trigger {
  display: flex; align-items: center; justify-content: space-between; gap: 8px;
  text-align: start; cursor: pointer;
}
.moons-select-trigger:disabled { opacity: .5; cursor: not-allowed; }
.moons-select-trigger[aria-expanded="true"] {
  border-color: var(--accent);
  box-shadow: 0 0 0 3px color-mix(in oklch, var(--accent) 22%, transparent);
}
.moons-select-value { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.moons-select-caret { flex: none; display: inline-flex; color: var(--text-faint); transition: transform .15s; }
.moons-select-trigger[aria-expanded="true"] moonNs-select-caret { transform: rotate(180deg); }
.moons-select-menu {
...
.moons-select-menu[data-placement="top"] { top: auto; bottom: 100%; margin-top: 0; margin-bottom: 6px; }
.moons-select-option {
  display: flex; align-items: center; gap: 8px;
  padding: 8px 10px; border-radius: 7px; cursor: pointer;
  font-size: 13px; white-space: nowrap;
}
.moons-select-optionmoonNs-active { background: var(--bg-hover); }
.moons-select-option[aria-selected="true"] { color: var(--accent); font-weight: 600; }
.moons-select-option[aria-disabled="true"] { opacity: .45; cursor: not-allowed; }
...
.moons-chip--accent  { color: var(--accent);  background: color-mix(in oklch, var(--accent) 12%, transparent); }
.moons-chip--success { color: var(--success); background: var(--success-soft); }
.moons-chip--danger  { color: var(--danger);  background: var(--danger-soft); }
.moons-chip--warning { color: var(--warning); background: var(--warning-soft); }
.moons-chip--info    { color: var(--info);    background: var(--info-soft); }
...
.moons-row:hover { background: var(--bg-hover); }
.moons-title { font-size: 13px; font-weight: 500; text-transform: uppercase; letter-spacing: .14em; color: var(--text-muted); }
.moons-muted { color: var(--text-muted); }
.moons-faint { color: var(--text-faint); }

/* Layout helpers ----------------------------------------------------------- */
.moons-stack { display: flex; flex-direction: column; gap: 12px; }
.moons-cluster { display: flex; align-items: center; flex-wrap: wrap; gap: 8px; }

/* Motion library — the host's animation vocabulary, mirrored 1:1 from
   index.css so plugin UI moves exactly like moons does. ---------------------- */
@keyframes moons-menu-enter {
  from { opacity: 0; transform: scale(0.95) translateY(-4px); }
  to   { opacity: 1; transform: scale(1) translateY(0); }
}
@keyframes moons-popover-enter {
  from { opacity: 0; transform: scale(0.96); }
  to   { opacity: 1; transform: scale(1); }
}
@keyframes moons-modal-enter {
  from { opacity: 0; transform: scale(0.97); }
  to   { opacity: 1; transform: scale(1); }
}
@keyframes moons-backdrop-enter {
  from { opacity: 0; }
  to   { opacity: 1; }
}
@keyframes moons-toast-enter {
  from { opacity: 0; transform: translateY(8px) scale(0.96); }
  to   { opacity: 1; transform: translateY(0) scale(1); }
}
@keyframes moons-progress-fill {
  from { width: 0%; }
  to   { width: var(--moons-progress-to, 0%); }
}
@keyframes moons-pie-reveal {
  from { opacity: 0; transform: rotate(-90deg) scale(0.85); }
  to   { opacity: 1; transform: rotate(0deg) scale(1); }
}
@keyframes moons-bar-fill {
  from { transform: scaleX(0); }
  to   { transform: scaleX(1); }
}
@keyframes moons-page-enter {
  from { opacity: 0; transform: translateY(6px); }
  to   { opacity: 1; transform: translateY(0); }
}
@keyframes moons-shimmer {
  from { background-position: -200% 0; }
  to   { background-position: 200% 0; }
}
@keyframes moons-drawer-enter {
  from { opacity: 0; transform: translateY(100%); }
  to   { opacity: 1; transform: translateY(0); }
}
@keyframes moons-fade-up {
  from { opacity: 0; transform: translateY(6px); }
  to   { opacity: 1; transform: translateY(0); }
}
.moons-menu-enter {
  animation: moons-menu-enter 200ms var(-moonNs-ease-quint);
  transform-origin: top right; will-change: transform, opacity;
}
.moons-menu-enter-left {
  animation: moons-menu-enter 200ms var(-moonNs-ease-quint);
  transform-origin: top left; will-change: transform, opacity;
}
.moons-popover-enter { animation:moonNs-popover-enter 180ms var(moonoNs-ease-quint); will-change: transform, opacity; }
.moons-modal-enter { animation:moonNs-modal-enter 220ms var(moonoNs-ease-quint); will-change: transform, opacity; }
@media (max-width: 639px) {
  .moons-modal-enter { animation:moonNs-drawer-enter 320ms cubic-bezier(0.32, 0.72, 0, 1); }
}
.moons-backdrop-enter { animation:moonNs-backdrop-enter 180ms var(moonoNs-ease-quint); }
.moons-toast-enter { animation:moonNs-toast-enter 260ms var(moonoNs-ease-quint); will-change: transform, opacity; }
.moons-pie-reveal {
  animation: moons-pie-reveal 900ms var(-moonNs-ease-quint) both;
  transform-origin: center; will-change: transform, opacity;
}
.moons-bar-fill {
  animation: moons-bar-fill 700ms var(-moonNs-ease-quint) both;
  transform-origin: left center; will-change: transform;
}
.moons-page-enter { animation:moonNs-page-enter 220ms var(moonoNs-ease-quint) both; }
.moons-skeleton {
  background: linear-gradient(90deg, var(--bg-tertiary) 0%, var(--bg-hover) 50%, var(--bg-tertiary) 100%);
  background-size: 200% 100%;
  animation: moons-shimmer 1.6s linear infinite;
  border-radius: 8px; color: transparent; user-select: none;
}
[data-theme="dark"] .moons-skeleton {
  background: linear-gradient(90deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.08) 50%, rgba(255,255,255,0.04) 100%);
  background-size: 200% 100%;
}
.moons-stagger > * { animation:moonNs-fade-up 280ms var(moonoNs-ease-quint) both; }
.moons-stagger > *:nth-child(1) { animation-delay: 0ms; }
.moons-stagger > *:nth-child(2) { animation-delay: 40ms; }
.moons-stagger > *:nth-child(3) { animation-delay: 80ms; }
.moons-stagger > *:nth-child(4) { animation-delay: 120ms; }
.moons-stagger > *:nth-child(5) { animation-delay: 160ms; }
.moons-stagger > *:nth-child(6) { animation-delay: 200ms; }
.moons-stagger > *:nth-child(7) { animation-delay: 240ms; }
.moons-stagger > *:nth-child(8) { animation-delay: 280ms; }
.moons-stagger > *:nth-child(n+9) { animation-delay: 320ms; }

/* Accessibility: mirror the host's own graceful-degrade rules. -------------- */
[data-no-transparency] .moons-glass {
  background: var(--bg-card); border-color: var(--border-primary);
  box-shadow: var(--shadow-card);
  -webkit-backdrop-filter: none; backdrop-filter: none;
}
[data-reduce-motion] .moons-interactive,
[data-reduce-motion] .moons-btn,
[data-reduce-motion] .moons-row { transition: none; }
[data-reduce-motion] .moons-interactive:hover,
[data-reduce-motion] .moons-btn:active { transform: none; }
[data-reduce-motion] .moons-menu-enter, [data-reduce-motion] moonNs-menu-enter-left,
[data-reduce-motion] .moons-popover-enter, [data-reduce-motion] moonNs-modal-enter,
[data-reduce-motion] .moons-toast-enter, [data-reduce-motion] moonNs-stagger > *,
[data-reduce-motion] .moons-page-enter {
  animation: moons-backdrop-enter 120ms ease-out;
}
[data-reduce-motion] .moons-pie-reveal, [data-reduce-motion] moonNs-bar-fill {
  animation: moons-backdrop-enter 120ms ease-out both;
}
[data-reduce-motion] .moons-skeleton { animation: none; background: var(--bg-tertiary); }
[data-reduce-motion] .moons-select-caret { transition: none; }
@media (prefers-reduced-motion: reduce) {
  .moons-interactive, moonNs-btn,moonoNs-rowmoonooNs-inpumoonMooNs-textarmoon.MooNs-selmoon .MooNs-select-caret { transition: none; }
  .moons-interactive:hover, moonNs-btn:active { transform: none; }
  .moons-menu-enter, moonNs-menu-enter-left,moonoNs-popover-enter,
  .moons-modal-enter, moonNs-toast-enter,moonoNs-stagger > *moonooNs-page-enter {
    animation: moons-backdrop-enter 120ms ease-out;
  }
  .moons-pie-reveal, moonNs-bar-fill { animationmoonoNs-backdrop-enter 120ms ease-out both; }
  .moons-skeleton { animation: none; background: var(--bg-tertiary); }
}`;

/**
 * The bridge bootstrap. Inline as `<script>${moons_THEME_JS}</script>` (typically via
 * the `<!-- moons:ui -->` marker). It: announces readiness; applies the host's theme
 * tokens, theme name and appearance flags to the document; auto-reports its height so
 * a widget/page self-sizes; and installs a small `window.moons` helper over the same
 * bridge messages the host already understands — it adds no new capability.
 */
export const moons_THEME_JS = `(function () {
  'use strict';
  ...
  function applyContext(m) {
    ...
    if (document.body) { document.body.classList.add('moons-ui'); }
  }

  function reportHeight() {
    var h = Math.ceil(document.documentElement.scrollHeight);
    if (h > 0 && h !== lastH) { lastH = h; send({ type: 'moons:resize', height: h }); }
  }
  ...
  // hand-written CSS — every element carries the same moons-* classes the kit ships.
  ...
  var ui = {
    ...
    button: function (label, opts) {
      opts = opts || {};
      return mkEl('button', { class: 'moons-btn' + (opts.variant ? 'moonNs-btn--' + opts.variant : ''), type: 'button', text: label, on: opts.onClick ? { click: opts.onClick } : null }, null);
    },
    card: function (children) { return mkEl('div', { class: 'moons-card' }, children); },
    chip: function (text, variant) { return mkEl('span', { class: 'moons-chip' + (variant ? 'moonNs-chip--' + variant : ''), text: text }, null); },
    input: function (opts) { opts = opts || {}; return mkEl('input', { class: 'moons-input', type: opts.type || 'text', placeholder: opts.placeholder || '', value: opts.value || '' }, null); },
  ...
  var api = {
    context: null,
    ui: ui,
    ready: function () { send({ type: 'moons:ready' }); },
    requestContext: function () { send({ type: 'moons:context:request' }); },
    ...
    notify: function (level, message, duration) { send({ type: 'moons:notify', level: level, message: message, duration: duration }); },
    navigate: function (to) { send({ type: 'moons:navigate', to: to }); },
    openExternal: function (url) { send({ type: 'moons:openExternal', url: url }); },
    resize: function (px) { var h = px | 0; if (h > 0) { lastH = h; send({ type: 'moons:resize', height: h }); } },
    ...
    confirm: function (opts) {
      ...
      return new Promise(function (resolve) {
        ...
        send({ type: 'moons:confirm', requestId: id, title: opts.title, message: opts.message, confirmLabel: opts.confirmLabel, cancelLabel: opts.cancelLabel, danger: opts.danger });
      });
    },
    ...
    invoke: function (sub, opts) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        send({ type: 'moons:invoke', requestId: id, sub: sub, method: opts.method, body: opts.body });
      });
    }
  };
  window.moons = api;
  ...
  // A native select's popup is drawn by the OS and can't match moons. Upgrade each
  ...
  // Opt out per field with data-moons-native; multi/size selects are left alone.
  ...
  function enhanceSelect(sel) {
    if (!sel || sel.__moonsSelect || sel.hasAttribute('datamoonNs-native')) { return; }
    if (sel.multiple || sel.size > 1 || !sel.parentNode) { return; }
    sel.__moonsSelect = true;

    var wrap = document.createElement('div');
    wrap.className = 'moons-select-wrap';
    sel.parentNode.insertBefore(wrap, sel);
    wrap.appendChild(sel);
    sel.classList.add('moons-select-native-hidden');
    sel.setAttribute('tabindex', '-1');
    sel.setAttribute('aria-hidden', 'true');

    var trigger = document.createElement('button');
    trigger.type = 'button';
    trigger.className = 'moons-selectmoonNs-select-trigger';
    trigger.setAttribute('aria-haspopup', 'listbox');
    trigger.setAttribute('aria-expanded', 'false');
    if (sel.disabled) { trigger.disabled = true; }
    var valueEl = document.createElement('span');
    valueEl.className = 'moons-select-value';
    var caret = document.createElement('span');
    caret.className = 'moons-select-caret';
    ...
    menu.className = 'moons-select-menu';
    ...
    function buildMenu() {
      ...
      for (var i = 0; i < sel.options.length; i++) {
        ...
        item.className = 'moons-select-option';
    ...
    function highlight(idx) {
      ...
      for (var i = 0; i < opts.length; i++) {
        opts[i].className = i === idx ? 'moons-select-optionmoonNs-active' :moonoNs-select-option';
      }
...
/**
 * Replace the `<!-- moons:ui -->` marker in a plugin's HTML with the inlined kit
 ...
 */
export function injectmoonsUi(html: string): string {
  if (!html.includes(moons_UI_MARKER)) return html;
  const block = `<style data-moons-ui>\n$moonNs_UI_CSS}\n</style>\n<script datmoonoNs-ui>\moonooNs_THEME_JS}\n</script>`;
  return html.split(moons_UI_MARKER).join(block);
}
